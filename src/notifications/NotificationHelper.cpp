...
bool CNotificationHelper::CheckAndHandleNotification(const uint64_t DevRowIdx, const int HardwareID, const std::string &ID, const std::string &sName, const unsigned char unit, const unsigned char cType, const unsigned char cSubType, const int nValue, const std::string &sValue, const float fValue) {
	...
	switch(cType) {
		case pTypeP1Power:
			nexpected = 5;
			if (nsize >= nexpected) {
				return CheckAndHandleNotification(DevRowIdx, sName, cType, cSubType, notification::type::USAGE, (float)atof(strarray[4].c_str()));
			}
			break;
		case pTypeRFXSensor:
			switch(cSubType) {
		...
		case pTypeSetpoint:
			switch(cSubType) {
				case sTypeSetpoint:
					return CheckAndHandleNotification(DevRowIdx, sName, cType, cSubType, notification::type::SETPOINT, fValue);
				default:
					break;
			}
			break;
		...
		case pTypeTEMP_HUM_BARO:
			...
			if (nsize >= nexpected) {
				...
				r1 = CheckAndHandleTempHumidityNotification(DevRowIdx, sName, Temp, Hum, true, true);
				r2 = CheckAndHandleDewPointNotification(DevRowIdx, sName, Temp, dewpoint);
				r3 = CheckAndHandleNotification(DevRowIdx, sName, cType, cSubType, notification::type::BARO, (float)atof(strarray[3].c_str()));
				return r1 && r2 && r3;
			}
			break;
		case pTypeRAIN:
			...
			if (nsize >= nexpected) {
				...
				r1 = CheckAndHandleRainNotification(DevRowIdx, sName, cType, cSubType, notification::type::RAIN, fValue2);
				r2 = CheckAndHandleRainNotification(DevRowIdx, sName, cType, cSubType, notification::type::RAINRATE, rainRate);
				return r1 && r2;
			}
			break;
		case pTypeTEMP_BARO:
			...
			if (nsize >= nexpected) {
				...
				r2 = CheckAndHandleNotification(DevRowIdx, sName, cType, cSubType, notification::type::BARO, Baro);
				return r1 && r2;
			}
			break;
		case pTypeUV:
			...
			if (nsize >= nexpected) {
				...
				if (cSubType == sTypeUV3)
				{
					...
					r1 = true;
				r2 = CheckAndHandleNotification(DevRowIdx, sName, cType, cSubType, notification::type::UV, Level);
				return r1 && r2;
			}
			break;
		case pTypeCURRENT:
		...
		case pTypeWIND:
			...
			if (nsize >= nexpected) {
				...
				r1 = CheckAndHandleNotification(DevRowIdx, sName, cType, cSubType, notification::type::WIND, wspeedms);
		...
		case pTypeYouLess:
			...
			if (nsize >= nexpected) {
				...
				return CheckAndHandleNotification(DevRowIdx, sName, cType, cSubType, notification::type::USAGE, usagecurrent);
			}
			break;
		...
		case pTypeRego6XXTemp:
			return CheckAndHandleNotification(DevRowIdx, sName, cType, cSubType, notification::type::TEMPERATURE, fValue);
		case pTypePOWER:
			nexpected = 1;
			if (nsize >= nexpected) {
				fValue2 = (float)atof(strarray[0].c_str());
				return CheckAndHandleNotification(DevRowIdx, sName, cType, cSubType, notification::type::USAGE, fValue2);
			}
			break;
		case pTypeRFXMeter:
			switch(cSubType) {
				case sTypeRFXMeterCount:
					return CheckAndHandleNotification(DevRowIdx, sName, cType, cSubType, notification::type::TODAYCOUNTER, fValue);
				default:
					break;
			}
			break;
		...
		case pTypeThermostat6:
			...
			if (cSubType == sTypeThermostat6TempHumBaro)
			{
				...
				if (nsize >= nexpected)
				{
					...
					r1 = CheckAndHandleTempHumidityNotification(DevRowIdx, sName, Temp, Hum, true, true);
					r2 = CheckAndHandleDewPointNotification(DevRowIdx, sName, Temp, dewpoint);
					r3 = CheckAndHandleNotification(DevRowIdx, sName, cType, cSubType, notification::type::BARO, Baro);
					return r1 && r2 && r3;
				}
			}
	...
	const bool bHaveHumidity)
{
	...
	std::string label = notification::type::Label(notification::type::TEMPERATURE);
	std::string signtemp = notification::type::Description(notification::type::TEMPERATURE, 1);
	std::string signhum = notification::type::Description(notification::type::HUMIDITY, 1);
	...
	const float dewpoint)
{
	...
	std::string signdewpoint = notification::type::Description(notification::type::DEWPOINT, 1);
	...
	const int value)
{
	...
	std::string signvalue = notification::type::Description(notification::type::VALUE, 1);
	...
	const float Ampere3)
{
	...
	std::string signamp1 = notification::type::Description(notification::type::AMPERE1, 1);
	std::string signamp2 = notification::type::Description(notification::type::AMPERE2, 1);
	std::string signamp3 = notification::type::Description(notification::type::AMPERE3, 1);
	...
	for (const auto &n : notifications)
	{
		...
		if ((atime >= n.LastSend) || (n.SendAlways) || (!n.CustomMessage.empty())) // emergency always goes true
		{
			...
			{
				ampere = Ampere1;
				ltype = notification::type::Description(notification::type::AMPERE1, 0);
			}
			else if (ntype == signamp2)
			{
				ampere = Ampere2;
				ltype = notification::type::Description(notification::type::AMPERE2, 0);
			}
			else if (ntype == signamp3)
			{
				ampere = Ampere3;
				ltype = notification::type::Description(notification::type::AMPERE3, 0);
			}
...
bool CNotificationHelper::CheckAndHandleNotification(
	const uint64_t Idx,
	const std::string &devicename,
	const notification::type::value ntype,
	...
	const float mvalue)
{
	...
	std::string ltype = notification::type::Description(ntype, 0);
	std::string nsign = notification::type::Description(ntype, 1);
	std::string label = notification::type::Label(ntype);
	...
	for (const auto &n : notifications)
	{
		if ((atime >= n.LastSend) || (n.SendAlways)) // emergency always goes true
		{
			...
			if (atype == ltype)
			{
				bSendNotification = true;
				msg = devicename;
				if (ntype == notification::type::SWITCH_ON)
				{
...
//GB3:	Use a midday hour to avoid a clash with possible DST jump
	...
	if (ntype == notification::type::RAIN)
	{
		if (subType == sTypeRAINWU || subType == sTypeRAINByRate)
		{
			...
			CheckAndHandleNotification(Idx, devicename, devType, subType, notification::type::RAIN, (float)total_real);
		}
		else
		{
	...
	{
		//Rain Rate
		CheckAndHandleNotification(Idx, devicename, devType, subType, notification::type::RAINRATE, mvalue);
	}
	return false;
}
	...
	for (const auto &n : m_notifications)
	{
		for (const auto &n2 : n.second)
		{
			    ...
			    && (n2.LastUpdate)) // emergency always goes true
			{
				...
				if (splitresults.size() < 3)
					continue;
				std::string ttype = notification::type::Description(notification::type::LASTUPDATE, 1);
				if (splitresults[0] == ttype)
				{
					...
					if (bSendNotification && !bStartTime && (!bRecoveryMessage || n2.SendAlways))
					{
						...
						std::string ltype = notification::type::Description(notification::type::LASTUPDATE, 0);
						std::string label = notification::type::Label(notification::type::LASTUPDATE);
						char szDate[50];
						char szTmp[300];
						struct tm ltime;
