...
namespace Plugins {
	...
	int CDeviceEx_init(CDeviceEx *self, PyObject *args, PyObject *kwds)
	{
		...
		try
		{
			...
			{
				pModState->pPlugin->Log(LOG_ERROR, R"(Expected: myVar = Oikomaticz.DeviceEx(DeviceID='xxxx'))");
	...
	PyObject *CUnitEx_insert(CUnitEx *self)
	{
		...
		if (pModState->pPlugin)
		{
			...
			if (self->ID == -1)
			{
				...
				{
					pModState->pPlugin->Log(LOG_ERROR, "(%s) Unit creation failed, Oikomaticz settings prevent accepting new devices.", pModState->pPlugin->m_Name.c_str());
				}
				else
				{
					std::vector<std::vector<std::string>> result;
					...
					if (result.empty())
					{
						...
						{
							pModState->pPlugin->Log(LOG_ERROR, "(%s) Unit creation failed, Hardware/DeviceID/Unit combination (%d/%s/%d) not found in Oikomaticz.",
			...
			{
				pModState->pPlugin->Log(LOG_ERROR, "(%s) Unit creation failed, '%s' already exists in Oikomaticz with Device ID '%d'.", pModState->pPlugin->m_Name.c_str(), sName.c_str(), self->ID);
			}
		}
		else
		{
		...
		if (pModState->pPlugin)
		{
			...
			if (self->Used)
			{
				...
				else
				{
					...
					if (self->SwitchType == device::tswitch::type::Selector)
						m_notifications.CheckAndHandleSwitchNotification(DevRowIdx, sName, (IsLightSwitchOn(lstatus)) ? notification::type::SWITCH_ON : notification::type::SWITCH_OFF, llevel);
					else
						m_notifications.CheckAndHandleSwitchNotification(DevRowIdx, sName, (IsLightSwitchOn(lstatus)) ? notification::type::SWITCH_ON : notification::type::SWITCH_OFF);
				}
	...
	PyObject *CUnitEx_delete(CUnitEx *self)
	{
		...
		if (!pModState->pPlugin)
		{
			...
			{
				pModState->pPlugin->Log(LOG_ERROR, "(%s) Unit deletion failed, '%s' does not represent a device in Oikomaticz.", pModState->pPlugin->m_Name.c_str(), sName.c_str());
			}
		}
		else
		{
