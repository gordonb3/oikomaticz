...
namespace Plugins
{
	...
	{
		// Oikomaticz potentially has only two possible modules and only one can be loaded
		...
		{
			_log.Log(LOG_ERROR, "(%s) Oikomaticz and DomoticzEx modules both found in interpreter, use one or the other.", __func__);
			return nullptr;
		}
	...
	bool CPlugin::Initialise()
	{
		...
		try
		{
			...
			else
			{
				...
				// Plugins use Oikomaticz.Log() not print(), so a NullStream is sufficient.
	...
	void CPlugin::RestoreThread()
	{
		if (m_PyInterpreter)
		{
			...
			if (m_bShared)
			{
				// Shared plugins all use the main interpreter's Oikomaticz module.
				// Update pPlugin so Oikomaticz.Log() etc. route to the correct plugin.
				module_state* pModState = FindModule();
				if (pModState)
					pModState->pPlugin = this;
			}
