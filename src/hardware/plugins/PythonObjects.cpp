...
namespace Plugins {
	...
	int CImage_init(CImage *self, PyObject *args, PyObject *kwds)
	{
		...
		try
		{
			...
			{
				pModState->pPlugin->Log(LOG_ERROR, "Expected: myVar = Oikomaticz.Image(Filename=\"MyImages.zip\")");
	...
	int CDevice_init(CDevice *self, PyObject *args, PyObject *kwds)
	{
		...
		try
		{
			...
			{
				pModState->pPlugin->Log(LOG_ERROR, R"(Expected: myVar = Oikomaticz.Device(Name="myDevice", Unit=0, TypeName="", Type=0, Subtype=0, Switchtype=0, Image=0, Options={}, Used=1))");
				pModState->pPlugin->LogPythonException(__func__);
			}
		}
		catch (std::exception *e)
