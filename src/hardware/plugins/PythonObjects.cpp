...
namespace Plugins {
	...
	int CImage_init(CImage *self, PyObject *args, PyObject *kwds)
	{
		...
		try
		{
			...
			{
				pModState->pPlugin->Log(LOG_ERROR, "Expected: myVar = Oikomaticz.Image(Filename=\"MyImages.zip\"). Note: Parameter names are case-insensitive.");
	...
	PyObject* CImage_insert(CImage* self, PyObject *args)
	{
		if (self->pPlugin)
		{
			...
			{
				_log.Log(LOG_ERROR, "(%s) Image creation failed, '%s' already exists in Oikomaticz with Image ID '%d'.", self->pPlugin->m_Name.c_str(), sName.c_str(), self->ImageID);
			}
		}
		else
		{
	...
	PyObject* CImage_delete(CImage* self, PyObject *args)
	{
		if (self->pPlugin)
		{
			...
			if (self->ImageID != -1)
			{
				...
				{
					self->pPlugin->Log(LOG_ERROR, "Image deletion failed, Image %d not found in Oikomaticz.", self->ImageID);
				}
			}
			else
			{
				self->pPlugin->Log(LOG_ERROR, "Image deletion failed, '%s' does not represent a Image in Oikomaticz.", sName.c_str());
			}
		}
		else
		{
	...
	extern void maptypename(const std::string &sTypeName, int &Type, int &SubType, int &SwitchType, std::string &sValue, PyObject* OptionsIn, PyObject* OptionsOut)
	{
		...
		else if (sTypeNameLower == "push off")
		{
			Type = pTypeGeneralSwitch;
			SubType = sSwitchGeneralSwitch;
			SwitchType = device::tswitch::type::PushOff;
		}
		else if (sTypeNameLower == "contact")
		{
			Type = pTypeGeneralSwitch;
			SubType = sSwitchGeneralSwitch;
			SwitchType = device::tswitch::type::Contact;
		}
	...
	int CDevice_init(CDevice *self, PyObject *args, PyObject *kwds)
	{
		...
		try
		{
			...
			{
				pModState->pPlugin->Log(LOG_ERROR, R"(Expected: myVar = Oikomaticz.Device(Name="myDevice", Unit=0, TypeName="", Type=0, SubType=0, SwitchType=0, Image=0, Options={}, Used=1). Note: Parameter names are case-insensitive.)");
				pModState->pPlugin->LogPythonException(__func__);
			}
		}
		catch (std::exception *e)
	...
	PyObject* CDevice_insert(CDevice* self)
	{
		if (self->pPlugin)
		{
			...
			if (self->ID == -1)
			{
				...
				{
					self->pPlugin->Log(LOG_ERROR, "Device creation failed, Oikomaticz settings prevent accepting new devices.");
				}
				else
				{
					std::vector<std::vector<std::string> > result;
					...
					if (result.empty())
					{
						...
						{
							self->pPlugin->Log(LOG_ERROR, "Device creation failed, Hardware/Unit combination (%d:%d) not found in Oikomaticz.", self->HwdID, self->Unit);
						}
					}
					else
					{
						self->pPlugin->Log(LOG_ERROR, "Device creation failed, Hardware/Unit combination (%d:%d) already exists in Oikomaticz.", self->HwdID, self->Unit);
					}
				}
			}
			else
			{
				self->pPlugin->Log(LOG_ERROR, "Device creation failed, '%s' already exists in Oikomaticz with Device ID '%d'.", sName.c_str(), self->ID);
			}
		}
		else
		{
	...
	PyObject* CDevice_delete(CDevice* self)
	{
		if (self->pPlugin)
		{
			...
			if (self->ID != -1)
			{
				...
				{
					self->pPlugin->Log(LOG_ERROR, "Device deletion failed, Hardware/Unit combination (%d:%d) not found in Oikomaticz.", self->HwdID, self->Unit);
				}
			}
			else
			{
				self->pPlugin->Log(LOG_ERROR, "Device deletion failed, '%s' does not represent a device in Oikomaticz.", sName.c_str());
			}
		}
		else
		{
	...
	int CConnection_init(CConnection * self, PyObject * args, PyObject * kwds)
	{
		...
		try
		{
			...
			{
				_log.Log(LOG_ERROR,
					 R"(Expected: myVar = Oikomaticz.Connection(Name="<Name>", Transport="<Transport>", Protocol="<Protocol>", Address="<IP-Address>", Port="<Port>", Baud=0). Note: Parameter names are case-insensitive.)");
			}
		}
		catch (std::exception *e)
		{
