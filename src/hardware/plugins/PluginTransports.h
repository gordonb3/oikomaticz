...
namespace Plugins {
	...
	public:
		CPluginTransportTCP(int HwdID, PyObject* pConnection, const std::string& Address, const std::string& Port) :
			CPluginTransportIP(HwdID, pConnection, Address, Port), m_Socket(nullptr), m_Resolver(ios), m_Acceptor(nullptr) { };
	...
	{
	public:
		CPluginTransportTCPSecure(int HwdID, PyObject* pConnection, const std::string& Address, const std::string& Port) : CPluginTransportTCP(HwdID, pConnection, Address, Port), m_Context(nullptr), m_TLSSock(nullptr) { };
		virtual	void		handleAsyncConnect(const boost::system::error_code& err, boost::asio::ip::tcp::resolver::iterator endpoint_iterator);
		virtual void		handleRead(const boost::system::error_code& e, std::size_t bytes_transferred);
	...
	{
	public:
		CPluginTransportUDP(int HwdID, PyObject* pConnection, const std::string& Address, const std::string& Port) : CPluginTransportIP(HwdID, pConnection, Address, Port), m_Socket(nullptr), m_Resolver(ios) { };
		virtual	bool		handleListen();
		virtual void		handleRead(const boost::system::error_code& e, std::size_t bytes_transferred);
	...
	{
	public:
		CPluginTransportICMP(int HwdID, PyObject* pConnection, const std::string& Address, const std::string& Port) : CPluginTransportIP(HwdID, pConnection, Address, Port), m_Socket(nullptr), m_Resolver(ios), m_Timer(nullptr), m_SequenceNo(-1) { };
