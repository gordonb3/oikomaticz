...
uint8_t V6_RGBWW_SetKelvinLevel[10] = {
	0x31, 0x00, 0x00, 0x08, 0x05, 0x00, 0x00, 0x00, 0x00, l_zone
}; //(6th hex values 0x00 to 0x64 : examples: 00 = 2700K (Warm White), 19 = 3650K, 32 = 4600K, 4B, = 5550K, 64 = 6500K (Cool White))
...
// LIMITLESSLED RGBW DIRECT BRIGHTNESS SETTING is by a 3BYTE COMMAND: (First send the Group ON for the group you want to set the brightness for. You send the group ON command 100ms before sending the
// 4E 00 55) Byte1: 0x4E (decimal: 78) Byte2: 0x00 to 0xFF (full brightness 0x3B) Byte3: Always 0x55 (decimal: 85)
...
bool CLimitLess::AddSwitchIfNotExits(const unsigned char Unit, const std::string &devname)
{
	...
	if (Unit != 5)
	{
		...
		if (result.empty())
		{
			m_sql.InsertDevice(m_HwdID, "1", Unit, pTypeColorSwitch, m_LEDType, device::tswitch::type::Dimmer, 0, " ", devname);
			return false;
		}
	}
	else
	{
		result = m_sql.safe_query("SELECT ID FROM DeviceStatus WHERE (HardwareID==%d) AND (Unit==%d) AND (Type==%d) AND (SubType==%d)", m_HwdID, int(Unit), pTypeColorSwitch,
					  sTypeColor_RGB_CW_WW);
		if (result.empty())
		{
			m_sql.InsertDevice(m_HwdID, "1", Unit, pTypeColorSwitch, sTypeColor_RGB_CW_WW, device::tswitch::type::Dimmer, 0, " ", devname);
			return false;
		}
	}
	return true;
...
bool CLimitLess::StartHardware()
{
	...
	{ // get the host info
		Log(LOG_ERROR, "AppLamp: Error with IP address!...");
		return false;
	}
	...
	{
		Log(LOG_ERROR, "AppLamp: Error with IP address (SO_BROADCAST)!...");
		return false;
	}
	...
	Log(LOG_STATUS, "AppLamp: Worker Started...");
...
bool CLimitLess::SendV6Command(const uint8_t *pCmd)
{
	...
	uint8_t RBuffer[100];
	int wPointer = 0;
	memcpy(OutBuffer + wPointer, V6_PreAmble, sizeof(V6_PreAmble));
	wPointer += sizeof(V6_PreAmble);
	...
	memcpy(OutBuffer + wPointer, pCmd, 10);
	wPointer += 10;
	...
	while (totRetries < v6_repeats)
	{
		...
		if (RBuffer[0x07] != 0x00)
		{
			...
			if (GetV6BridgeID())
			{
				...
				if (RBuffer[0x07] != 0x00)
				{
					Log(LOG_ERROR, "AppLamp: Error sending command to Bridge!...");
				}
				else
					return true;
			...
			{
				Log(LOG_ERROR, "AppLamp: Invalid command send to Bridge!...");
			}
			return false;
		}
		return true;
...
void CLimitLess::Do_Work()
{
	...
	while (!IsStopRequested(1000))
	{
		...
		if (bDoInit)
		{
			...
			if (m_BridgeType == LBTYPE_V6)
			{
				if (!GetV6BridgeID())
				{
					Log(LOG_ERROR, "AppLamp: Bridge not found, check IP Address/Port!...");
					Log(LOG_ERROR, "AppLamp: Worker stopped!...");
					return;
				}
				Log(LOG_STATUS, "AppLamp: Bridge found!...");
			}
		}
	...
	Log(LOG_STATUS, "AppLamp: Worker stopped...");
}
	...
	if (m_LEDType == sTypeColor_RGB_W)
	{
		switch (pLed->command)
		{
			...
			case Color_SetColor: {
				...
				{
					Log(LOG_STATUS, "AppLamp: SetRGBColour - Color mode %d is unhandled, if you have a suggestion for what it should do, please post on the Domoticz forum",
						 pLed->color.mode);
				}
			}
			break;
