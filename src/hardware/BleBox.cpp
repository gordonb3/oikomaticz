...
void BleBox::AddNode(const std::string& name, const std::string& IPAddress, bool reloadNodes)
{
	...
	if (deviceType.bleBoxTypeId == 4) // gatebox
	{
		...
		idx = (int)m_sql.InsertDevice(m_HwdID, 0, id, 2, pTypeGeneralSwitch, sTypeAC, device::tswitch::type::PushOn, 0, "Unavailable", deviceName, signalQuality, 255, 0);
		m_sql.safe_query("UPDATE DeviceStatus SET StrParam1='%d', StrParam2='%q' WHERE (ID==%d)", deviceTypeID, IPAddress.c_str(), idx);

		idx = (int)m_sql.InsertDevice(m_HwdID, 0, id, 3, pTypeGeneralSwitch, sTypeAC, device::tswitch::type::PushOn, 0, "Unavailable", deviceName, signalQuality, 255, 0);
		m_sql.safe_query("UPDATE DeviceStatus SET StrParam1='%d', StrParam2='%q' WHERE (ID==%d)", deviceTypeID, IPAddress.c_str(), idx);
	}
