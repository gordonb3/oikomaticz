...
void CEcoCompteur::Do_Work()
{
	...
	Log(LOG_STATUS, "EcoCompteur: Worker started...");
	...
	Log(LOG_STATUS, "EcoCompteur: Worker stopped...");
}
...
void CEcoCompteur::GetScript()
{
	...
	{
		Log(LOG_ERROR, "EcoCompteur: Error getting 'inst.json' from url : " + m_url);
		return;
	}
	// Parse inst.json
	Json::Value root;
	...
	{
		Log(LOG_ERROR, "EcoCompteur: Error parsing JSON from url : " + m_url);
		return;
	}
	...
	{
		Log(LOG_ERROR, "EcoCompteur: Error getting 'log2.csv' from url : " + m_url);
		return;
	}
	...
	if (sLog2.length() == 0)
	{
		Log(LOG_ERROR, "EcoCompteur: log2.csv looks empty");
		return;
	}
