...
void MySensorsBase::UpdateNode(const int nodeID, const std::string &name)
{
	...
	{
		Log(LOG_ERROR, "MySensors: Update command received for unknown node_id: %d", nodeID);
	}
}
...
void MySensorsBase::UpdateChild(const int nodeID, const int childID, const bool UseAck, const int AckTimeout)
{
	...
	{
		Log(LOG_ERROR, "MySensors: Update command received for unknown node_id: %d, child_id: %d", nodeID, childID);
	}
}
...
bool MySensorsBase::SendNodeSetCommandImpl(const int NodeID, const int ChildID, const _eMessageType messageType, const _eSetType SubType, const std::string &Payload, const bool bUseAck, const int AckTimeout)
{
	...
	{
		if (repeat != 0)
			Log(LOG_ERROR, "MySensors: Repeating previous command (%d/%d)", repeat + 1, repeats);
	...
	if (m_bAckReceived)
		return true;
	Log(LOG_ERROR, "MySensors: Command not received by Node !! (node_id: %d, child_id: %d)", NodeID, ChildID);
...
bool MySensorsBase::WriteToHardware(const char *pdata, const unsigned char /*length*/)
{
	...
	if (packettype == pTypeLighting2)
	{
		...
		if (_tMySensorNode *pNode = FindNode(node_id))
		{
			...
			if (!pChild)
			{
				Log(LOG_ERROR, "MySensors: Light command received for unknown node_id: %d, child_id: %d", node_id, child_sensor_id);
				return false;
			}
			...
			Log(LOG_ERROR, "MySensors: Light command received for unknown node_id: %d", node_id);
			return false;
		}
	}
	else if (packettype == pTypeGeneralSwitch)
	{
		...
		else
		{
			...
			if (_tMySensorNode *pNode = FindNode(node_id))
			{
				...
				Log(LOG_ERROR, "MySensors: Blinds/Window command received for unknown node_id: %d", node_id);
				return false;
			}
		}
	}
		...
		if (_tMySensorNode *pNode = FindNode(node_id))
		{
			...
			if (pLed->command == Color_SetColor)
			{
				...
				if (pLed->color.mode == ColorModeWhite)
				{
					...
					Log(LOG_STATUS, "MySensors: SetRGBColour - Color mode %d is unhandled, if you have a suggestion for what it should do, please post on the Domoticz forum", pLed->color.mode);
					return false;
				}
	...
	{
		Log(LOG_ERROR, "MySensors: Unknown action received");
		return false;
	}
	return true;
}
...
void MySensorsBase::ParseLine(const std::string &sLine)
{
...
#ifdef _DEBUG
	Log(LOG_NORM, "MySensors: NodeID: %d, ChildID: %d, MessageType: %d, Ack: %d, SubType: %d, Payload: %s", node_id, child_sensor_id, message_type, ack, sub_type, payload.c_str());
#endif
	...
	if (message_type == MT_Internal)
	{
		...
		case I_VERSION:
		{
			if (node_id == 0)
			{
				...
				Log(LOG_NORM, "MySensors: Gateway Version: %s", payload.c_str());
			}
			else {
				Log(LOG_NORM, "MySensors: VERSION from NodeID: %d, ChildID: %d, Payload: %s", node_id, child_sensor_id, payload.c_str());
			}
		}
		break;
		case I_ID_REQUEST:
		...
		case I_CONFIG:
			...
			break;
		case I_SKETCH_NAME:
			Log(LOG_STATUS, "MySensors: Node: %d, Sketch Name: %s", node_id, payload.c_str());
			if (_tMySensorNode *pNode = FindNode(node_id))
			{
		...
		case I_SKETCH_VERSION:
			Log(LOG_STATUS, "MySensors: Node: %d, Sketch Version: %s", node_id, payload.c_str());
			if (_tMySensorNode *pNode = FindNode(node_id))
			{
		...
		case I_BATTERY_LEVEL:
			...
			break;
		case I_LOG_MESSAGE:
			//Log(LOG_NORM, "MySensors: 'Log': %s", payload.c_str());
			break;
		case I_GATEWAY_READY:
			Log(LOG_NORM, "MySensors: Gateway Ready...");
		...
		case I_INCLUSION_MODE:
			Log(LOG_NORM, "MySensors: Inclusion mode=%s", payload.c_str());
		...
		case I_PRE_SLEEP_NOTIFICATION:
			...
			{
				std::lock_guard<std::mutex> l(m_node_sleep_mutex);
				auto itt = m_node_sleep_queue.find(node_id);
	...
	else if (message_type == MT_Set)
	{
		...
		default:
			...
			{
				Log(LOG_ERROR, "MySensors: Unknown/Invalid sensor type (%d)", sub_type);
			}
			else
			{
				Log(LOG_ERROR, "MySensors: Unhandled sensor (sub-type=%d), please report with log!", sub_type);
			}
			break;
		}
		if (bHaveValue)
...
void MySensorsBase::SendTextSensorValue(const int nodeID, const int childID, const std::string &tvalue)
{
	if (_tMySensorNode *pNode = FindNode(nodeID))
	{
		...
		{
			Log(LOG_ERROR, "MySensors: Text update received for unknown node_id: %d, child_id: %d", nodeID, childID);
		}
	}
	else
	{
...
#ifdef _DEBUG
		Log(LOG_STATUS, "MySensors: going to send: %s", toSend.c_str());
#endif
		WriteInt(toSend);
	}
}
	...
	namespace server {
		...
		void CWebServer::Cmd_MySensorsUpdateNode(WebEmSession & session, const request& req, Json::Value &root)
		{
			...
			if (pBaseHardware == nullptr)
				return;
			if (
				(pBaseHardware->HwdType != hardware::type::MySensorsUSB) &&
				(pBaseHardware->HwdType != hardware::type::MySensorsTCP) &&
				(pBaseHardware->HwdType != hardware::type::MySensorsMQTT)
				)
				return;
