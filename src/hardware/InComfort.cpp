...
{
	int sec_counter = 0;
	Log(LOG_STATUS, "InComfort: Worker started...");
	...
	Log(LOG_STATUS, "InComfort: Worker stopped...");
}
...
{
	Log(LOG_NORM, "InComfort: Setpoint of sensor with idx idx changed to temp");
...
std::string CInComfort::GetHTTPData(const std::string &sURL)
{
	...
	{
		Log(LOG_ERROR, "InComfort: Error getting current state!");
	}
	return sResult;
}
...
void CInComfort::ParseAndUpdateDevices(const std::string &jsonData)
{
	...
	{
		Log(LOG_ERROR, "InComfort: Invalid data received. Data is not json formatted.");
		return;
	}
	if (root["nodenr"].empty() == true)
	{
		Log(LOG_ERROR, "InComfort: Invalid data received. Nodenr not found.");
		return;
	}
