...
class I2C : public CDomoticzHardwareBase
{
	...
	bool readBME280ID(int fd, int &ChipID, int &Version);
	bool readBME280All(int fd, float &temp, float &pressure, int &humidity);
	...
	int WriteCmdAddr(int fd, uint8_t CmdAddr, uint8_t devAction);
	...
	// BMP085 stuff
	// Forecast
	uint8_t bmp_CalculateForecast(float pressure);
