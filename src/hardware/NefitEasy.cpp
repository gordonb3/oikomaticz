...
void CNefitEasy::Do_Work()
{
	...
	Log(LOG_STATUS, "NefitEasy: Worker started...");
	...
	while (!IsStopRequested(1000))
	{
		...
		if ((sec_counter % fast_pollint == 0) || (bFirstTime))
		{
			...
			{
				Log(LOG_ERROR, "NefitEasy: Error getting/processing status result...");
			}
		}
		if ((sec_counter % slow_pollint == 0) || (bFirstTime))
		{
			...
			{
				Log(LOG_ERROR, "NefitEasy: Error getting/processing pressure result...");
			}
		}
		if (sec_counter % NEFIT_GAS_INTERVAL == 0)
		{
			...
			{
				Log(LOG_ERROR, "NefitEasy: Error getting/processing gas result...");
			}
		}
		bFirstTime = false;
	}
	Log(LOG_STATUS, "NefitEasy: Worker stopped...");
}
...
void CNefitEasy::SetUserMode(bool bSetUserModeClock)
{
	...
	try
	{
		...
		{
			Log(LOG_ERROR, "NefitEasy: Error setting User Mode!");
			return;
		}
		GetStatusDetails();
	}
...
bool CNefitEasy::GetStatusDetails()
{
	...
	try
	{
		...
		{
			Log(LOG_ERROR, "NefitEasy: Error getting http data!");
			return false;
		}
	}
	catch (...)
	...
	if ((!ret) || (!root.isObject()))
	{
		if (sResult.find("Error: REQUEST_TIMEOUT") != std::string::npos)
			Log(LOG_ERROR, "NefitEasy: Request Timeout !");
		else
			Log(LOG_ERROR, "NefitEasy: Invalid data received (main)!");
		return false;
	}
	if(!CheckId(NEFITEASY_STATUS_URL, root))
	{
		Log(LOG_ERROR, "NefitEasy: Invalid response received (main)");
		return false;
	}
...
bool CNefitEasy::GetOutdoorTemp()
{
	...
	if ((!ret) || (!root.isObject()))
	{
		Log(LOG_ERROR, "NefitEasy: Invalid data received! (ODT)");
		return false;
	}
	if(!CheckId(NEFITEASY_OUTDOORTEMP_URL, root))
	{
		Log(LOG_ERROR, "NefitEasy: Invalid response received! (ODT)");
		return false;
	}
...
bool CNefitEasy::GetFlowTemp()
{
	...
	if ((!ret) || (!root.isObject()))
	{
		Log(LOG_ERROR, "NefitEasy: Invalid data received! (FT)");
		return false;
	}
	if(!CheckId(NEFITEASY_FLOWTEMP_URL, root))
	{
		Log(LOG_ERROR, "NefitEasy: Invalid response received (FT)");
		return false;
	}
	if (root["value"].empty() == true)
	{
...
bool CNefitEasy::GetPressure()
{
	...
	if ((!ret) || (!root.isObject()))
	{
		Log(LOG_ERROR, "NefitEasy: Invalid data received! (Press)");
		return false;
	}
	if(!CheckId(NEFITEASY_PRESSURE_URL, root))
	{
		Log(LOG_ERROR, "NefitEasy: Invalid response received! (Press)");
		return false;
	}
	if (root["value"].empty())
	{
...
bool CNefitEasy::GetDisplayCode()
{
	...
	if ((!ret) || (!root.isObject()))
	{
		Log(LOG_ERROR, "NefitEasy: Invalid data received! (DP)");
		return false;
	}
	if(!CheckId(NEFITEASY_DISPLAYCODE_URL, root))
	{
		Log(LOG_ERROR, "NefitEasy: Invalid response received! (DP)");
		return false;
	}
	if (root["value"].empty() == true)
	{
...
bool CNefitEasy::GetGasUsage()
{
	...
	if ((!ret) || (!root.isObject()))
	{
		Log(LOG_ERROR, "NefitEasy: Invalid data received! (Gas)");
		return false;
	}
	if(!CheckId(NEFITEASY_GAS_URL, root))
	{
		Log(LOG_ERROR, "NefitEasy: Invalid response received! (Gas)");
		return false;
	}
	if (root["value"].empty())
	{
...
void CNefitEasy::SetSetpoint(const int /*idx*/, const float temp)
{
	...
	try
	{
		...
		if (!HTTPClient::POST(szURL.str(), root.toStyledString(), ExtraHeaders, sResult))
		{
			Log(LOG_ERROR, "NefitEasy: Error setting setpoint!");
			return;
		}
		szURL.clear();
		szURL.str("");
