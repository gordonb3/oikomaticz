...
void CPhilipsHue::InsertUpdateLamp(const int NodeID, const _eHueLightType LType, const _tHueLightState tstate, const std::string& Name, const std::string& Options, const std::string& modelid, const bool AddMissingDevice)
{
	if (LType == HLTYPE_RGB_W || LType == HLTYPE_CW_WW || LType == HLTYPE_RGB_CW_WW)
	{
		...
		if (result.empty())
		{
			//Set SwitchType to device::tswitch::type::Dimmer
			m_sql.safe_query("UPDATE DeviceStatus SET SwitchType=%d WHERE (HardwareID == %d) AND (DeviceID == '%q')",
				int(device::tswitch::type::Dimmer), m_HwdID, szID);
		}
	}
	else if (LType == HLTYPE_SCENE)
	{
		...
		if (result.empty())
		{
			//Set SwitchType to device::tswitch::type::PushOn
			m_sql.safe_query("UPDATE DeviceStatus SET SwitchType=%d WHERE (HardwareID == %d) AND (DeviceID == '%q')",
				int(device::tswitch::type::Dimmer), m_HwdID, szID);
		}
	}
	else
	{
		...
		if (result.empty())
		{
			//Set SwitchType
			m_sql.safe_query("UPDATE DeviceStatus SET SwitchType=%d WHERE (HardwareID == %d) AND (DeviceID == '%q')",
				int(LType == HLTYPE_DIM ? device::tswitch::type::Dimmer : device::tswitch::type::OnOff), m_HwdID, szID);
		}
	}
}
...
{
	// call v2 sensors UpdateAll() and map to Oikomaticz devices (change-aware)
	...
	try
	{
		if (m_v2sensors->UpdateAll())
		{
			...
			// Map contact resources to Oikomaticz switches (one device per owner/device), but only when changed
			for (const auto& c : m_v2sensors->GetContacts())
			{
				...
				// Compute Oikomaticz node for device (owner-based)
				...
				// Update/create Oikomaticz device and record state
				InsertUpdateSwitch(domoticzNodeID, 1, device::tswitch::type::DoorContact, isOpen, friendlyName, 0);
			...
			for (const auto& t : m_v2sensors->GetTampers())
			{
				...
				InsertUpdateSwitch(domoticzNodeID, 1, device::tswitch::type::OnOff, isTampered, friendlyName, 0);
			...
			for (const auto& p : m_v2sensors->GetDevicePowers())
			{
				...
				// Determine the Oikomaticz node IDs (owner-based)
				...
				if (!result.empty())
				{
					...
					if (prevBattery != batteryLevel)
					{
						...
						InsertUpdateSwitch(domoticzNodeID_contact, 1, device::tswitch::type::DoorContact, contactIsOn, contactName, (uint8_t)batteryLevel);
				...
				if (!result.empty())
				{
					...
					if (prevBattery != batteryLevel)
					{
						...
						InsertUpdateSwitch(domoticzNodeID_tamper, 1, device::tswitch::type::OnOff, tamperIsOn, tamperName, (uint8_t)batteryLevel);
						m_sql.safe_query("UPDATE DeviceStatus SET BatteryLevel=%d, LastUpdate=datetime('now','localtime') WHERE ID=%d", batteryLevel, deviceRowId);
						m_v2_battery_level[ownerRid] = batteryLevel;
					}
				}
	...
	for (auto iSensor = root["sensors"].begin(); iSensor != root["sensors"].end(); ++iSensor)
	{
		...
		else if (current_sensor.m_type == SensorTypeZLLPresence)
		{
			if ((previous_sensor.m_state.m_presence != current_sensor.m_state.m_presence)
				|| (bNewSensor))
			{
				InsertUpdateSwitch(sID, unitcode, device::tswitch::type::Motion, current_sensor.m_state.m_presence, device_name, current_sensor.m_config.m_battery);
			}
		}
		...
		else if (current_sensor.m_type == SensorTypeZLLLightLevel)
		{
			if ((previous_sensor.m_state.m_dark != current_sensor.m_state.m_dark)
				|| (bNewSensor))
			{
				InsertUpdateSwitch(sID, unitcode, device::tswitch::type::Dusk, current_sensor.m_state.m_dark, device_name, current_sensor.m_config.m_battery);
			}
...
bool CPhilipsHue::InsertUpdateSelectorSwitch(const int NodeID, const uint8_t Unitcode, const uint8_t selectorLevel, const std::string& Name, const uint8_t BatteryLevel)
{
	...
	if (result.empty())
	{
		...
		m_sql.safe_query("UPDATE DeviceStatus SET Name='%q', SwitchType=%d, CustomImage=%i WHERE (HardwareID == %d) AND (DeviceID == '%08X') AND (Unit == '%d')", Name.c_str(), (device::tswitch::type::Selector), 0, m_HwdID, NodeID, xcmd.unitcode);
		return true;
	}
	return false;
}

void CPhilipsHue::InsertUpdateSwitch(const int NodeID, const uint8_t Unitcode, const device::tswitch::type::value SType, const bool bIsOn, const std::string& Name, const uint8_t BatteryLevel)
{
	_tGeneralSwitch xcmd;
	xcmd.len = sizeof(_tGeneralSwitch) - 1;
	xcmd.id = NodeID;
