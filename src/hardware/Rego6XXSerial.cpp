...
void CRego6XXSerial::Do_Work()
{
	...
	while (!IsStopRequested(1000))
	{
		...
		if (!isOpen())
		{
			if (m_retrycntr==0)
			{
				Log(LOG_STATUS,"Rego6XX: serial retrying in %d seconds...", Rego6XX_RETRY_DELAY);
			}
			m_retrycntr++;
		...
		else if(m_errorcntr > Rego6XX_MAX_ERRORS_UNITL_RESTART)
		{
			...
			Log(LOG_ERROR, "Rego6XX: Reopening serial port");
			sleep_seconds(2);
				...
				if (g_allRegisters[m_pollcntr].type != REGO_TYPE_NONE)
				{
					...
					switch (m_regoType)
					{
						...
						case 2:
							...
							break;
						default:
							Log(LOG_ERROR, "Rego6XX: Unknown type!");
							break;
					}
					cmd.data.value[0] = 0;
					cmd.data.value[1] = 0;
	...
	Log(LOG_STATUS,"Rego6XX: Worker stopped...");
}
...
bool CRego6XXSerial::OpenSerialDevice()
{
	...
	try
	{
		...
		Log(LOG_STATUS,"Rego6XX: Using serial port: %s", m_szSerialPort.c_str());
	}
	catch (boost::exception & e)
	{
		Log(LOG_ERROR,"Rego6XX: Error opening serial port!");
#ifdef _DEBUG
