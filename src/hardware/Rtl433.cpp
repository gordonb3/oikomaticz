	...
	m_cmdline(cmdline)
{
	...
	/*
			...
			if (!ParseJsonLine(line))
			{
				...
				Log(LOG_STATUS, "Rtl433: Unhandled sensor reading, please report: (%s)", line.c_str());
			}
		#endif
	*/
}
...
bool CRtl433::ParseData(std::map<std::string, std::string>& data)
{
	...
	if (FindField(data, "snr"))
	{
		...
		*/
		...
		if (snr > 11) snr = 11; // Oikomaticz RSSI field can only be 0-11, 12 is used for non-RF received devices
	...
	{
		//can remove this comment : Log(LOG_STATUS, "Rtl433: : CM180 haveSequence(%d) sensoridx(%d) havePower(%d) haveEnergy(%d))", haveSequence, sensoridx, havePower, haveEnergy);
		sensoridx = sensoridx + 1;
		//can rmeove this comment : Log(LOG_STATUS, "Rtl433: : CM180 sensoridx(%d) unit(%d) batterylevel(%d) power(%f) energy(%f) model(%s)", sensoridx, unit, batterylevel, power, energy, model.c_str());
...
void CRtl433::Do_Work()
{
	...
	if (!m_cmdline.empty())
		Log(LOG_STATUS, "Rtl433: Worker started... (Extra Arguments: %s)", m_cmdline.c_str());
	else
		Log(LOG_STATUS, "Rtl433: Worker started...");
...
#ifdef WIN32
				Log(LOG_STATUS, "Rtl433: rtl_433 startup failed. Make sure it's properly installed. (%s)  https://cognito.me.uk/computers/rtl_433-windows-binary-32-bit)", szCommand.c_str());
#else
				Log(LOG_STATUS, "Rtl433: rtl_433 startup failed. Make sure it's properly installed (%s). https://github.com/merbanan/rtl_433", szCommand.c_str());
#endif
				for (int i = 0; i < 30; i++)
				{
...
#define RTL433_USE_fgets
		...
		while (!IsStopRequested(100))
		{
			...
			if (fgetline(_hPipe, (char*)&line, sizeof(line)) != nullptr)
			{
				...
				if (sLine != szLastLine)
				{
					...
					{
						// this is also logged when parsed data is invalid
						Log(LOG_STATUS, "Rtl433: Unhandled sensor reading, please report: (%s)", sLine.c_str());
					}
				}
			}
#else
	...
	Log(LOG_STATUS, "Rtl433: Worker stopped...");
}
