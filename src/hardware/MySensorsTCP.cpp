...
{
	Log(LOG_STATUS, "MySensors: connected to: %s:%d", m_szIPAddress.c_str(), m_usIPPort);
	m_bIsStarted = true;
	m_LineReceived.clear();
...
{
	Log(LOG_STATUS, "MySensors: disconnected");
}

void MySensorsTCP::Do_Work()
{
	int sec_counter = 0;
	Log(LOG_STATUS, "MySensors: trying to connect to: %s:%d", m_szIPAddress.c_str(), m_usIPPort);
	...
	Log(LOG_STATUS, "MySensors: TCP/IP Worker stopped...");
}
...
void MySensorsTCP::OnError(const boost::system::error_code& error)
{
	...
	{
		Log(LOG_ERROR, "MySensors: Can not connect to: %s:%d", m_szIPAddress.c_str(), m_usIPPort);
	}
	else if (
	...
	{
		Log(LOG_STATUS, "MySensors: Connection reset!");
	}
	else
		Log(LOG_ERROR, "MySensors: %s", error.message().c_str());
}
