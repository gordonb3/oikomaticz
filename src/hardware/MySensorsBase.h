...
class MySensorsBase : public CDomoticzHardwareBase
{
	...
	enum _eSetType
	{
		...
		V_UNIT_PREFIX = 43, // Allows sensors to send in a string representing the unit prefix to be displayed in GUI.This is not parsed by controller!E.g.cm, m, km, inch.S_DISTANCE, S_DUST,
				    // S_AIR_QUALITY
	...
	struct _tMySensorSmartSleepQueueItem
	{
		...
		bool _bUseAck;
		int _AckTimeout;
		_tMySensorSmartSleepQueueItem(const int NodeID, const int ChildID, const _eMessageType messageType, const _eSetType SubType, const std::string &Payload, const bool bUseAck,
					      const int AckTimeout)
			: _Payload(Payload)
		{
			_NodeID = NodeID;
			_ChildID = ChildID;
	...
	_tMySensorNode *FindNode(uint8_t nodeID);
	void UpdateNode(int nodeID, const std::string &name);
	void RemoveNode(int nodeID);
	void RemoveChild(int nodeID, int childID);
	void UpdateChild(int nodeID, int childID, bool UseAck, int AckTimeout);
	static std::string GetMySensorsValueTypeStr(enum _eSetType vType);
	static std::string GetMySensorsPresentationTypeStr(enum _ePresentationType pType);
	std::string GetGatewayVersion();
	void SendTextSensorValue(int nodeID, int childID, const std::string &tvalue);
	...
	void UpdateChildDBInfo(int NodeID, int ChildID, _ePresentationType pType, const std::string &Name);
	bool GetChildDBInfo(int NodeID, int ChildID, _ePresentationType &pType, std::string &Name, bool &UseAck);

	void SendCommandInt(int NodeID, int ChildID, _eMessageType messageType, bool UseAck, int SubType, const std::string &Payload);
	bool SendNodeSetCommand(int NodeID, int ChildID, _eMessageType messageType, _eSetType SubType, const std::string &Payload, bool bUseAck, int AckTimeout);
	bool SendNodeSetCommandImpl(int NodeID, int ChildID, _eMessageType messageType, _eSetType SubType, const std::string &Payload, bool bUseAck, int AckTimeout);
	void SendNodeCommand(int NodeID, int ChildID, _eMessageType messageType, int SubType, const std::string &Payload);

	void UpdateSwitch(_eSetType vType, unsigned char Idx, int SubUnit, bool bOn, double Level, const std::string &defaultname, int BatLevel);
	...
	bool GetBlindsValue(int NodeID, int ChildID, int &blind_value);

	void LoadDevicesFromDatabase();
	void Add2Database(int nodeID, const std::string &SketchName, const std::string &SketchVersion);
	void DatabaseUpdateSketchName(int nodeID, const std::string &SketchName);
	void DatabaseUpdateSketchVersion(int nodeID, const std::string &SketchVersion);

	void SendSensor2Domoticz(_tMySensorNode *pNode, _tMySensorChild *pSensor, _eSetType vType);

	void MakeAndSendWindSensor(int nodeID, const std::string &sname);
	...
	void UpdateVar(int NodeID, int ChildID, int VarID, const std::string &svalue);
	bool GetVar(int NodeID, int ChildID, int VarID, std::string &sValue);
