...
void KMTronicTCP::Do_Work()
{
	...
	Log(LOG_STATUS, "KMTronic: TCP/IP Worker started...");
	...
	Log(LOG_STATUS, "KMTronic: TCP/IP Worker stopped...");
}
...
void KMTronicTCP::GetMeterDetails()
{
	...
	if (!HTTPClient::GET(szURL, sResult))
	{
		...
		if (sResult.empty())
		{
			Log(LOG_ERROR, "KMTronic: Error connecting to: %s", m_szIPAddress.c_str());
			return;
		}
	}
#else
