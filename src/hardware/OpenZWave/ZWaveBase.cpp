...
void ZWaveBase::SendSwitchIfNotExists(const _tZWaveDevice* pDevice)
{
	...
	int SwitchType = (
		...
		|| (pDevice->devType == ZDTYPE_SWITCH_RGBW)
		|| (pDevice->devType == ZDTYPE_SWITCH_COLOR)
		) ? device::tswitch::type::Dimmer : device::tswitch::type::OnOff;
	...
	if (!result.empty())
	{
		...
		if (itt != m_devices.end())
		{
			Log(
				LOG_STATUS,
				"SendSwitchIfNotExists: Device '%s' (%s) with DeviceID '%s' matches '%s' (%s). Oikomaticz will use the Dimmer (and hide the Switch).",
...
void ZWaveBase::SendDevice2Domoticz(_tZWaveDevice* pDevice)
{
	...
	else if (pDevice->devType == ZDTYPE_SENSOR_POWER)
	{
		...
		(pDevice->devType == ZDTYPE_SENSOR_POWERENERGYMETER)
		|| (pDevice->devType == ZDTYPE_SENSOR_KVAH)
		|| (pDevice->devType == ZDTYPE_SENSOR_KVAR)
		|| (pDevice->devType == ZDTYPE_SENSOR_KVARH)
		)
	{
		// check for possible counter issues
