...
void CAirconWithMe::Do_Work()
{
	...
	Log(LOG_STATUS, "AirconWithMe: Worker started...");
	GetValues();
	...
	Log(LOG_STATUS, "AirconWithMe: Worker stopped...");
}
...
void CAirconWithMe::UpdateSelectorSwitch(const int32_t uid, const int32_t value, const UIDinfo& valueInfo)
{
	...
	if (result.empty())
	{
		m_sql.safe_query("UPDATE DeviceStatus SET Name='%q', SwitchType=%d, CustomImage=%i WHERE(HardwareID == %d) AND (DeviceID == '%08X') AND (Unit == '%d')", valueInfo.mDefaultName.c_str(), (device::tswitch::type::Selector), 0, m_HwdID, xcmd.id, xcmd.unitcode);
