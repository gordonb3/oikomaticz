...
{
	Log(LOG_STATUS,"Ec3kMeter: connected to: %s:%d", m_szIPAddress.c_str(), m_usIPPort);
	m_bIsStarted=true;
...
{
	Log(LOG_STATUS,"Ec3kMeter: disconnected");
}
...
void Ec3kMeterTCP::Do_Work()
{
	...
	Log(LOG_STATUS,"Ec3kMeter: TCP/IP Worker stopped...");
}
...
void Ec3kMeterTCP::OnError(const boost::system::error_code& error)
{
	...
	{
		Log(LOG_ERROR, "Ec3kMeter: Can not connect to: %s:%d", m_szIPAddress.c_str(), m_usIPPort);
	}
	else if (
	...
	{
		Log(LOG_STATUS, "Ec3kMeter: Connection reset!");
	}
	else
		Log(LOG_ERROR, "Ec3kMeter: %s", error.message().c_str());
}
...
void Ec3kMeterTCP::ParseData(const unsigned char *pData, int Len)
{
	...
	{
		Log(LOG_ERROR, "Ec3kMeter: invalid data received!");
		return;
	}
	if (root[SENSOR_ID].empty() == true)
	{
		Log(LOG_ERROR, "Ec3kMeter: id not found in telegram");
		return;
	}
	if (root[DATA].empty() == true)
	{
		Log(LOG_ERROR, "Ec3kMeter: data not found in telegram");
		return;
	}
	...
	{
		Log(LOG_ERROR, "Ec3kMeter: energy (ws) not found in telegram");
		return;
	}

	if (data[W_CURRENT].empty() == true)
	{
		Log(LOG_ERROR, "Ec3kMeter: current consumption not found in telegram");
		return;
	}
	if (data[W_MAX].empty() == true)
	{
		Log(LOG_ERROR, "Ec3kMeter: maximum consumption not found in telegram");
		return;
	}
	if (data[TIME_ON].empty() == true)
	{
		Log(LOG_ERROR, "Ec3kMeter: time on not found in telegram");
		return;
	}
	if (data[TIME_TOTAL].empty() == true)
	{
		Log(LOG_ERROR, "Ec3kMeter: total time not found in telegram");
		return;
	}
	if (data[RESET_COUNT].empty() == true)
	{
		Log(LOG_ERROR, "Ec3kMeter: reset count not found in telegram");
		return;
	}
