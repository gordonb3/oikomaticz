...
void CHEOS::UpdateNodeStatus(const std::string &DevID, const device::tmedia::status::value nStatus, const std::string &sStatus)
{
	...
	m_sql.safe_query("UPDATE DeviceStatus SET nValue=%d, sValue='%q', LastUpdate='%q' WHERE (HardwareID == %d) AND (DeviceID == '%q') AND (Unit == 1) AND (SwitchType == %d)",
		int(nStatus), sStatus.c_str(), sLastUpdate.c_str(), m_HwdID, DevID.c_str(), device::tswitch::type::Media);
}
...
void CHEOS::UpdateNodesStatus(const std::string &DevID, const std::string &sStatus)
{
	...
	m_sql.safe_query("UPDATE DeviceStatus SET sValue='%q', LastUpdate='%q' WHERE (HardwareID == %d) AND (DeviceID == '%q') AND (Unit == 1) AND (SwitchType == %d)",
		sStatus.c_str(), sLastUpdate.c_str(), m_HwdID, DevID.c_str(), device::tswitch::type::Media);
}
...
void CHEOS::AddNode(const std::string &Name, const std::string &PlayerID)
{
	...
	m_sql.InsertDevice(m_HwdID, 0, PlayerID.c_str(), 1, pTypeLighting2, sTypeAC, device::tswitch::type::Media, 0, "Unavailable", Name, 12, 255, 1);
			...
			if (result.size() == 1)
			{
				...
				if (sType == device::tswitch::type::Media)
				{
					...
					case hardware::type::HEOS:
						CDomoticzHardwareBase *pBaseHardware = m_mainworker.GetHardwareByIDType(result[0][3], hardware::type::HEOS);
						if (pBaseHardware == nullptr)
							return;
