...
class CDomoticzHardwareBase : public StoppableTask
{
	...
	virtual void GetManualSwitchParameters(const std::multimap<std::string, std::string> &Parameters, device::tswitch::type::value &SwitchTypeInOut, int &LightTypeInOut,
	...
	void CreateBlindSwitch(int NodeID, uint8_t ChildID, device::tswitch::type::value switchtype, bool bDeviceUsed, bool bReversePosition, bool bReverseState, uint8_t cmnd, uint8_t level, const std::string &defaultName, const std::string &userName, int32_t batteryLevel, uint8_t rssiLevel = 12);
