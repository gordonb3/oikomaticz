				...
				if (connected)
				{
					...
					if ((type == "NATherm1") || (type == "NRV"))
					{
						...
						if (type == "NATherm1")
						{
							...
							// Set option SwitchType to device::tswitch::type::Contact
							...
							if (!result.empty())
								...
								if (m_bFirstTimeHomeStatus)
								{
                                	                                //m_sql.UpdateDeviceValue("SwitchType", device::tswitch::type::Dusk, std::to_string(uId));  //12
									m_sql.UpdateDeviceValue("SwitchType", device::tswitch::type::Contact, std::to_string(uId)); // 2
					...
					if (type == "NLF" || type == "NLM" || type == "NLFN" || type == "NLIS" || type == "NLL" || type == "NLFE" || type == "NLD" || type == "Z3L" || type == "BNLD" || type == "BNIL" || type == "BN3L")
					{
						...
						// Set option SwitchType to device::tswitch::type::Dimmer only if we have "brightness"
						if (!module["brightness"].empty())
						{
							...
							if (!result.empty())
								...
								if (m_bFirstTimeHomeStatus)
								{
                                                                	m_sql.UpdateDeviceValue("SwitchType", device::tswitch::type::Dimmer, std::to_string(uId)); // 7
					...
					if ((type == "NLV") || (type == "BNAS") || (type == "NLLV") || (type == "NLIV") || (type == "Z3V"))
					{
						...
						CreateBlindSwitch(crcId, ChildID, device::tswitch::type::BlindsPercentage, bDeviceUsed, bReversePosition, bReverseState, Command, level, moduleName.c_str(), m_Name.c_str(), batteryLevel, mrf_status);
