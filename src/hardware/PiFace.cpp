...
int CPiFace::LoadConfig()
{
    ...
    if (!ConfigFile.is_open())
    {
        ...
        ConfigFile.close();
        Log(LOG_ERROR,"PiFace: Error config file: %s not found", configfile.c_str() );
        Log(LOG_ERROR,"PiFace: Auto creating for board 0");
        LoadDefaultConfig();
    ...
    if (ConfigFile.is_open())
    {
	    ...
	    if (StartPos > 0)
	    {
		    ...
		    {
			    Log(LOG_STATUS, "PiFace: config file: Valid address: %d , Pin: %d and Port %c Parameter: %s , Value %s", Address, PinNumber, PortType, Parametername.c_str(),
			    ...
			    {
				    default:
					    Log(LOG_ERROR, "PiFace: Error config file: unknown parameter %s found", Parametername.c_str());
					    break;
				    case 0:
				    case 1:
					    // found enable(d)
					    ...
					    if (ValueFound >= 0)
					    {
						    ...
						    Log(LOG_ERROR, "PiFace: Error config file: unknown value %s found", Parametervalue.c_str());
					    break;
				    ...
				    case 8:
					    ...
					    {
						    Log(LOG_ERROR, "PiFace: Error config file: invalid value %s found", Parametervalue.c_str());
						    break;
					    }
			...
			case 11:
				...
				{
					default:
						Log(LOG_ERROR, "PiFace: Error config file: unknown value %s found",
							 Parametervalue.c_str());
						break;
			...
			case 12:
			    ...
			    Log(LOG_ERROR, "PiFace: Error config file: misformed config line %s found", Line.c_str());
	    }
	}
	ConfigFile.close();
#ifndef DISABLE_NEW_FUNCTIONS
        if (Regenerate_Config)
        {
            ...
            Log(LOG_ERROR,"PiFace: We got an initial value setting, so we are now recreating the logfile to avoid looping", configfile.c_str() );
            AutoCreate_piface_config();
        }
#endif
    }
    else {
        Log(LOG_ERROR,"PiFace: Error PiFace config file: %s not found", configfile.c_str() );
        Log(LOG_ERROR,"PiFace: loading defaults for PiFace(s)");
        LoadDefaultConfig();
        AutoCreate_piface_config();
    }
    return result;
...
void CPiFace::AutoCreate_piface_config()
{
	char const *const explanation[] = {
		...
		"// Using the initial_value to set a counter to a desired (start) value, will result in spikes in the Oikomaticz graphs.\r\n",
		"// Note2: The initial_value is in counter counts, Oikomaticz scales (divides) this by the factor that is set in the GUI ",
		"settings \r\n",
		"//\r\n",
#endif
...
void CPiFace::CallBackSendEvent(const unsigned char *pEventPacket, const unsigned int PacketLength)
{
    ...
    if (m_send_queue.size() < 100)
        ...
        Log(LOG_ERROR, "PiFace: to much messages on queue!");
}
...
void CPiFace::CallBackSetPinInterruptMode(unsigned char devId,unsigned char pinID, bool Interrupt_Enable)
{
    ...
    //Log(LOG_NORM,"PiFace: SetPin Interrupt mode: devid: %d, Pinnr: %d, Enable %d-- Prev 0x%02X Cur 0x%02X",devId,pinID,Interrupt_Enable,Prev_Int_Enable_State,Cur_Int_Enable_State);
}
...
bool CPiFace::WriteToHardware(const char *pdata, const unsigned char length)
{
    ...
    if ((SendData->LIGHTING1.packettype == pTypeLighting1) && (SendData->LIGHTING1.subtype == sTypeIMPULS))
    {
        ...
        if (PortType == 'O')
        {
            //Log(LOG_NORM,"Piface: WriteToHardware housecode %c, packetlength %d", SendData->LIGHTING1.housecode,SendData->LIGHTING1.packetlength );
            CurrentLatchState = Read_MCP23S17_Register(devId, MCP23x17_OLATA);
            //Log(LOG_NORM,"PiFace: Read input state 0x%02X", m_OutputState[devId].Current);
            ...
            //  Log(LOG_NORM,"Piface: WriteToHardware housecode %c, devid %d, output %d, PrevOut 0x%02X, Set 0x%02X",PortType, devId, pinnr, CurrentLatchState,OutputData );
        }
        else
        {
            Log(LOG_ERROR, "Piface: wrong housecode %c", PortType);
            return false;
        }
    }
    else
    {
        Log(LOG_ERROR, "PiFace: WriteToHardware packet type %d or subtype %d unknown", SendData->LIGHTING1.packettype, SendData->LIGHTING1.subtype);
        return false;
    }
    return true;
}
...
{
    int devId;
    Log(LOG_STATUS,"PiFace: Worker started...");
    int msec_counter = 0;
    int sec_counter = 0;
    ...
    Log(LOG_STATUS,"PiFace: Worker stopped...");
}
...
int CPiFace::Init_SPI_Device(int Init)
{
    ...
    Log(LOG_STATUS,"PiFace: Starting PiFace_SPI_Start()");
#ifdef HAVE_LINUX_SPI
        ...
        if (ioctl (m_fd, SPI_IOC_WR_MODE, &spiMode)         >= 0)
        {
            if (ioctl (m_fd, SPI_IOC_WR_BITS_PER_WORD, &spiBPW) >= 0)
            {
                if (ioctl (m_fd, SPI_IOC_WR_MAX_SPEED_HZ, &speed)   >= 0)
                {
                    result=1;
                    //we are successfull
                    Log(LOG_NORM,"PiFace: SPI device opened successfully");

                }
                else
                    Log(LOG_NORM,"PiFace: SPI Speed Change failure: %s", strerror (errno)) ;
            }
            else
                Log(LOG_NORM,"PiFace: SPI BPW Change failure: %s", strerror (errno)) ;

        }
        else
            Log(LOG_NORM,"PiFace: SPI Mode Change failure: %s", strerror (errno)) ;
    }
    else
        Log(LOG_NORM,"PiFace: Unable to open SPI device: %s", strerror (errno));
...
int CPiFace::Detect_PiFace_Hardware()
{
    ...
    if (NrOfFoundBoards)
    {
        Log(LOG_STATUS,"PiFace: Found the following PiFaces:");
        for (devId=0; devId<4; devId++)
        {
            if (m_DetectedHardware[devId]==true)
                Log(LOG_STATUS,"PiFace: %d",devId);
        }
    }
    else Log(LOG_STATUS,"PiFace: Sorry, no PiFaces were found");
