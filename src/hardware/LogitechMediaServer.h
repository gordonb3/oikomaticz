...
class CLogitechMediaServer : public CDomoticzHardwareBase
{
	struct LogitechMediaServerNode
	{
		...
		std::string		IP;
		time_t			LastOK;
		device::tmedia::status::value	nStatus;
		std::string		sStatus;
...
private:
	notification::type::value	NotificationType(device::tmedia::status::value nStatus);
	void Do_Work();
	void GetPlayerInfo();
	...
	void UpdateNodeStatus(const LogitechMediaServerNode &Node, const device::tmedia::status::value nStatus, const std::string &sStatus, const bool bPingOK);
	void ReloadNodes();
