...
bool CEnOceanESP2::WriteToHardware(const char* pdata, const unsigned char /*length*/)
{
	...
	{
		Log(LOG_ERROR, "EnOcean: Can not switch with this DeviceID, use a switch created with our id_base!...");
		return false;
	}
	...
	case C_ORG_4BS:
	{
		if ((pFrame->DATA_BYTE0 & 0x08) == 0)
		{
			if (pFrame->DATA_BYTE0 & 0x80)
			{
				...
				Log(LOG_NORM, "EnOcean: 4BS, Teach-in diagram: Sender_ID: 0x%08lX\nManufacturer: 0x%02x (%s)\nProfile: 0x%02X\nType: 0x%02X (%s)",
		...
		else
		{
			...
			if (result.empty())
			{
				...
				if (pszHumenTxt)
				{
					Log(LOG_NORM, "EnOcean: Need Teach-In for %s", pszHumenTxt);
					free(pszHumenTxt);
				}
				return true;
			}
