...
bool CEnOceanESP2::WriteToHardware(const char* pdata, const unsigned char /*length*/)
{
	...
	if (!result.empty())
	{
		device::tswitch::type::value switchtype = static_cast<device::tswitch::type::value>(std::stoul(result[0][0]));
	...
	case C_ORG_4BS:
	{
		...
		{ // Following sensors need to have had a teach-in
			...
			{ // A5-12-00, Automated Meter Reading, Counter
				...
				uint32_t MR = ground(GetDeviceValue((pFrame->DATA_BYTE3 << 16) | (pFrame->DATA_BYTE2 << 8) | pFrame->DATA_BYTE1, 0, 16777215, 0.0F, scaleMax));
			...
			{ // A5-09-04, CO2 Gas Sensor with Temp and Humidity
				...
				SendAirQualitySensor(pFrame->ID_BYTE2, pFrame->ID_BYTE1, 9, ground(CONC), GetEEPLabel(enocean::RORG_4BS, Profile, iType));
