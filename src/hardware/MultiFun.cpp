...
MultiFun::MultiFun(const int ID, const std::string &IPAddress, const unsigned short IPPort)
	...
	, m_LastQuickAccess(0)
{
	Log(LOG_STATUS, "MultiFun: Create instance");
	m_HwdID = ID;
...
MultiFun::~MultiFun()
{
	Log(LOG_STATUS, "MultiFun: Destroy instance");
}
...
bool MultiFun::StartHardware()
{
	...
	Log(LOG_STATUS, "MultiFun: Start hardware");
#endif
...
{
#ifdef DEBUG_MultiFun
	Log(LOG_STATUS, "MultiFun: Stop hardware");
#endif
...
{
#ifdef DEBUG_MultiFun
	Log(LOG_STATUS, "MultiFun: Start work");
#endif
	...
	while (!IsStopRequested(1000))
	{
		...
		if (sec_counter % MULTIFUN_POLL_INTERVAL == 0)
		{
			...
			firstTime = false;
#ifdef DEBUG_MultiFun
			Log(LOG_STATUS, "MultiFun: fetching changed data");
#endif
		}
	}
	DestroySocket();
...
bool MultiFun::ConnectToDevice()
{
	...
	{
		Log(LOG_ERROR, "MultiFun: Unable to connect to specified IP Address on specified Port (%s:%d)", m_IPAddress.c_str(), m_IPPort);
		DestroySocket();
		return false;
	}

	Log(LOG_STATUS, "MultiFun: connected to %s:%d", m_IPAddress.c_str(), m_IPPort);
...
void MultiFun::DestroySocket()
{
	...
	{
#ifdef DEBUG_MultiFun
		Log(LOG_STATUS, "MultiFun: destroy socket");
#endif
		delete m_socket;
		m_socket = nullptr;
	}
...
void MultiFun::GetTemperatures()
{
	...
	if (ret > 0)
	{
		...
		{
			Log(LOG_ERROR, "MultiFun: Receive wrong number of bytes");
		}
		else
		{
	...
	{
		Log(LOG_ERROR, "MultiFun: Receive info about temperatures failed");
	}
}
...
void MultiFun::GetRegisters(bool firstTime)
{
	...
	{
		Log(LOG_ERROR, "MultiFun: Receive info about registers failed");
	}
}
...
int MultiFun::SendCommand(const unsigned char* cmd, const unsigned int cmdLength, unsigned char *answer, bool write)
{
	...
	{
		Log(LOG_ERROR, "MultiFun: Send command failed");
		DestroySocket();
		return -1;
	}
	...
	{
		Log(LOG_ERROR, "MultiFun: no data received");
		return -1;
	}
		...
		if (cmd[0] == databuffer[0] && cmd[1] == databuffer[1] && cmd[2] == databuffer[2] && cmd[3] == databuffer[3] && cmd[6] == databuffer[6])
		{
			if (cmd[7] == databuffer[7])
			{
				...
				if ((int)databuffer[4] * 256 + (int)databuffer[5] == (unsigned char)(answerLength + 2))
				{
					...
					Log(LOG_ERROR, "MultiFun: bad response after write");
				}
				else
				{
					Log(LOG_ERROR, "MultiFun: bad size of frame");
				}
			}
			else
				if (cmd[7] + 0x80 == databuffer[7])
				{
					...
					{
						Log(LOG_ERROR, "MultiFun: Receive error (%s)", errors[databuffer[8] - 1]);
					}
					else
					{
						Log(LOG_ERROR, "MultiFun: Receive unknown error");
					}
				}
				else
				{
					Log(LOG_ERROR, "MultiFun: Receive error (unknown function code)");
				}
		}
		else
		{
				Log(LOG_ERROR, "MultiFun: received bad frame prefix");
		}
	}
	else
	{
		Log(LOG_ERROR, "MultiFun: received frame is too short.");
		DestroySocket();
	}
