...
{
	Log(LOG_STATUS, "SolarEdgeAPI Worker started...");
	int sec_counter = 295;
	...
	Log(LOG_STATUS, "SolarEdgeAPI Worker stopped...");
}
...
bool SolarEdgeAPI::GetSite()
{
	...
	{
		Log(LOG_ERROR, "SolarEdgeAPI: Error getting http data (Sites)!");
		return false;
	}
#ifdef DEBUG_SolarEdgeAPIW
	...
	{
		Log(LOG_ERROR, "SolarEdgeAPI: Invalid data received!");
		return false;
	}
	if (root["sites"].empty() == true)
	{
		Log(LOG_ERROR, "SolarEdgeAPI: Invalid data received, or invalid APIKey");
		return false;
	}
...
void SolarEdgeAPI::GetInverters()
{
	...
	if (!HTTPClient::GET(sURL.str(), sResult))
	{
		Log(LOG_ERROR, "SolarEdgeAPI: Error getting http data (Equipment)!");
		return;
	}
#ifdef DEBUG_SolarEdgeAPIW
...
void SolarEdgeAPI::GetInverterDetails(const _tInverterSettings* pInverterSettings, const int iInverterNumber)
{
	...
	if (!HTTPClient::GET(sURL.str(), sResult))
	{
		Log(LOG_ERROR, "SolarEdgeAPI: Error getting http data (Equipment details)!");
		return;
	}
#ifdef DEBUG_SolarEdgeAPIW
