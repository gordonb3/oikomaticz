...
void CGpio::InterruptHandler()
{
	...
	for (auto &pin : pins)
		if (pin.GetPin() == pinPass)
		{
			...
			{
				Log(LOG_STATUS, "GPIO: Could not open file descriptor for GPIO %d", pinPass);
				pinPass = -1;
				return;
			}
			break;
	...
	Log(LOG_STATUS, "GPIO: Interrupt handler for GPIO %d started (TID: %d)", pin, (pid_t)syscall(SYS_gettid));
	while (!IsStopRequested(0))
	{
		if (waitForInterrupt(fd, 2000) > 0)
		{
			if (counter > 100)
			{
				Log(LOG_STATUS, "GPIO: Suppressing interruptstorm on GPIO %d, sleeping for 2 seconds..", pin);
				counter = -1;
			...
			{
				Log(LOG_NORM, "GPIO: Processing interrupt for GPIO %d...", pin);
				if (m_debounce > 0)
					sleep_milliseconds(m_debounce); // Debounce reading
				UpdateSwitch(pin, GPIOReadFd(fd));
				Log(LOG_NORM, "GPIO: Done processing interrupt for GPIO %d.", pin);

				if (counter > 0)
				{
					//Log(LOG_STATUS, "GPIO: Suppressed %d interrupts on previous call for GPIO %d.", counter, pin);
					counter = 0;
				}
			}
			else
	...
	Log(LOG_STATUS, "GPIO: Interrupt handler for GPIO %d stopped. TID: %d", pin, (pid_t)syscall(SYS_gettid));
}
...
bool CGpio::StartHardware()
{
	...
	Log(LOG_STATUS, "GPIO: This hardware is deprecated. Please transfer to the new SysFS hardware type!");
	...
	if (InitPins())
	{
		/* Disabled for now, devices should be added manually (this was the old behaviour, which we'll follow for now). Keep code for possible future usage.
		 ...
		 {
			Log(LOG_NORM, "GPIO: Error creating pins in DB, aborting...");
			RequestStop();
		 }*/
	...
	{
		Log(LOG_NORM, "GPIO: No exported pins found, aborting...");
		RequestStop();
	}
	m_bIsStarted = true;
	sOnConnected(this);
...
bool CGpio::StopHardware()
{
	...
	m_bIsStarted = false;
	StopHeartbeatThread();
	Log(LOG_NORM, "GPIO: Hardware stopped");
	return true;
}
...
void CGpio::UpdateStartup()
{
	...
	if (!result.empty())
	{
		...
		Log(LOG_NORM, "GPIO: Optional connected Master Oikomaticz now updates its status");
...
void CGpio::Poller()
{
	...
	Log(LOG_STATUS, "GPIO: Poller started (interval: %d sec, TID: %d)", m_pollinterval, (pid_t)syscall(SYS_gettid));
	...
	Log(LOG_STATUS, "GPIO: Poller stopped. TID: %d", (pid_t)syscall(SYS_gettid));
}
...
bool CGpio::InitPins()
{
	...
	for (int gpio_pin = GPIO_PIN_MIN; gpio_pin <= GPIO_PIN_MAX; gpio_pin++)
	{
		...
		if (fd != -1)
		{
			...
			//Log(LOG_NORM, "GPIO: Pin %d added (value: %d, direction: %d, edge: %d, active_low: %d, db_state: %d)",
