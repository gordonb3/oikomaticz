...
void CGpio::UpdateStartup()
{
	...
	if (!result.empty())
	{
		...
		Log(LOG_NORM, "Optional connected Master Oikomaticz now updates its status");
...
bool CGpio::CreateDomoticzDevices()
{
	...
	for(auto it = pins.begin(); it != pins.end(); ++it)
	{
		...
		if (createNewDevice)
			m_sql.safe_query(
				"INSERT INTO DeviceStatus (HardwareID, OrgHardwareID, DeviceID, Unit, Type, SubType, SwitchType, Used, SignalLevel, BatteryLevel, Name, nValue, sValue, Options) "
				"VALUES (%d, %d, 0, %d, %d, %d, %d, 0, 12, 255, '%q', %d, '', %d)",
				m_HwdID, 0, it->GetPin(), pTypeLighting1, sTypeIMPULS, int(device::tswitch::type::OnOff), it->GetIsInput() ? "Input" : "Output", GPIORead(it->GetPin(), "value"), it->GetIsInput() ? GPIO_IN
: GPIO_OUT);
