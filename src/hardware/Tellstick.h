...
class CTellstick : public CDomoticzHardwareBase
{
	...
	static bool Create(CTellstick **tellstick, int ID, int repeats, int repeatInterval);

	void SetSettings(int repeats, int repeatInterval);
	bool WriteToHardware(const char *pdata, unsigned char length) override;

      private:
	explicit CTellstick(const TelldusFunctions &functions, int ID, int repeats, int repeatInterval);
	...
	static void sensorEventCallback(const char *protocol, const char *model, int id, int dataType, const char *value, int timestamp, int callbackId, void *context);
	bool AddSwitchIfNotExits(int id, const char *devname, bool isDimmer);
	void Init();
