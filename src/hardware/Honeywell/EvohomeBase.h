...
#define RFX_SETID3(ID, id1, id2, id3)                                                                                                                                                                  \
	{                                                                                                                                                                                              \
		id1 = ID >> 16 & 0xFF;                                                                                                                                                                 \
		id2 = ID >> 8 & 0xFF;                                                                                                                                                                  \
		id3 = ID & 0xFF;                                                                                                                                                                       \
...
class CEvohomeDataType
{
	...
	static void Add(const uint24_t &in, unsigned char *msg, unsigned char &nOfs)
	{
		msg[nOfs++] = (in.val >> 16) & 0xFF;
		msg[nOfs++] = (in.val >> 8) & 0xFF;
		msg[nOfs++] = in.val & 0xFF;
	}
	static void Add(const uint16_t &in, unsigned char *msg, unsigned char &nOfs)
	{
		msg[nOfs++] = (in >> 8) & 0xFF;
		msg[nOfs++] = in & 0xFF;
	}
	static void Add(const int16_t &in, unsigned char *msg, unsigned char &nOfs)
	{
		msg[nOfs++] = (in >> 8) & 0xFF;
		msg[nOfs++] = in & 0xFF;
	}
	static void Add(const uint8_t &in, unsigned char *msg, unsigned char &nOfs)
	{
		msg[nOfs++] = in;
	}
	static void Add(const CEvohomeDataType &in, unsigned char *msg, unsigned char &nOfs)
	{
		in.Add(msg, nOfs);
	}
	void Add(unsigned char *msg, unsigned char &nOfs) const
	{
		nOfs = Encode(msg, nOfs);
	}

	static void Get(uint24_t &out, const unsigned char *msg, unsigned char &nOfs)
	{
		out.val = msg[nOfs++] << 16;
		out.val |= msg[nOfs++] << 8;
		out.val |= msg[nOfs++];
	}
	static void Get(uint16_t &out, const unsigned char *msg, unsigned char &nOfs)
	{
		out = msg[nOfs++] << 8;
		out |= msg[nOfs++];
	}
	static void Get(int16_t &out, const unsigned char *msg, unsigned char &nOfs)
	{
		out = msg[nOfs++] << 8;
		out |= msg[nOfs++];
	}
	static void Get(uint8_t &out, const unsigned char *msg, unsigned char &nOfs)
	{
		out = msg[nOfs++];
	}
	static void Get(CEvohomeDataType &out, const unsigned char *msg, unsigned char &nOfs)
	{
		out.Get(msg, nOfs);
	}
	void Get(const unsigned char *msg, unsigned char &nOfs)
	{
		nOfs = Decode(msg, nOfs);
	}
};
...
class CEvohomeTemp : public CEvohomeDataType
{
	...
	CEvohomeTemp(const unsigned char *msg, unsigned char nOfs)
		: m_nTemp(0)
	{
		Decode(msg, nOfs);
	}
	...
	static unsigned char DecodeTemp(int16_t &out, const unsigned char *msg, unsigned char nOfs)
	{
		Get(out, msg, nOfs);
		return nOfs;
	}
	...
	double GetTemp()
	{
		return m_nTemp / 100.0;
	}
	bool IsValid()
	{
		return m_nTemp != 0x7FFF;
	}
...
class CEvohomeID : public CEvohomeDataType
{
	...
	CEvohomeID() = default;
	CEvohomeID(unsigned int nID)
	{
		SetID(nID);
	}
	CEvohomeID(unsigned char idType, unsigned int idAddr)
	{
		SetID(idType, idAddr);
	}
	CEvohomeID(const std::string &szID)
	{
		SetID(szID);
	}
	CEvohomeID(const unsigned char *msg, unsigned char nOfs)
	{
		Decode(msg, nOfs);
	}
	~CEvohomeID() = default;
	...
	static unsigned char GetIDType(unsigned int nID)
	{
		return ((nID >> 18) & 0x3F);
	}
	static unsigned int GetIDAddr(unsigned int nID)
	{
		return (nID & 0x3FFFF);
	}
	static unsigned int GetID(unsigned char idType, unsigned int idAddr)
	{
		return idType << 18 | idAddr;
	}
	...
	unsigned int GetIDAddr() const
	{
		return GetIDAddr(GetID());
	}
	...
	void SetID(unsigned char idType, unsigned int idAddr)
	{
		SetID(GetID(idType, idAddr));
	}
...
class CEvohomeDateTime : public CEvohomeDataType
{
	...
	CEvohomeDateTime(const unsigned char *msg, unsigned char nOfs)
	{
		Decode(msg, nOfs);
	}
	~CEvohomeDateTime() = default;

	template <class T> CEvohomeDateTime &operator=(const T &in)
	{
		year = in->year;
		month = in->month;
		day = in->day;
	...
	{
		return DecodeTime(*this, msg, nOfs);
	}
	unsigned char DecodeDate(const unsigned char *msg, unsigned char nOfs)
	{
...
class CEvohomeMsg
{
	...
	CEvohomeMsg(packettype nType, int nAddr, int nCommand)
		: flags(0)
		, type(nType)
		, timestamp(0)
		, command(nCommand)
	...
	{
		SetID(1, nAddr);
		SetFlag(flgpkt | flgcmd);
	}
	CEvohomeMsg(packettype nType, int nAddr1, int nAddr2, int nCommand)
		: flags(0)
		, type(nType)
		, timestamp(0)
		, command(nCommand)
	...
	CEvohomeMsg(const CEvohomeMsg &src)
		: readofs(0)
		, enccount(0)
	{
		*this = src;
	...
	bool IsValid() const
	{
		return ((flags & flgvalid) == flgvalid) && (flags & (flgid2 | flgid3));
	}
	bool DecodePacket(const char *rawmsg);

	template <typename T> CEvohomeMsg &Add(const T &in)
	{
		CEvohomeDataType::Add(in, payload, payloadsize);
		return *this;
	}
	template <typename T> CEvohomeMsg &Add_if(const T &in, bool p)
	{
		if (p)
			Add(in);
		return *this;
	}
	template <typename T> CEvohomeMsg &Get(T &out)
	{
		CEvohomeDataType::Get(out, payload, readofs);
		return *this;
	}
	template <typename T> CEvohomeMsg &Get(T &out, int nOfs)
	{
		SetPos(nOfs);
		CEvohomeDataType::Get(out, payload, readofs);
		return *this;
	}
	template <typename T> CEvohomeMsg &Get_if(T &out, bool p)
	{
		if (p)
			Get(out);
		return *this;
	...
	void SetPos(unsigned char nPos)
	{
		readofs = nPos;
	}
