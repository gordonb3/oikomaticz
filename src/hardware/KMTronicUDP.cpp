...
void KMTronicUDP::Do_Work()
{
	...
	Log(LOG_STATUS, "KMTronic: UDP Worker started...");
	...
	Log(LOG_STATUS, "KMTronic: UDP Worker stopped...");
}
...
bool KMTronicUDP::WriteToHardware(const char *pdata, const unsigned char /*length*/)
{
	...
	if (packettype == pTypeLighting2)
	{
		...
		{ // get the host info
			Log(LOG_ERROR, "KMTronic: Error with IP address!...");
			return false;
		}
		...
		closesocket(udpSocket);
    		if (n < 0) {
			Log(LOG_ERROR, "KMTronic: Error sending relay command to: %s", m_szIPAddress.c_str());
			return false;
		}
		return true;
	}
...
void KMTronicUDP::GetMeterDetails()
{
	...
	closesocket(udpSocket);
    	if (n < 0) {
		Log(LOG_ERROR, "KMTronic: Error reading relay status from: %s", m_szIPAddress.c_str());
		return;
	}
