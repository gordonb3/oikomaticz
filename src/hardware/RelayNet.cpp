...
bool RelayNet::StopHardware()
{
	...
	m_bIsStarted = false;
	Log(LOG_STATUS, "RelayNet: Relay Module disconnected %s", m_szIPAddress.c_str());
...
void RelayNet::Do_Work()
{
	...
	{
		Log(LOG_STATUS, "RelayNet: %d-second poller started (%s)", m_poll_interval, m_szIPAddress.c_str());
	}
	...
	if (m_poll_inputs || m_poll_relays)
	{
		Log(LOG_STATUS, "RelayNet: %d-second poller stopped (%s)", m_poll_interval, m_szIPAddress.c_str());
	}
}
...
void RelayNet::SetupDevices()
{
	...
	if (m_relay_count)
	{
		for (int relayNumber = 1; relayNumber <= m_relay_count; relayNumber++)
		{
			...
			if (result.empty())
			{
				Log(LOG_STATUS, "RelayNet: Create %s/Relay%i", m_szIPAddress.c_str(), relayNumber);
	...
	if (m_input_count)
	{
		for (int inputNumber = 1; inputNumber <= m_input_count; inputNumber++)
		{
			...
			if (result.empty())
			{
				Log(LOG_STATUS, "RelayNet: Create %s/Input%i", m_szIPAddress.c_str(), inputNumber);
...
bool RelayNet::WriteToHardwareHttp(const char *pdata)
{
	...
	if (packettype == pTypeLighting2)
	{
		...
		if ((relay >= 1) && (relay <= m_relay_count))
		{
			...
			{
				Log(LOG_ERROR, "RelayNet: [1] Error sending relay command to: %s", m_szIPAddress.c_str());
				return false;
			}
			...
			{
				Log(LOG_ERROR, "RelayNet: [2] Error sending relay command to: %s", m_szIPAddress.c_str());
				return false;
			}
...
{
	Log(LOG_STATUS, "RelayNet: Connected to Relay Module %s", m_szIPAddress.c_str());
	m_bIsStarted = true;
}
...
{
	Log(LOG_STATUS, "RelayNet: Relay Module disconnected %s, reconnect", m_szIPAddress.c_str());
}
...
void RelayNet::OnError(const boost::system::error_code& error)
{
	...
	{
		Log(LOG_ERROR, "RelayNet: OnError: Can not connect to: %s:%d", m_szIPAddress.c_str(), m_usIPPort);
	}
	else if ((error == boost::asio::error::eof) || (error == boost::asio::error::connection_reset))
	{
		Log(LOG_STATUS, "RelayNet: OnError: Connection reset!");
	}
	else
	{
		Log(LOG_ERROR, "RelayNet: OnError: %s", error.message().c_str());
	}
}
