...
{
	Log(LOG_STATUS, "Mochad: connected to: %s:%d", m_szIPAddress.c_str(), m_usIPPort);
	m_bIsStarted = true;
...
{
	Log(LOG_STATUS, "Mochad: disconnected");
}
...
{
	Log(LOG_STATUS, "Mochad: trying to connect to %s:%d", m_szIPAddress.c_str(), m_usIPPort);
	int sec_counter = 0;
	...
	Log(LOG_STATUS,"Mochad: TCP/IP Worker stopped...");
}
...
void MochadTCP::OnError(const boost::system::error_code& error)
{
	...
	{
		Log(LOG_ERROR, "Mochad: Can not connect to: %s:%d", m_szIPAddress.c_str(), m_usIPPort);
	}
	else if (
	...
	{
		Log(LOG_STATUS, "Mochad: Connection reset!");
	}
	else
		Log(LOG_ERROR, "Mochad: %s", error.message().c_str());
}
...
//			case light1_sAllOn:
//			case light1_sAllOff:
		Log(LOG_STATUS, "Mochad: Unknown command %d:%d:%d", pdata[1],pdata[2],pdata[6]);
		return false;
	}
...
checkFunc:
	...
	return;
onError:
	Log(LOG_ERROR, "Mochad: Cannot decode '%s'", m_mochadbuffer);
