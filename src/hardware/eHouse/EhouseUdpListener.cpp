...
{
	int i;
	if (eHEIndex >= ETHERNET_EHOUSE_RM_MAX)
		return;
	if (eHEIndex < 0)
		return;
	for (i = 0; i <= eHEIndex; i++)
...
{
	int i = 0;
	if ((devh != 55) && (devh != 1) && (devh != 2))
		return -1;
	if ((devh == 1) && (devl == 1))
		return 0;
	if ((devh == 2) && (devl == 1))
		return EHOUSE1_RM_MAX;
	if (devl < 0)
		return -1;
	if (devl < EHOUSE1_RM_MAX)
		return devl;
...
void eHouseTCP::UpdateAuraToSQL(unsigned char AddrH, unsigned char AddrL, unsigned char index)
{
	...
	{
		Log(LOG_STATUS, "Aura to much !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! to much");
		return;
	}
	size = 4;
	...
	{
		if (m_CHANGED_DEBUG)
			Log(LOG_STATUS, "Temp Preset #%d changed to: %d", (int)index, (int)acurr);
	...
	// for (i = 0; i < size; i++)
	{
		...
		if ((acurr != aprev) || (m_AuraDevPrv[index]->Addr == 0))
		{
			...
			if (m_CHANGED_DEBUG)
				Log(LOG_STATUS, "Temp #%d changed to: %f", (int)index, acurr);
		}
	}
...
void eHouseTCP::UpdateCMToSQL(unsigned char AddrH, unsigned char AddrL, unsigned char index)
{
	...
	int acurr, aprev;
	if (AddrL < 250)
		return; // only CM/LM in this routine
	...
	for (i = 0; i < size; i++)
	{
		...
		if ((curr != prev) || (m_ECMPrv->data[1] == 0))
		{
			if (m_CHANGED_DEBUG)
				if (m_ECMPrv->data[1])
					Log(LOG_STATUS, "[CM 192.168.%d.%d] Out #%d changed to: %d", (int)AddrH, (int)AddrL, (int)i, (int)curr);
...

	{
		...
		for (i = 0; i < size; i++)
		{
			...
			if ((curr != prev) || (m_ECMPrv->data[1] == 0))
			{
				if (m_CHANGED_DEBUG)
					if (m_ECMPrv->data[1])
						Log(LOG_STATUS, "[LAN 192.168.%d.%d] Input #%d changed to: %d", (int)AddrH, (int)AddrL, (int)i, (int)curr);
		...
		if (m_ECM->CMB.CURRENT_ADC_PROGRAM != m_ECMPrv->CMB.CURRENT_ADC_PROGRAM)
		{
			...
			if (m_CHANGED_DEBUG)
				if (m_ECMPrv->data[1])
					Log(LOG_STATUS, "[LAN 192.168.%d.%d] Current ADC Program #%d changed to: %d", (int)AddrH, (int)AddrL, (int)i, (int)curr);
		}
		...
		if (m_ECM->CMB.CURRENT_PROGRAM != m_ECMPrv->CMB.CURRENT_PROGRAM)
		{
			...
			if (m_CHANGED_DEBUG)
				if (m_ECMPrv->data[1])
					Log(LOG_STATUS, "[LAN 192.168.%d.%d] Current Program #%d changed to: %d", (int)AddrH, (int)AddrL, (int)i, (int)curr);
		}
		...
		for (i = 0; i < size; i++)
		{
			...
			if ((curr != prev) || (m_ECMPrv->data[1] == 0))
			{
				if (m_CHANGED_DEBUG)
					if (m_ECMPrv->data[1])
						Log(LOG_STATUS, "[LAN 192.168.%d.%d] Dimmer #%d changed to: %d", (int)AddrH, (int)AddrL, (int)i, (int)curr);
...

	{
		...
		if (m_ECM->CMB.CURRENT_ZONE != m_ECMPrv->CMB.CURRENT_ZONE)
		{
			...
			if (m_CHANGED_DEBUG)
				if (m_ECMPrv->data[1])
					Log(LOG_STATUS, "[LAN 192.168.%d.%d] Current ZONE #%d changed to: %d", (int)AddrH, (int)AddrL, (int)i, (int)curr);
		}
		size = 48;
		// Outputs signals
		...
		for (i = 0; i < size; i++)
		{
			...
			if ((curr != prev) || (m_ECMPrv->data[1] == 0))
			{
				if (m_CHANGED_DEBUG)
					if (m_ECMPrv->data[1])
						Log(LOG_STATUS, "[LAN 192.168.%d.%d] Outs #%d changed to: %d", (int)AddrH, (int)AddrL, (int)i, (int)curr);
			}
		}
		...
		for (i = 0; i < size; i++)
		{
			...
			if ((curr != prev) || (m_ECMPrv->data[1] == 0))
			{
				// if (ECMPrv->data[1] ) Log(LOG_STATUS, "[LAN 192.168.%d.%d] Alarm #%d changed to: %d", (int) AddrH, (int) AddrL, (int)i, (int) curr);
		...
		for (i = 0; i < size; i++)
		{
			...
			if ((curr != prev) || (m_ECMPrv->data[1] == 0))
			{
				// if (ECMPrv->data[1]) Log(LOG_STATUS, "[LAN 192.168.%d.%d] Warning #%d changed to: %d", (int) AddrH, (int) AddrL, (int) i, (int) curr);
		...
		for (i = 0; i < size; i++)
		{
			...
			if ((curr != prev) || (m_ECMPrv->data[1] == 0))
			{
				// if (ECMPrv->data[1]) Log(LOG_STATUS, "[LAN 192.168.%d.%d] Monitoring #%d changed to: %d", (int) AddrH, (int) AddrL, (int) i, (int) curr);
...
void eHouseTCP::UpdateLanToSQL(unsigned char AddrH, unsigned char AddrL, unsigned char index)
{
	...
	{
		Log(LOG_ERROR, "LAN INDEX !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! to much");
		return;
	}
		...
		if ((curr != prev) || (m_eHERMPrev[index]->data[0] == 0))
		{
			if (m_CHANGED_DEBUG)
				if (m_eHERMPrev[index]->data[0])
					Log(LOG_STATUS, "[LAN 192.168.%d.%d] Out #%d changed to: %d", (int)AddrH, (int)AddrL, (int)i, (int)curr);
			UpdateSQLStatus(AddrH, AddrL, EH_LAN, 0x21, i, 100, curr, "", 100);
		}
	}
	// inputs
	size = 24;
	{
		...
		if (m_eHERMs[index]->eHERM.CURRENT_PROFILE != m_eHERMPrev[index]->eHERM.CURRENT_PROFILE)
		{
			...
			if (m_CHANGED_DEBUG)
				if (m_eHERMPrev[index]->data[0])
					Log(LOG_STATUS, "[LAN 192.168.%d.%d] Current Profile #%d changed to: %d", (int)AddrH, (int)AddrL, (int)i, (int)curr);
		}
		...
		for (i = 0; i < size; i++)
		{
			...
			if ((acurr != aprev) || (m_eHERMPrev[index]->data[0] == 0))
			{
				if (m_CHANGED_DEBUG)
					if (m_eHERMPrev[index]->data[0])
						Log(LOG_STATUS, "[LAN 192.168.%d.%d] ADC H Preset #%d changed to: %d", (int)AddrH, (int)AddrL, (int)i, (int)acurr);
			}
		}
		// ADCs L Preset
		for (i = 0; i < size; i++)
		{
			...
			if ((acurr != aprev) || (m_eHERMPrev[index]->data[0] == 0))
			{
				if (m_CHANGED_DEBUG)
					if (m_eHERMPrev[index]->data[0])
						Log(LOG_STATUS, "[LAN 192.168.%d.%d] ADC Low Preset #%d changed to: %d", (int)AddrH, (int)AddrL, (int)i, (int)acurr);
			}
		}
		// ADCs
		for (i = 0; i < size; i++)
		...
		for (i = 0; i < size; i++)
		{
			...
			if ((acurr != aprev) || (m_eHERMPrev[index]->data[0] == 0))
			{
				if (m_CHANGED_DEBUG)
					if (m_eHERMPrev[index]->data[0])
						Log(LOG_STATUS, "[LAN 192.168.%d.%d] TEMP Low Preset #%d changed to: %d", (int)AddrH, (int)AddrL, (int)i, (int)acurr);
			}
		}
		...
		for (i = 0; i < size; i++)
		{
			...
			if ((acurr != aprev) || (m_eHERMPrev[index]->data[0] == 0))
			{
				if (m_CHANGED_DEBUG)
					if (m_eHERMPrev[index]->data[0])
						Log(LOG_STATUS, "[LAN 192.168.%d.%d] Temp H Preset #%d changed to: %d", (int)AddrH, (int)AddrL, (int)i, (int)acurr);
		...
		if ((curr != prev) || (m_eHouseProStatusPrv->data[0] == 0))
		{
			if (m_CHANGED_DEBUG)
				if (m_eHouseProStatusPrv->data[0])
					Log(LOG_STATUS, "[PRO 192.168.%d.%d] OUT #%d changed to: %d", (int)AddrH, (int)AddrL, (int)i, (int)curr);
...
void eHouseTCP::UpdateWiFiToSQL(unsigned char AddrH, unsigned char AddrL, unsigned char index)
{
	...
	{
		Log(LOG_STATUS, " WIFI INDEX !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! to much");
		return;
	}
		...
		if ((curr != prev) || (m_eHWIFIPrev[index]->data[0] == 0))
		{
			if (m_CHANGED_DEBUG)
				if (m_eHWIFIPrev[index]->data[0])
					Log(LOG_STATUS, "[WIFI 192.168.%d.%d] Out #%d changed to: %d", (int)AddrH, (int)AddrL, (int)i, (int)curr);
	...
	size = 8;
	{
		for (i = 0; i < size; i++)
		{
			...
			if ((curr != prev) || (m_eHWIFIPrev[index]->data[0] == 0))
			{
				if (m_CHANGED_DEBUG)
					if (m_eHWIFIPrev[index]->data[0])
						Log(LOG_STATUS, "[WIFI 192.168.%d.%d] Input #%d changed to: %d", (int)AddrH, (int)AddrL, (int)i, (int)curr);
		...
		for (i = 0; i < size; i++)
		{
			...
			if ((curr != prev) || (m_eHWIFIPrev[index]->data[0] == 0))
			{
				if (m_CHANGED_DEBUG)
					if (m_eHWIFIPrev[index]->data[0])
						Log(LOG_STATUS, "[WIFI 192.168.%d.%d] Dimmer #%d changed to: %d", (int)AddrH, (int)AddrL, (int)i, (int)curr);
...
void eHouseTCP::UpdateRS485ToSQL(unsigned char AddrH, unsigned char AddrL, unsigned char index)
{
	...
	{
		Log(LOG_STATUS, "RS485 to much !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! to much");
		return;
	}
...

	{
		for (i = 0; i < size; i++)
		{
			...
			if ((curr != prev) || (m_eHRMPrev[index]->data[0] == 0))
			{
				if (m_CHANGED_DEBUG)
					if (m_eHRMPrev[index]->data[0])
						Log(LOG_STATUS, "[RS485 (%d,%d)] Input #%d changed to: %d", (int)AddrH, (int)AddrL, (int)i, (int)curr);
		...
		if (m_eHRMs[index]->eHERM.CURRENT_PROFILE != m_eHRMPrev[index]->eHERM.CURRENT_PROFILE)
		{
			...
			if (m_CHANGED_DEBUG)
				if (m_eHRMPrev[index]->data[0])
					Log(LOG_STATUS, "[RS485 (%d,%d)] Current Profile #%d changed to: %d", (int)AddrH, (int)AddrL, (int)i, (int)curr);
		}
		if (m_eHRMs[index]->eHERM.CURRENT_PROGRAM != m_eHRMPrev[index]->eHERM.CURRENT_PROGRAM)
		{
			...
			if (m_CHANGED_DEBUG)
				if (m_eHRMPrev[index]->data[0])
					Log(LOG_STATUS, "[RS485 (%d,%d) Current Program #%d changed to: %d", (int)AddrH, (int)AddrL, (int)i, (int)curr);
		}
		...
		for (i = 0; i < size; i++)
		{
			...
			if ((curr != prev) || (m_eHRMPrev[index]->data[0] == 0))
			{
				if (m_CHANGED_DEBUG)
					if (m_eHRMPrev[index]->data[0])
						Log(LOG_STATUS, "[RS485 (%d,%d)] Dimmer #%d changed to: %d", (int)AddrH, (int)AddrL, (int)i, (int)curr);
...
void eHouseTCP::TerminateUDP()
{
	...
	if (m_ViaTCP)
	{
		...
		{
#ifdef WIN32
			Log(LOG_STATUS, "ioctlsocket failed with error: %d", WSAGetLastError());
#else
			Log(LOG_STATUS, "ioctlsocket failed with error");
#endif
...
void eHouseTCP::IntToHex(unsigned char *buf, const unsigned char *inbuf, int received)
{
	...
	{
		buf[index] = table[inbuf[i] >> 4];
		index++;
		buf[index] = table[inbuf[i] & 0xf];
		index++;
	}
	buf[index] = 13;
	index++;
...

{
	...
	if (data[1] == 1)
		...
		nr = STATUS_ARRAYS_SIZE; // for EM
	else
		nr = data[2] % STATUS_ARRAYS_SIZE; // addrl for RM
	...
	if (m_PlanID < 0)
		m_PlanID = UpdateSQLPlan((int)data[1], (int)data[2], EH_RS485, (char *)&m_GetLine); // Add RoomPlan for RoomManager (RM)
	...
	for (i = 0; i < sizeof(m_eHn[nr]->Outs) / sizeof(m_eHn[nr]->Outs[0]); i++) // binary outputs names
	{
		...
		UpdateSQLState(data[1], data[2], EH_RS485, pTypeGeneralSwitch, sSwitchTypeAC, device::tswitch::type::OnOff, 0x21, i + 1, 1, 0, "0", Name, (char *)&m_GetLine, true, 100, m_PlanID);
	}
	...
	for (i = 0; i < sizeof(m_eHn[nr]->Inputs) / sizeof(m_eHn[nr]->Inputs[0]); i++) // binary inputs names
	{
		...
		if (nr != 0)
			UpdateSQLState(data[1], data[2], EH_RS485, pTypeGeneralSwitch, sSwitchGeneralSwitch, // not HM
				       device::tswitch::type::Contact, VISUAL_INPUT_IN, i + 1, 1, 0, "", Name, (char *)&m_GetLine, true, 100, m_PlanID);
	}
	...
	for (i = 0; i < sizeof(m_eHn[nr]->Dimmers) / sizeof(m_eHn[nr]->Dimmers[0]); i++) // dimmers names
	{
		...
		UpdateSQLState(data[1], data[2], EH_RS485, pTypeLighting2, sTypeAC, device::tswitch::type::Dimmer, VISUAL_DIMMER_OUT, i + 1, 1, 0, "", Name, (char *)&m_GetLine, true, 100, m_PlanID);
	}
	UpdateSQLState(data[1], data[2], EH_RS485, pTypeColorSwitch, sTypeColor_RGB_W, device::tswitch::type::Dimmer, VISUAL_DIMMER_RGB, 1, 1, 0, "", Name, "RGB", true, 100, m_PlanID); // RGB dimmer
	...
	for (i = 0; i < sizeof(m_eHn[nr]->Outs) / sizeof(m_eHn[nr]->Outs[0]); i += 2) // Blinds Names (use twin - single outputs) out #1,#2=> blind #1
	{
		...
		if (nr == STATUS_ARRAYS_SIZE)
			UpdateSQLState(data[1], data[2], EH_RS485, pTypeLighting2, sTypeAC, device::tswitch::type::BlindsPercentage, VISUAL_BLINDS, i + 1, 1, 0, "", Name, (char *)&m_GetLine, true, 100, m_PlanID);
	}
	...
	k = UpdateSQLState(data[1], data[2], EH_RS485, pTypeGeneralSwitch, sSwitchTypeSelector, device::tswitch::type::Selector, VISUAL_PGM, 1, 1, 0, "0", Name, "Scene", true, 100, m_PlanID);
	k = UpdateSQLState(data[1], data[2], EH_RS485, pTypeGeneralSwitch, sSwitchTypeSelector, device::tswitch::type::Selector, VISUAL_PGM, 1, 1, 0, "0", Name, "Scene", true, 100, m_PlanID);
	...
	/*    strcpy(PGMs,"SelectorStyle:1;LevelNames:"); //Add Regulation Program Selector
		...
		PGMs[strlen(PGMs) - 1] = 0; //remove last '|'
		k=UpdateSQLState(data[1], data[2], EH_LAN, pTypeGeneralSwitch, sSwitchTypeSelector, device::tswitch::type::Selector, VISUAL_APGM, 1, 1, 0,
	   "0", Name, "Reg. Scene", true, 100); k=UpdateSQLState(data[1], data[2], EH_LAN, pTypeGeneralSwitch, sSwitchTypeSelector,
	   device::tswitch::type::Selector, VISUAL_APGM, 1, 1, 0, "0", Name, "Reg. Scene", true, 100); UpdatePGM(data[1], data[2], VISUAL_APGM, PGMs,k);
	*/
...
{ // size =data[0]; //not used / invalid only LSB for eHouse UDP protocol compatibility
	...
	if (data[3] != 'n')
		return; // not names
	if (data[4] != '1')
		return;								   // other controller type than ERM
	...
	if (m_PlanID < 0)
		m_PlanID = UpdateSQLPlan((int)data[1], (int)data[2], EH_LAN, (char *)&m_GetLine); // Add RoomPlan for RoomManager (RM)
	...
	for (i = 0; i < sizeof(m_eHEn[nr]->Outs) / sizeof(m_eHEn[nr]->Outs[0]); i++) // binary outputs names
	{
		...
		UpdateSQLState(data[1], data[2], EH_LAN, pTypeGeneralSwitch, sSwitchTypeAC, device::tswitch::type::OnOff, 0x21, i, 1, 0, "0", Name, (char *)&m_GetLine, true, 100, m_PlanID);
	}
	...
	for (i = 0; i < sizeof(m_eHEn[nr]->Inputs) / sizeof(m_eHEn[nr]->Inputs[0]); i++) // binary inputs names
	{
		...
		UpdateSQLState(data[1], data[2], EH_LAN, pTypeGeneralSwitch, sSwitchGeneralSwitch, device::tswitch::type::Contact, VISUAL_INPUT_IN, i, 1, 0, "", Name, (char *)&m_GetLine, true, 100, m_PlanID);
	}
	...
	for (i = 0; i < sizeof(m_eHEn[nr]->Dimmers) / sizeof(m_eHEn[nr]->Dimmers[0]); i++) // dimmers names
	{
		...
		UpdateSQLState(data[1], data[2], EH_LAN, pTypeLighting2, sTypeAC, device::tswitch::type::Dimmer, VISUAL_DIMMER_OUT, i, 1, 0, "", Name, (char *)&m_GetLine, true, 100, m_PlanID);
	}
	UpdateSQLState(data[1], data[2], EH_LAN, pTypeColorSwitch, sTypeColor_RGB_W, device::tswitch::type::Dimmer, VISUAL_DIMMER_RGB, 0, 1, 0, "", Name, "RGB", true, 100, m_PlanID); // RGB dimmer
	GetStr(data);																		      // rollers names
...
//			if (k <= 10) strncat(PGMs, tmp, strlen(tmp));
	...
	PGMs[strlen(PGMs) - 1] = 0; // remove last '|'
	// Log(LOG_ERROR, "[PRG %d] %s", strlen(PGMs), PGMs);

	k = UpdateSQLState(data[1], data[2], EH_LAN, pTypeGeneralSwitch, sSwitchTypeSelector, device::tswitch::type::Selector, VISUAL_PGM, 1, 1, 0, "0", Name, "Scene", true, 100, m_PlanID);
	k = UpdateSQLState(data[1], data[2], EH_LAN, pTypeGeneralSwitch, sSwitchTypeSelector, device::tswitch::type::Selector, VISUAL_PGM, 1, 1, 0, "0", Name, "Scene", true, 100, m_PlanID);
	// ISO2UTF8(PGMs);
	UpdatePGM(data[1], data[2], VISUAL_PGM, PGMs, k);
	k = 0;
	...
	k = UpdateSQLState(data[1], data[2], EH_LAN, pTypeGeneralSwitch, sSwitchTypeSelector, device::tswitch::type::Selector, VISUAL_APGM, 1, 1, 0, "0", Name, "Reg. Scene", true, 100, m_PlanID);
	k = UpdateSQLState(data[1], data[2], EH_LAN, pTypeGeneralSwitch, sSwitchTypeSelector, device::tswitch::type::Selector, VISUAL_APGM, 1, 1, 0, "0", Name, "Reg. Scene", true, 100, m_PlanID);
	// ISO2UTF8(PGMs);
...
{ // size =data[0]; //not used / invalid only LSB for eHouse UDP protocol compatibility
	...
	if (m_PlanID < 0)
		m_PlanID = UpdateSQLPlan((int)data[1], (int)data[2], EH_PRO, (char *)&m_GetLine); // Add RoomPlan for RoomManager (RM)
	...
	for (i = 0; i < sizeof(m_eHouseProN->Outs) / sizeof(m_eHouseProN->Outs[0]); i++) // binary outputs names
	{
		...
		UpdateSQLState(data[1], data[2], EH_PRO, pTypeGeneralSwitch, sSwitchTypeAC, device::tswitch::type::OnOff, 0x21, i, 1, 0, "0", Name, (char *)&m_GetLine, true, 100, m_PlanID);
	}
	...
	for (i = 0; i < sizeof(m_eHouseProN->Inputs) / sizeof(m_eHouseProN->Inputs[0]); i++) // binary inputs names
	{
		...
		UpdateSQLState(data[1], data[2], EH_PRO, pTypeGeneralSwitch, sSwitchGeneralSwitch, device::tswitch::type::Contact, VISUAL_INPUT_IN, i, 1, 0, "", Name, (char *)&m_GetLine, true, 100, m_PlanID);
	}
	...
	for (i = 0; i < sizeof(m_eHouseProN->Dimmers) / sizeof(m_eHouseProN->Dimmers[0]); i++) // dimmers names
	{
		...
		//    UpdateSQLState(data[1], data[2], EH_PRO, pTypeLighting2, sTypeAC, device::tswitch::type::Dimmer, VISUAL_DIMMER_OUT, i, 1, 0, "", Name, (char *) &GetLine, true, 100);
	}
	// UpdateSQLState(data[1], data[2], EH_LAN, pTypeColorSwitch, sTypeColor_RGB_W, device::tswitch::type::Dimmer, VISUAL_DIMMER_RGB, 0, 1, 0, "", Name, "RGB", true, 100);  //RGB dimmer
	GetStr(data); // rollers names
	...
	{
		GetStr(data);
		UpdateSQLState(data[1], data[2], EH_PRO, pTypeLighting2, sTypeAC, device::tswitch::type::BlindsPercentage, VISUAL_BLINDS, i, 1, 0, "", Name, (char *)&m_GetLine, true, 100, m_PlanID);
	}
	...
	if (k > 0)
	{
		k = UpdateSQLState(data[1], data[2], EH_PRO, pTypeGeneralSwitch, sSwitchTypeSelector, device::tswitch::type::Selector, VISUAL_PGM, 1, 1, 0, "0", Name, "Scene", true, 100, m_PlanID);
		k = UpdateSQLState(data[1], data[2], EH_PRO, pTypeGeneralSwitch, sSwitchTypeSelector, device::tswitch::type::Selector, VISUAL_PGM, 1, 1, 0, "0", Name, "Scene", true, 100, m_PlanID);
		// ISO2UTF8(PGMs);
		UpdatePGM(data[1], data[2], VISUAL_PGM, PGMs, k);
	}
	...
	if (k > 0)
	{
		k = UpdateSQLState(data[1], data[2], EH_PRO, pTypeGeneralSwitch, sSwitchTypeSelector, device::tswitch::type::Selector, VISUAL_APGM, 1, 1, 0, "0", Name, "Reg. Scene", true, 100, m_PlanID);
		k = UpdateSQLState(data[1], data[2], EH_PRO, pTypeGeneralSwitch, sSwitchTypeSelector, device::tswitch::type::Selector, VISUAL_APGM, 1, 1, 0, "0", Name, "Reg. Scene", true, 100, m_PlanID);
		// ISO2UTF8(PGMs);
		UpdatePGM(data[1], data[2], VISUAL_APGM, PGMs, k);
	}
	...
	if (k > 0)
	{
		k = UpdateSQLState(data[1], data[2], EH_PRO, pTypeGeneralSwitch, sSwitchTypeSelector, device::tswitch::type::Selector, VISUAL_SECUPGM, 1, 1, 0, "0", Name, "Pgm. Secu", true, 100, m_PlanID);
		k = UpdateSQLState(data[1], data[2], EH_PRO, pTypeGeneralSwitch, sSwitchTypeSelector, device::tswitch::type::Selector, VISUAL_SECUPGM, 1, 1, 0, "0", Name, "Pgm. Secu", true, 100, m_PlanID);
		// ISO2UTF8(PGMs);
	...
	if (k > 0)
	{
		k = UpdateSQLState(data[1], data[2], EH_PRO, pTypeGeneralSwitch, sSwitchTypeSelector, device::tswitch::type::Selector, VISUAL_ZONEPGM, 1, 1, 0, "0", Name, "Zones", true, 100, m_PlanID);
		k = UpdateSQLState(data[1], data[2], EH_PRO, pTypeGeneralSwitch, sSwitchTypeSelector, device::tswitch::type::Selector, VISUAL_ZONEPGM, 1, 1, 0, "0", Name, "Zones", true, 100, m_PlanID);
		// ISO2UTF8(PGMs);
...
{ // size =data[0]; //not used / invalid only LSB for eHouse UDP protocol compatibility
	...
	if (m_PlanID < 0)
		m_PlanID = UpdateSQLPlan((int)data[1], (int)data[2], EH_WIFI, (char *)&m_GetLine); // Add RoomPlan for RoomManager (RM)
	...
	for (i = 0; i < sizeof(m_eHWIFIn[nr]->Outs) / sizeof(m_eHWIFIn[nr]->Outs[0]); i++) // binary outputs names
	{
		...
		UpdateSQLState(data[1], data[2], EH_WIFI, pTypeGeneralSwitch, sSwitchTypeAC, device::tswitch::type::OnOff, 0x21, i + 1, 1, 0, "0", Name, (char *)&m_GetLine, true, 100, m_PlanID);
	}
	...
	for (i = 0; i < sizeof(m_eHWIFIn[nr]->Inputs) / sizeof(m_eHWIFIn[nr]->Inputs[0]); i++) // binary inputs names
	{
		...
		UpdateSQLState(data[1], data[2], EH_WIFI, pTypeGeneralSwitch, sSwitchGeneralSwitch, device::tswitch::type::Contact, VISUAL_INPUT_IN, i + 1, 1, 0, "", Name, (char *)&m_GetLine, true, 100, m_PlanID);
	}
	...
	for (i = 0; i < sizeof(m_eHWIFIn[nr]->Dimmers) / sizeof(m_eHWIFIn[nr]->Dimmers[0]); i++) // dimmers names
	{
		...
		UpdateSQLState(data[1], data[2], EH_WIFI, pTypeLighting2, sTypeAC, device::tswitch::type::Dimmer, VISUAL_DIMMER_OUT, i + 1, 1, 0, "", Name, (char *)&m_GetLine, true, 100, m_PlanID);
	}
	UpdateSQLState(data[1], data[2], EH_WIFI, pTypeColorSwitch, sTypeColor_RGB_W, device::tswitch::type::Dimmer, VISUAL_DIMMER_RGB, 1, 1, 0, "", Name, "RGB", true, 100, m_PlanID); // RGB dimmer
	GetStr(data);																		       // rollers names
	...
	{
		GetStr(data);
		UpdateSQLState(data[1], data[2], EH_WIFI, pTypeLighting2, sTypeAC, device::tswitch::type::BlindsPercentage, VISUAL_BLINDS, i + 1, 1, 0, "", Name, (char *)&m_GetLine, true, 100, m_PlanID);
	}
	...
	*/
	...
	k=UpdateSQLState(data[1], data[2], EH_LAN, pTypeGeneralSwitch, sSwitchTypeSelector, device::tswitch::type::Selector, VISUAL_PGM, 1, 1, 0, "0", Name,
	"Scene", true, 100); k=UpdateSQLState(data[1], data[2], EH_LAN, pTypeGeneralSwitch, sSwitchTypeSelector, device::tswitch::type::Selector, VISUAL_PGM,
	...
	k=UpdateSQLState(data[1], data[2], EH_LAN, pTypeGeneralSwitch, sSwitchTypeSelector, device::tswitch::type::Selector, VISUAL_APGM, 1, 1, 0, "0", Name,
	"Reg. Scene", true, 100); k=UpdateSQLState(data[1], data[2], EH_LAN, pTypeGeneralSwitch, sSwitchTypeSelector, device::tswitch::type::Selector,
...

{
	...
	{
		Log(LOG_STATUS, "[%s] Disabled", LogPrefix);
		return;
	}
	Log(LOG_STATUS, "!!!!! eHouseUDP: Do Work Starting");
	...
	if (m_ViaTCP == 0)
	{
		...
		opt = 1;
		setsockopt(m_eHouseUDPSocket, SOL_SOCKET, SO_REUSEADDR, (char *)&opt, sizeof(opt));
		opt = 1;
		setsockopt(m_eHouseUDPSocket, SOL_SOCKET, SO_KEEPALIVE, (char *)&opt, sizeof(opt));
...
#else
		opt = 100;
		setsockopt(m_eHouseUDPSocket, SOL_SOCKET, SO_RCVTIMEO, (char *)&opt, sizeof(opt));
		opt = 100;
		setsockopt(m_eHouseUDPSocket, SOL_SOCKET, SO_SNDTIMEO, (char *)&opt, sizeof(opt));
		opt = 1;
		setsockopt(m_eHouseUDPSocket, IPPROTO_UDP, TCP_NODELAY, (char *)&opt, sizeof(opt)); // < 0)
		/*{
		LOG(LOG_ERROR, "[TCP Cli Status] Set TCP No Delay failed");
		perror("[TCP Cli Status] Set TCP No Delay failed\n");
		}
	...
	while (!IsStopRequested(0)) // main loop
	{
		...
		if (m_ViaTCP)
		{
			...
			if ((SecIter % 100) == 1) // 15-30 sec - send keep alive
			{
				...
				// Log(LOG_STATUS, "[TCP Keep Alive ] ");
				char dta[2];
				dta[0] = 'q';
				send(m_TCPSocket, dta, 1, 0);
			}
		}
		else if ((SecIter % 100) == 1)
		{
		...
		else
		{
			...
			else // recover multiple packets for Slow links (eg. Internet)
			{
				...
				else if (nbytes < 0) // SOCKET ERROR
				{
					...
					comm++;
					// Log(LOG_STATUS, "Nbytes<0");
					if ((m_ViaTCP) && (comm > 600)) // TCP must try reconnect
					{
						comm = 0;
						Log(LOG_STATUS, "Connection Lost. Reconnecting...");
		...
		if (nbytes > 254)
		{
			...
			{ // get names
				...
				Log(LOG_STATUS, "[%s NAMES: 192.168.%d.%d ] ", LogPrefix, udp_status[1], udp_status[2]);
				if ((m_eHEnableAutoDiscovery))
				{
					...
					if (m_eHEnableProDiscovery)
						GetUDPNamesPRO(udp_status, nbytes);
					// if (!TESTTEST)
			...
			{ // eHouse PRO  status
				...
				if (udp_status[0] != 0xff)
					continue;
				if (udp_status[1] != 0x55)
					continue;
				if (udp_status[2] != 0xff)
					continue;
				if (udp_status[3] != 0x55)
					continue;
				if (m_ViaTCP)
				{
				...
				if (((sum & 0xffff) == (sum2 & 0xffff)) || (m_ViaTCP)) // if valid then perform
				{
					...
					if (m_StatusDebug)
						LOG(LOG_STATUS, "[%s PRO] status installation from Server: 192.168.%d.%d", LogPrefix, m_ipaddrh, m_ipaddrl);
					memcpy(m_eHouseProStatus->data, &udp_status, sizeof(m_eHouseProStatus->data));
					UpdatePROToSQL(devaddrh, devaddrl);
				}
				else
					Log(LOG_STATUS, "[%s Pro] Invalid Checksum %d,%d", LogPrefix, devaddrh, devaddrl);
			}
			continue;
		}
		...
		{ // size more than 0
			...
			if (((sum & 0xffff) == (sum2 & 0xffff)) || (eh1)) // if valid then perform
			{
				...
				if (m_disablers485)
				{
					if ((devaddrh == 55) || (devaddrh == 1) || (devaddrh == 2))
						continue;
				}
				...
				{
					if (m_StatusDebug)
						Log(LOG_STATUS, "[%s] St: (%-3d,%-3d) - OK (%dB)", LogPrefix, devaddrh, devaddrl, nbytes);
				}
				else
					// if    (udp_status[3] == 'l')
					if (nbytes > 6)
				...
				if (index >= 0)
				{
					...
					{
						if (TESTTEST)
							LOG(LOG_STATUS, "Ignore other instalation from Serwer: 192.168.%d.%d", m_ipaddrh, m_ipaddrl);
						continue;
					}

					eHaloc(index, devaddrh, devaddrl);
					if (memcmp(&m_eHn[index]->BinaryStatus[0], &udp_status, nbytes) != 0)
				...
				else // ethernet controllers or CM without eHouse1 status
				{
					if (IsCM(devaddrh, devaddrl)) // CommManager/Level Manager
					{
						...
						m_ECMn->BinaryStatusLength = nbytes;
						m_ECMn->TCPQuery++;
						if (memcmp(m_ECM->data, &udp_status[70], sizeof(m_ECM->data) - 70) != 0)
							m_CloudStatusChanged = 1;
						...
						if (devaddrl != 0)
						{
							...
							else if (devaddrl >= m_INITIAL_ADDRESS_WIFI) // eHouse WiFi Controllers
							{
								if (devaddrl - m_INITIAL_ADDRESS_WIFI <= EHOUSE_WIFI_MAX)
								{
									...
									{
										if (memcmp(m_eHWIFIn[eHWiFiIndex]->BinaryStatus, udp_status, nbytes) != 0)
											m_CloudStatusChanged = 1;
							...
							else
							{
								if (devaddrh == 0x81) // Aura Thermostats Via eHouse PRO
								{
									...
									m_AuraDev[aindex]->ID |= m_AuraN[aindex]->BinaryStatus[i++];
									if (m_DEBUG_AURA)
										LOG(LOG_STATUS, "[Aura UDP %d] ID: %lx\t", aindex + 1, (long int)m_AuraDev[aindex]->ID);
									m_AuraDev[aindex]->DType = m_AuraN[aindex]->BinaryStatus[i++]; // device type
									if (m_DEBUG_AURA)
										LOG(LOG_STATUS, " DevType: %d\t", m_AuraDev[index]->DType);
									i++; // params count
									int m, k = 0;
									int bkpi = i + 2;
									char FirstTime = 0;
									for (m = 0; m < sizeof(m_AuraN[aindex]->ParamSymbol); m++)
									{
										// if ((AuraDev[index].ParamSize>2) && (!FirstTime))
										{
											...
											if (((m_AuraN[aindex]->ParamSymbol[m] == 'C') || (m_AuraN[aindex]->ParamSymbol[m] == 'T')) && (!FirstTime))
											{
												...
												//                                                 AuraDev[aindex]->TempSet =
												//                                                 AuraN[aindex]->ParamPreset[m];
												...
												if (m_AuraDev[aindex]->LocalTempSet != m_AuraDev[aindex]->PrevLocalTempSet)
												{
													...
													if (m_DEBUG_AURA)
														LOG(LOG_STATUS, "LTempC: %f\t", m_AuraDev[aindex]->TempSet);
												}
												if (m_AuraDev[aindex]->ServerTempSet != m_AuraDev[aindex]->PrevServerTempSet)
												{
													...
													if (m_DEBUG_AURA)
														LOG(LOG_STATUS, "STempC: %f\t", m_AuraDev[aindex]->TempSet);
												...
												// for (i = 0; i<AuraDev[auraindex].ParamsCount; i++)
												{
													...
													m_eHouseProStatus->status.AdcVal[aindex] =
														m_AuraN[aindex]
															->ParamValue[0]; // temp * 10;//.MSB<<8) +
																	 // eHouseProStatus.status.AdcVal[nr_of_ch].LSB;
									...
									if (m_DEBUG_AURA)
										LOG(LOG_STATUS, "DoorState: %d\t", m_AuraDev[aindex]->DoorState);
									m_AuraDev[aindex]->Door = m_AuraN[aindex]->BinaryStatus[i++];
									if (m_DEBUG_AURA)
										LOG(LOG_STATUS, "Door: %d\t", m_AuraDev[aindex]->Door);
									...
									m_adcs[aindex]->flagsa |= m_AuraN[aindex]->BinaryStatus[i++];
									if (m_DEBUG_AURA)
										LOG(LOG_STATUS, "FlagsA: %lx\t", m_adcs[aindex]->flagsa);
									...
									if (m_DEBUG_AURA)
										LOG(LOG_STATUS, "FlagsB: %lx\t", (long unsigned int)m_adcs[aindex]->flagsb);
									m_AuraDev[aindex]->LQI = m_AuraN[aindex]->BinaryStatus[i++];
									if (m_DEBUG_AURA)
										LOG(LOG_STATUS, "LQI: %d\t", m_AuraDev[aindex]->LQI);
