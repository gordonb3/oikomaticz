...
void eHouseTCP::ExecQueuedEvents()
{
	...
	if (!m_EventsCountInQueue)
		return; // nothing availabla - we don't waste time here
	...
	for (i = 0; i < EVENT_QUEUE_MAX; i++)
	{
		if (m_EvQ[i]->LocalEventsToRun[2]) // not empty event
		{
			...
			{
				if (m_EvQ[i]->LocalEventsDrop)
					Log(LOG_STATUS, "[Perform Queue Event] Q[%d] Now -> Retry: %d", i, m_EvQ[i]->LocalEventsDrop);
				...
				if (m_EvQ[i]->LocalEventsDrop > RE_SEND_RETRIES) // to much retries (no controller or no communication)
				{
					if (m_DEBUG_TCPCLIENT)
						Log(LOG_STATUS, "[Exec Event] Q[%d] Drop Event", i);
...
{ // start
	...
	Log(LOG_STATUS, "[Add Event]: %02X%02X%02X%02X%02X%02X%02X%02X%02X%02X", Event[0], Event[1], Event[2], Event[3], Event[4], Event[5], Event[6], Event[7], Event[8], Event[9]);
	...
	if (i >= 0) // event already exists in EventQueue
	{
		if (m_EvQ[i]->LocalEventsTimeOuts >= RUN_NOW) // Wasn't confirmed - No Successful Execution so far
		{
			...
			m_EvQ[i]->LocalEventsTimeOuts = RUN_NOW - 1; // Increase time of event not to disappear from queue if it recently run
		m_EventsCountInQueue++;
		if (m_DEBUG_TCPCLIENT)
			Log(LOG_STATUS, "[Add Event] Already Exists Q[%d]", i);
		return i;
	}
	...
	for (i = 0; i < EVENT_QUEUE_MAX; i++)
	{
		...
		{
			if (m_DEBUG_TCPCLIENT)
				Log(LOG_STATUS, "[Add Event] Adding Q[%d]", i);
			...
			if (m_EvQ[i]->LocalEventsToRun[0] == m_SrvAddrH)
				...
				m_EvQ[i]->iface = EV_VIA_EHOUSE_PRO;
			return i;
		}
	}
	if (m_DEBUG_TCPCLIENT)
		Log(LOG_STATUS, "[Add Event] End of proc");
	return -1;
}
