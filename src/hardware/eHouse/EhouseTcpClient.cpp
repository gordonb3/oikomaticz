...
signed int eHouseTCP::IndexOfEthDev(unsigned char AddrH, unsigned char AddrL)
{
	...
	if ((m_SrvAddrH == AddrH) && (m_SrvAddrL == AddrL))
		return INDEX_PRO;
	...
	for (i = 0; i < EHOUSE_WIFI_MAX; i++)
	{
		if (strlen((char *)&m_eHWIFIn[i]) > 0)
			if ((m_eHWiFi[i]->status.AddrH == AddrH) && (m_eHWiFi[i]->status.AddrL == AddrL))
				return INDEX_WIFI + i;
	}
...
char eHouseTCP::SendTCPEvent(const unsigned char *Events, unsigned char EventCount, unsigned char AddrH, unsigned char AddrL, const unsigned char *EventsToRun)
{
	...
	for (i = 0; i < MAX_CLIENT_SOCKETS; i++) // search first free socket (TCP Client instance)
	{
		...
		if (m_TC[i].ActiveConnections >= 0)
			continue;
		if (m_TC[i].NotFinished)
			continue;
	...
	ExecQueuedEvents();
#else
	m_EhouseTcpClientThread[tcp_client_socket_index] = std::make_shared<std::thread>([this, tcp_client_socket_index] { EhouseSubmitData(tcp_client_socket_index); });
...
#endif
{
	...
	{
		Log(LOG_ERROR, "[eHouse TCP Client] Too many sockets");
		return;
	}
#endif
	...
	{
		Log(LOG_ERROR, "[TCP Cli %d] Could not create socket", SocketIndex);
	...
	{
		// Log(LOG_ERROR, "[TCP Cli %d] Cant Set TCP NODELAY", SocketIndex);
	}
	...
	if (m_DEBUG_TCPCLIENT)
		Log(LOG_STATUS, "[TCP Cli %d] Connecting to: %s", SocketIndex, line);
	if (connect(ClientCon->Socket, (struct sockaddr *)&server, sizeof(server)) < 0)
	{
		Log(LOG_ERROR, "[TCP Cli %d] error connecting: %s", SocketIndex, line);
		eHTerminate(SocketIndex)
	}
	if (m_DEBUG_TCPCLIENT)
		Log(LOG_STATUS, "[TCP Cli %d] Authorizing", SocketIndex);
	...
	{
		// Log(LOG_STATUS, "[TCP Cli %d] Not Receive Complete Data: %d",SocketIndex,status);
		if (status < 0)
			eHTerminate(SocketIndex)			// Error in socket
	...
	{		 // only Hashed password with VendorCode available for OpenSource
		{
			// Log(LOG_STATUS, "[TCP Cli %d] Xored Password",SocketIndex);
		...
		eHTerminate(SocketIndex) // Wrong data size of received data
			if (m_DEBUG_TCPCLIENT) Debug(DEBUG_HARDWARE, "[TCP Cli %d] Sending ch-re", SocketIndex);
	...
	{
		// Log(LOG_STATUS, "[TCP Cli %d] NotSend Complete Data: %d",SocketIndex,status);
		//                if (!TC[SocketIndex].TimeOut) eHTerminate(SocketIndex)  //not used here socket timeouts used
		if (!(iter--))
			eHTerminate(SocketIndex)			 // To many retries then Close
				if (status < 0) eHTerminate(SocketIndex) // Error in connection
	}
	if (m_DEBUG_TCPCLIENT)
		Log(LOG_STATUS, "[TCP Cli %d] Receive Confirmation", SocketIndex);
	...
	{
		if (status < 0)
			eHTerminate(SocketIndex) // error in connection
	...
	{
		Log(LOG_STATUS, "[TCP Cli %d] Events Accepted", SocketIndex);
		unsigned char k = 0;
	...
	{
		Log(LOG_STATUS, "[TCP Cli %d] No Confirmation", SocketIndex);
	}
#ifdef WIN32
	timeout = 100;
#else
	...
	if (setsockopt(ClientCon->Socket, IPPROTO_TCP, TCP_NODELAY, &kkk, sizeof(kkk)) < 0) // Set socket send data immediately
	{
		// Log(LOG_STATUS, "[TCP Cli %d] Cant Set TCP NODELAY", SocketIndex);
	}
	...
	if (m_DEBUG_TCPCLIENT)
		Log(LOG_STATUS, "[TCP Cli %d] Sending termination of connection", SocketIndex);
	if ((status = send(ClientCon->Socket, (char *)&challange, 1, 0)) != 1)
	{
		if (m_DEBUG_TCPCLIENT)
			Log(LOG_STATUS, "[TCP Cli %d] Not sent termination (No Problem)", SocketIndex);
		if (status < 0)
			eHTerminate(SocketIndex)
	}
	if (m_DEBUG_TCPCLIENT)
		Log(LOG_STATUS, "[TCP Cli %d] Termination Sent OK", SocketIndex);
...
void eHouseTCP::KillSocket(int SocketIndex)
{
...
#endif
	if (m_DEBUG_TCPCLIENT)
		Log(LOG_STATUS, "[TCP Cli %d] Closing Connection", SocketIndex);
	...
	if (m_DEBUG_TCPCLIENT)
		Log(LOG_STATUS, "[TCP Cli %d] End TCP Client", SocketIndex);
	return;
	//                TC[SocketIndex].TimeOut=0;    //used socket timeouts

	if (m_DEBUG_TCPCLIENT)
		Log(LOG_STATUS, "[!!!!!!!!! TCP Cli ||||| %d] End TCP Client NOT EXITED", SocketIndex);
}
