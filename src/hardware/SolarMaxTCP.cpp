...
bool SolarMaxTCP::ConnectInternal()
{
	...
	{
		Log(LOG_ERROR, "SolarMax: TCP could not create a TCP/IP socket!");
		return false;
	}
	/*
	...
	*/
	...
	if (nRet == SOCKET_ERROR)
	{
		...
		m_socket = INVALID_SOCKET;
		Log(LOG_ERROR, "SolarMax: TCP could not connect to: %s:%d", m_szIPAddress.c_str(), m_usIPPort);
		return false;
	}

	Log(LOG_STATUS, "SolarMax: TCP connected to: %s:%d", m_szIPAddress.c_str(), m_usIPPort);
...
void SolarMaxTCP::Do_Work()
{
	...
	while (!IsStopRequested(1000))
	{
		...
		if (m_socket == INVALID_SOCKET)
		{
			...
			if (m_retrycntr >= RETRY_DELAY)
			{
				...
				if (!ConnectInternal())
				{
					Log(LOG_STATUS, "SolarMax: retrying in %d seconds...", RETRY_DELAY);
				}
			}
		}
		else
		{
			if ((sec_counter % POLL_INTERVAL == 0) || (bFirstTime))
			{
				...
				if (IsStopRequested(0))
					break;
				if (bread <= 0) {
					Log(LOG_ERROR, "SolarMax: TCP/IP connection closed! retrying in %d seconds...", RETRY_DELAY);
					disconnect();
					m_retrycntr = 0;
					continue;
				}
	...
	Log(LOG_STATUS, "SolarMax: TCP/IP Worker stopped...");
}
...
void SolarMaxTCP::ParseLine()
{
	...
	{
		Log(LOG_ERROR, "SolarMax: Invalid data received!");
		return;
	}
