...
bool CPinger::WriteToHardware(const char *pdata, const unsigned char length)
{
	...
	if (result.empty())
		Log(LOG_ERROR, "Pinger: Unknown ID (%08X)", nodeID);
	else
		Log(LOG_ERROR, "Pinger: This is a read-only sensor! (Name: %s, IP: %s)", result[0][0].c_str(), result[0][1].c_str());
...
{
	//Log(LOG_STATUS, "Pinger: %s = %s", Node.Name.c_str(), (bPingOK == true) ? "OK" : "Error");
	if (!bPingOK)
	{
		//Log(LOG_STATUS, "Pinger: Could not ping host: %s", Node.Name.c_str());
	}
...
void CPinger::Do_Work()
{
	...
	Log(LOG_STATUS, "Pinger: Worker started...");
	...
	Log(LOG_STATUS, "Pinger: Worker stopped...");
}
