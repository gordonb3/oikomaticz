...
void CWunderground::Do_Work()
{
...
#endif
	int sec_counter = 590;
	Log(LOG_STATUS, "Wunderground: Worker started...");
	...
	Log(LOG_STATUS,"Wunderground: Worker stopped...");
}
...
std::string CWunderground::GetWeatherStationFromGeo()
{
	...
	if (m_sql.GetPreferencesVar("Location", nValue, sValue))
	{
		...
		if (strarray.size() == 2)
		{
			...
			{
				Log(LOG_ERROR, "Wunderground: Error getting location/near result! (Check API key!)");
				return "";
			}
#ifdef DEBUG_WUNDERGROUNDW
			...
			{
				Log(LOG_ERROR, "WUnderground: Problem getting location/near result. Invalid data received! (Check Station ID!)");
				return "";
			}
			...
			if (!bValid)
			{
				Log(LOG_ERROR, "WUnderground: Problem getting location/near result.Invalid data received, or no data returned!");
				return "";
			}
			...
			Log(LOG_ERROR, "WUnderground: Problem getting location/near result. No stations returned!");
		}
	}
	return "";
}
...
void CWunderground::GetMeterDetails()
{
	...
	if (!HTTPClient::GET(sURL.str(), sResult))
	{
		Log(LOG_ERROR,"Wunderground: Error getting http data! (Check API key!)");
		return;
	}
#ifdef DEBUG_WUNDERGROUNDW
	...
	if ((!ret) || (!root.isObject()))
	{
		Log(LOG_ERROR,"WUnderground: Invalid data received! (Check Station ID!)");
		return;
	}
	...
	if (!bValid)
	{
		Log(LOG_ERROR, "WUnderground: Invalid data received, or no data returned!");
		return;
	}
	...
	if (!m_bFirstTime)
	{
		...
		if (difftime(tlocal, tobserver) >= 1800)
		{
			...
			Log(LOG_ERROR, "WUnderground: Receiving old data from WU! (No new data return for more than 30 minutes)");
			return;
		}
	}
	m_bFirstTime = false;
