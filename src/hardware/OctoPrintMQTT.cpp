...
{
	Log(LOG_STATUS, "OCTO_MQTT: Subscribed");
	m_IsConnected = true;
}
...
void COctoPrintMQTT::on_connect(int rc)
{
	...
	*/
	...
	if (rc == 0) {
		if (m_IsConnected) {
			Log(LOG_STATUS, "OCTO_MQTT: re-connected to: %s:%d", m_szIPAddress.c_str(), m_usIPPort);
		}
		else {
			Log(LOG_STATUS, "OCTO_MQTT: connected to: %s:%d", m_szIPAddress.c_str(), m_usIPPort);
			m_IsConnected = true;
			sOnConnected(this);
		}
		...
		Log(LOG_ERROR, "OCTO_MQTT: Connection failed!, restarting (rc=%d)", rc);
		m_bDoReconnect = true;
	}
}
...
void COctoPrintMQTT::on_disconnect(int rc)
{
	if (rc != 0)
	{
		if (!IsStopRequested(0))
		{
			if (rc == 5)
			{
				Log(LOG_ERROR, "OCTO_MQTT: disconnected, Invalid Username/Password (rc=%d)", rc);
			}
			else
			{
				Log(LOG_ERROR, "OCTO_MQTT: disconnected, restarting (rc=%d)", rc);
			}
			m_bDoReconnect = true;
		}
	}
...
{
	m_bDoReconnect = false;
	Log(LOG_STATUS, "OCTO_MQTT: Connecting to %s:%d", m_szIPAddress.c_str(), m_usIPPort);
	...
	if (!m_CAFilename.empty()) {
		...
		{
			Log(LOG_ERROR, "OCTO_MQTT: Failed enabling TLS mode, return code: %d (CA certificate: '%s')", rc, m_CAFilename.c_str());
			return false;
		}
		Log(LOG_STATUS, "OCTO_MQTT: enabled TLS mode");
	}
	...
	if (rc != MOSQ_ERR_SUCCESS)
	{
		Log(LOG_ERROR, "OCTO_MQTT: Failed to start, return code: %d (Check IP/Port)", rc);
		m_bDoReconnect = true;
		return false;
	}
	return true;
...
void COctoPrintMQTT::Do_Work()
{
	...
	Log(LOG_STATUS, "OCTO_MQTT: Worker stopped...");
}
...
void COctoPrintMQTT::SendMessage(const std::string &Topic, const std::string &Message)
{
	...
	try {
		if (!m_IsConnected)
		{
			Log(LOG_STATUS, "OCTO_MQTT: Not Connected, failed to send message: %s", Message.c_str());
			return;
		}
	...
	{
		Log(LOG_ERROR, "OCTO_MQTT: Failed to send message: %s", Message.c_str());
	}
}
...
void COctoPrintMQTT::on_message(const struct mosquitto_message *message)
{
...
#endif
#ifdef _DEBUG
	Log(LOG_NORM, "OCTO_MQTT: Topic: %s", topic.c_str());
#else
	Debug(DEBUG_HARDWARE, "OCTO_MQTT: Topic: %s", topic.c_str());
#endif
	if (qMessage.empty())
		return;
	...
	if (bIsJSON)
	{
		...
		try {
			...
			{
				Log(LOG_ERROR, "OCTO_MQTT: Invalid data received!");
				return;
			}

			if (root["_timestamp"].empty())
			{
				Log(LOG_ERROR, "OCTO_MQTT: Invalid data received! (no _timestamp field in JSON payload ?)");
				return;
			}
			...
			if (szMsgType == "temperature")
			{
				if (strarray.size() < 3)
				{
					Log(LOG_ERROR, "OCTO_MQTT: Invalid temperature received!");
					return;
				}
				if (root["actual"].empty())
				{
					Log(LOG_ERROR, "OCTO_MQTT: Invalid temperature data received! (no actual field in JSON payload ?)");
					return;
				}
			...
			else if (szMsgType == "progress")
			{
				if (strarray.size() < 3)
				{
					Log(LOG_ERROR, "OCTO_MQTT: Invalid event received!");
					return;
				}
				...
				if (szProgrssName == "printing")
				{
					...
					{
						Log(LOG_ERROR, "OCTO_MQTT: Invalid progress data received! (no progress field in JSON payload ?)");
						return;
					}
			...
			else if (szMsgType == "event")
			{
				...
				if (szEventName != m_szLastEventName)
				{
					...
					else if (szEventName == "ZChange")
					{
						...
						if (root["new"].empty())
						{
							Log(LOG_ERROR, "OCTO_MQTT: Invalid ZChange data received! (no new field in JSON payload ?)");
							return;
						}
		...
		{
			Log(LOG_ERROR, "OCTO_MQTT: Error parsing message!!! (JSON Payload)");
			return;
		}
	}
	else {
