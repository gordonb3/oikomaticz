...
void CRFLinkSerial::Do_Work()
{
	...
	while (!IsStopRequested(200))
	{
		...
		if (msec_counter == 5)
		{
			...
			if (isOpen())
			{
				...
				Log(LOG_STATUS, "RFLink: t1=%d t2=%d t3=%d", m_LastHeartbeat, m_LastHeartbeatReceive,
				m_LastReceivedTime);

				if (atime - m_LastReceivedTime > 25) {
				//Log(LOG_STATUS, "RFLink: ping...");
				write("10;PING;\n");
				} else
				if (atime - m_LastReceivedTime > 50) {
				//Timeout
				Log(LOG_ERROR, "RFLink: Nothing received for more than 50 seconds, restarting...");
				m_retrycntr = 0;
				...
				*/
				...
				if (sec_counter % 50 == 0)
				{
					time_t atime = mytime(nullptr);
					//Send ping (keep alive)
					//Log(LOG_STATUS, "RFLink: t1=%d t3=%d", atime, m_LastReceivedTime);
					if (difftime(atime,m_LastReceivedTime) > 50) {
						//Receive Timeout
						//Log(LOG_STATUS, "RFLink: ping50...");
						write("10;PING;\n");
						m_retrycntr = 0;
						m_LastReceivedTime = atime;
					} else {
						if (difftime(atime,m_LastReceivedTime) > 25) {
						   //Log(LOG_STATUS, "RFLink: ping25...");
						   write("10;PING;\n");
						}
						//else {
							//Log(LOG_STATUS, "RFLink: ping0...");
						//}
					}
				}
			}
		...
		if (!isOpen())
		{
			if (m_retrycntr==0)
			{
				Log(LOG_STATUS,"RFLink: serial retrying in %d seconds...", RFLINK_RETRY_DELAY);
			}
			m_retrycntr++;
	...
	Log(LOG_STATUS,"RFLink: Worker stopped...");
}
...
bool CRFLinkSerial::OpenSerialDevice()
{
	...
	try
	{
		...
		Log(LOG_STATUS,"RFLink: Using serial port: %s", m_szSerialPort.c_str());
	}
	catch (boost::exception & e)
	{
		Log(LOG_ERROR,"RFLink: Error opening serial port!");
#ifdef _DEBUG
