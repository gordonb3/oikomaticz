...
void CYouLess::Do_Work()
{
	...
	Log(LOG_STATUS, "YouLess: Worker started...");
	...
	Log(LOG_STATUS,"YouLess: Worker stopped...");
}
...
bool CYouLess::GetP1Details()
{
	...
	{
		Log(LOG_ERROR, "YouLess: Error getting meter details from %s !", m_szIPAddress.c_str() );
		return false;
	}
	Json::Value root;
...
void CYouLess::GetMeterDetails()
{
	...
	if (!HTTPClient::GET(szURL.str(), sResult))
	{
		Log(LOG_ERROR,"YouLess: Error connecting to: %s", m_szIPAddress.c_str());
		return;
	}
