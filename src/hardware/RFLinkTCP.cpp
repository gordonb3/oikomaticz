...
{
	Log(LOG_STATUS,"RFLink: connected to: %s:%d", m_szIPAddress.c_str(), m_usIPPort);
	m_bDoRestart=false;
	m_bIsStarted=true;
	m_rfbufferpos = 0;
...
void CRFLinkTCP::OnDisconnect()
{
	...
	Log(LOG_STATUS,"RFLink: disconnected");
}
...
void CRFLinkTCP::Do_Work()
{
	...
	int sec_counter = 0;
	Log(LOG_STATUS, "RFLink: trying to connect to %s:%d", m_szIPAddress.c_str(), m_usIPPort);
	...
	while (!IsStopRequested(1000))
	{
		...
		if ((sec_counter % 20 == 0) && (isConnected()))
		{
			...
			{
				//Timeout
				Log(LOG_ERROR, "RFLink: Nothing received for more than 30 seconds, restarting...");
				m_retrycntr = 0;
	...
	Log(LOG_STATUS,"RFLink: TCP/IP Worker stopped...");
}
...
void CRFLinkTCP::OnError(const boost::system::error_code& error)
{
	...
	{
		Log(LOG_ERROR, "RFLink: Can not connect to: %s:%d", m_szIPAddress.c_str(), m_usIPPort);
	}
	else if (
	...
	{
		Log(LOG_STATUS, "RFLink: Connection reset!");
	}
	else
		Log(LOG_ERROR, "RFLink: %s", error.message().c_str());
}
