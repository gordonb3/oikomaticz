		...
		if (m_saved_state[i].direction == GPIO_IN)
		{
			...
			if (createNewDevice)
			{
				...
				m_sql.safe_query(
					...
					m_HwdID, deviceid[0].c_str(), m_saved_state[i].pin_number, pTypeLighting2, sTypeAC, int(device::tswitch::type::Contact), "Input", m_saved_state[i].value);
			}
		}
		else
		{
			...
			if (createNewDevice)
			{
				...
				m_sql.safe_query(
					...
					m_HwdID, deviceid[0].c_str(), m_saved_state[i].pin_number, pTypeLighting2, sTypeAC, int(device::tswitch::type::OnOff), "Output", m_saved_state[i].value);
			}
		}
	}
}
