		...
		if (s.direction == GPIO_IN)
		{
			...
			if (createNewDevice)
			{
				...
				m_sql.safe_query("INSERT INTO DeviceStatus (HardwareID, OrgHardwareID, DeviceID, Unit, Type, SubType, SwitchType, Used, "
					...
					m_HwdID, 0, deviceid[0].c_str(), s.unit, pTypeLighting2, sTypeAC, int(device::tswitch::type::Contact), "Input", s.value);
			}
		}
		else
		{
			...
			if (createNewDevice)
			{
				...
				m_sql.safe_query("INSERT INTO DeviceStatus (HardwareID, OrgHardwareID, DeviceID, Unit, Type, SubType, SwitchType, Used, "
					...
					m_HwdID, 0, deviceid[0].c_str(), s.unit, pTypeLighting2, sTypeAC, int(device::tswitch::type::OnOff), "Output", s.value);
			}
		}
	}
}
