		...
		if (s.direction == GPIO_IN)
		{
			...
			if (createNewDevice)
			{
				...
				m_sql.safe_query("INSERT INTO DeviceStatus (HardwareID, DeviceID, Unit, Type, SubType, SwitchType, Used, "
						 ...
						 m_HwdID, deviceid[0].c_str(), s.pin_number, pTypeLighting2, sTypeAC, int(device::tswitch::type::Contact), "Input", s.value);
			}
		}
		else
		{
			...
			if (createNewDevice)
			{
				...
				m_sql.safe_query("INSERT INTO DeviceStatus (HardwareID, DeviceID, Unit, Type, SubType, SwitchType, Used, "
						 ...
						 m_HwdID, deviceid[0].c_str(), s.pin_number, pTypeLighting2, sTypeAC, int(device::tswitch::type::OnOff), "Output", s.value);
			}
		}
	}
}
