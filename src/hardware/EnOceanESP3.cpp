		...
		case RORG_4BS: // 4 byte communication
			{
				...
				if ((DATA_BYTE0 & RORG_4BS_TEACHIN_LRN_BIT) == 0)	// LRN_BIT is 0 -> Teach-in datagram
				{
					...
					if ((DATA_BYTE0 & RORG_4BS_TEACHIN_EEP_BIT) == 0)
					{
						...
						Log(LOG_NORM, "EnOcean: 4BS, Variant 1 Teach-in diagram: Sender_ID: %08X", iNodeID);
					...
					else
					{
						...
						Log(LOG_NORM,"EnOcean: 4BS, Variant 2 Teach-in diagram: Sender_ID: %08X\nManufacturer: %03X (%s)\nProfile: %02X\nType: %02X (%s)",
					...
					if (result.empty())
					{
						...
						Log(LOG_NORM, "EnOcean: Sender_ID %08X inserted in the database", iNodeID);
					}
					else
						Log(LOG_NORM, "EnOcean: Sender_ID %08X already in the database", iNodeID);
					ReloadVLDNodes();
				}
				else	// RORG_4BS_TEACHIN_LRN_BIT is 1 -> Data datagram
				{
					...
					if (result.empty())
					{
						Log(LOG_NORM, "EnOcean: Need Teach-In for %s", nodeID.c_str());
						return;
					}
		...
		case RORG_RPS: // repeated switch communication
			{
				...
				else
				{
					...
					{
#ifdef ENOCEAN_BUTTON_DEBUG
						Log(LOG_STATUS,"EnOcean: %s, ignore button press", nodeID.c_str());
#endif // ENOCEAN_BUTTON_DEBUG
						break;
					}
				}
...
#ifdef ENOCEAN_BUTTON_DEBUG
							Log(LOG_NORM, "EnOcean: RReceived RPS T21-Message message: %02X Node %08x ButtonID: %i Pressed: %i UD: %i",
								DATA_BYTE3,
								iNodeID,
								ButtonID,
								Pressed,
		...
		case RORG_UTE:
				// Universal teach-in (0xD4)
				{
					...
					if(cmd == 0x0)
					{
						...
						Log(LOG_NORM, "EnOcean: teach-in request received from %08X (manufacturer: %03X). number of channels: %d, device profile: %02X-%02X-%02X", iNodeID, manID, nb_channel, rorg,func,type);
