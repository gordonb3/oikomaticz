...
bool CEnOceanESP3::ParseData()
{
	...
	Log(LOG_STATUS,"EnOcean: %s",sstr.str().c_str());
#endif
	...
	if (m_ReceivedPacketType==PACKET_RESPONSE)
	{
		...
		if (ResponseCode!=0)
		{
			...
			Log(LOG_ERROR,"EnOcean: Response Error (Code: %d, %s)",ResponseCode,szError.c_str());
			return false;
		}
		if ((m_bBaseIDRequested)&&(m_bufferpos==6))
		{
			...
			Log(LOG_STATUS,"EnOcean: Transceiver ID_Base: 0x%08lx",m_id_base);
		}
		if (m_bufferpos==33)
		{
			//Version Information
			Log(LOG_STATUS,"EnOcean: Version_Info, App: %02x.%02x.%02x.%02x, API: %02x.%02x.%02x.%02x, ChipID: %02x.%02x.%02x.%02x, ChipVersion: %02x.%02x.%02x.%02x, Description: %s",
		...
		default:
			{
				...
				if (pszHumenTxt)
				{
					Log(LOG_NORM, "EnOcean: %s", pszHumenTxt);
					free(pszHumenTxt);
				}
			}
			break;
...
void CEnOceanESP3::ParseRadioDatagram()
{
	...
	switch (m_buffer[0])
	{
		...
		case RORG_4BS: // 4 byte communication
			{
				...
				if ((DATA_BYTE0 & RORG_4BS_TEACHIN_LRN_BIT) == 0)	// LRN_BIT is 0 -> Teach-in datagram
				{
					...
					if ((DATA_BYTE0 & RORG_4BS_TEACHIN_EEP_BIT) == 0)
					{
						...
						Log(LOG_NORM, "EnOcean: 4BS, Variant 1 Teach-in diagram: Sender_ID: 0x%08lX", id);
					...
					else
					{
						...
						Log(LOG_NORM,"EnOcean: 4BS, Variant 2 Teach-in diagram: Sender_ID: 0x%08lX\nManufacturer: 0x%02x (%s)\nProfile: 0x%02X\nType: 0x%02X (%s)",
					...
					if (result.empty())
					{
						...
						Log(LOG_NORM, "EnOcean: Sender_ID 0x%08lX inserted in the database", id);
					}
					else
						Log(LOG_NORM, "EnOcean: Sender_ID 0x%08lX already in the database", id);
					ReloadVLDNodes();
				}
				else	// RORG_4BS_TEACHIN_LRN_BIT is 1 -> Data datagram
				{
					...
					if (result.empty())
					{
						Log(LOG_NORM, "EnOcean: Need Teach-In for %s", szDeviceID);
						return;
					}
		...
		case RORG_RPS: // repeated switch communication
			{
				...
				else
				{
					...
					{
#ifdef ENOCEAN_BUTTON_DEBUG
						Log(LOG_STATUS,"EnOcean: %s, ignore button press", szDeviceID);
#endif // ENOCEAN_BUTTON_DEBUG
						break;
					}
				}
...
#ifdef ENOCEAN_BUTTON_DEBUG
							Log(LOG_NORM, "EnOcean: Received RPS T21-Message message: 0x%02X Node 0x%08x ButtonID: %i Pressed: %i UD: %i",
								DATA_BYTE3,
								id,
								ButtonID,
								Pressed,
		...
		case RORG_UTI:
				// Universal teach-in (0xD4)
				{
					...
					if(cmd == 0x0)
					{
						...
						Log(LOG_NORM, "EnOcean: teach-in request received from %08lX (manufacturer: %03X). number of channels: %d, device profile: %02X-%02X-%02X", id, manID, nb_channel, rorg,func,type);
						...
						if((rorg == 0xD2) && (func == 0x01) && ( (type == 0x12) || (type == 0x0F) ))
						{
							...
							for(nbc = 0; nbc < nb_channel; nbc ++)
							{
								...
								Log(LOG_NORM, "EnOcean: channel = %d", nbc+1);
					...
					Log(LOG_NORM, "EnOcean: Unhandled RORG (%02x), uni_bi (%02x [1=bidir]), response_expected (%02x [0=yes]), request (%02x), cmd (%02x)", m_buffer[0], uni_bi_directional_communication,eep_teach_in_response_expected, teach_in_request, cmd);
				}
			break;
		...
		case RORG_VLD:
			...
			break;
		default:
			Log(LOG_NORM, "EnOcean: Unhandled RORG (%02x)", m_buffer[0]);
			break;
	}
}
