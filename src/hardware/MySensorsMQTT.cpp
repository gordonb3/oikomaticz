	...
	, MyTopicOut(TOPIC_OUT)
{
	...
	do {
		...
		if (std::string::npos == nextPiece)
		{
			...
			Log(LOG_ERROR, "MySensorsMQTT: Truncating CAfilename; Stray topic was present.");
			break;
		}
...
void MySensorsMQTT::on_message(const struct mosquitto_message *message)
{
	...
	Log(LOG_NORM, "MySensorsMQTT: Topic: %s, Message: %s", topic.c_str(), qMessage.c_str());
...
void MySensorsMQTT::on_connect(int rc)
{
	...
	if (m_IsConnected)
	{
		Log(LOG_STATUS, "MySensorsMQTT: connected to: %s:%d", m_szIPAddress.c_str(), m_usIPPort);
