...
{
	Log(LOG_STATUS, "Satel Integra: Create instance");
	m_HwdID = ID;
	...
	if (userCode.empty())
	{
		Log(LOG_STATUS, "Satel Integra: no user code was entered (arm/disarm/switching will not work)");
	}
...
{
	Log(LOG_STATUS, "Satel Integra: Destroy instance");
}
...
bool SatelIntegra::StartHardware()
{
	...
	Log(LOG_STATUS, "Satel Integra: Start hardware");
#endif
...
void SatelIntegra::Do_Work()
{
	...
	Log(LOG_STATUS, "Satel Integra: worker started");
	...
	while (!IsStopRequested(interval))
	{
		...
		if (msec_poll_counter >= m_pollInterval)
		{
			msec_poll_counter = 0;
#ifdef DEBUG_SatelIntegra
			Log(LOG_STATUS, "Satel Integra: fetching changed data");
#endif
		...
		if (msec_temp_counter >= SATEL_TEMP_POLL_INTERVAL_MS)
		{
			msec_temp_counter = 0;
#ifdef DEBUG_SatelIntegra
			Log(LOG_STATUS, "Satel Integra: fetching temperature");
#endif
			ReadTemperatures();
		}
	}//while (!IsStopRequested())
	DestroySocket();
	Log(LOG_STATUS, "Satel Integra: worker stopped");
}
...
bool SatelIntegra::CheckAddress()
{
	...
	{
		Log(LOG_ERROR, "Satel Integra: Empty IP Address or bad Port");
		return false;
	}
	...
	else
	{
		...
		if (he == nullptr)
		{
			Log(LOG_ERROR, "Satel Integra: cannot resolve host name");
			return false;
		}
...
bool SatelIntegra::ConnectToIntegra()
{
	...
	if (m_socket == INVALID_SOCKET)
	{
		Log(LOG_ERROR, "Satel Integra: Unable to create socket");
		return false;
	}

	if (connect(m_socket, (const sockaddr*)&m_addr, sizeof(m_addr)) == SOCKET_ERROR)
	{
		Log(LOG_ERROR, "Satel Integra: Unable to connect to specified IP Address on specified Port (%s:%d)", m_IPAddress.c_str(), m_IPPort);
		DestroySocket();
		return false;
	}
#if defined WIN32
...
#endif
	Log(LOG_STATUS, "Satel Integra: connected to %s:%d", m_IPAddress.c_str(), m_IPPort);
...
void SatelIntegra::DestroySocket()
{
	...
	{
#ifdef DEBUG_SatelIntegra
		Log(LOG_STATUS, "Satel Integra: destroy socket");
#endif
...
bool SatelIntegra::ReadNewData()
{
	...
	Log(LOG_ERROR, "Satel Integra: Get info about new data is failed");
	return false;
}
...
bool SatelIntegra::GetInfo()
{
	...
	if (SendCommand(cmd, 1, buffer, 15) > 0)
	{
		...
		if (m_modelIndex > -1)
		{
			Log(LOG_STATUS, "Satel Integra: Model %s", models[m_modelIndex].name);

			unsigned char cmd[1];
			cmd[0] = 0x1A; // RTC
			if (SendCommand(cmd, 1, buffer, 10) > 0)
			{
				Log(LOG_STATUS, "Satel Integra: RTC %.2x%.2x-%.2x-%.2x %.2x:%.2x:%.2x",
				...
				if (SendCommand(cmd, 1, buffer, 13) > 0)
				{
					...
					Log(LOG_STATUS, "Satel Integra: ETHM-1 ver. %c.%c%c %c%c%c%c-%c%c-%c%c (32 bytes mode = %s)",
						buffer[1], buffer[2], buffer[3], buffer[4], buffer[5], buffer[6], buffer[7], buffer[8], buffer[9], buffer[10], buffer[11], m_data32 ? "true" : "false");
					return true;
				}
				Log(LOG_ERROR, "Satel Integra: unknown version of ETHM-1");
			}
			else
			{
				Log(LOG_ERROR, "Satel Integra: Unknown basic status");
			}
		}
		else
		{
			Log(LOG_ERROR, "Satel Integra: Unknown model '%02x'", buffer[0]);
		}
	}
	else
	{
		Log(LOG_ERROR, "Satel Integra: Get info about Integra is failed");
	}
...
bool SatelIntegra::ReadZonesState(const bool firstTime)
{
...
#ifdef DEBUG_SatelIntegra
	Log(LOG_STATUS, "Satel Integra: Read zones states");
#endif
		...
		for (unsigned int index = 0; index < zonesCount; ++index)
		{
			if (!IsStopRequested(0))
			{
				...
				if (firstTime)
				{
...
#ifdef DEBUG_SatelIntegra
					Log(LOG_STATUS, "Satel Integra: Reading zone %d name", index + 1);
#endif
					unsigned char cmd[3];
					cmd[0] = 0xEE;
					cmd[1] = 0x05;
					...
					{
						Log(LOG_ERROR, "Satel Integra: Receive info about zone %d failed (probably zone is not used)", index + 1);
					}
				}
	...
	{
		Log(LOG_ERROR, "Satel Integra: Send 'Read Outputs' failed");
		return false;
	}
...
bool SatelIntegra::ReadTemperatures(const bool firstTime)
{
...
#ifdef DEBUG_SatelIntegra
	Log(LOG_STATUS, "Satel Integra: Read zones temperatures");
#endif
	...
	for (unsigned int index = 0; index < zonesCount; ++index)
		...
		{
#ifdef DEBUG_SatelIntegra
			Log(LOG_STATUS, "Satel Integra: Reading zone %d temperature", index + 1);
#endif
			unsigned char cmd[2];
			...
			if (SendCommand(cmd, 2, buffer, 4) > 0)
			{
				...
				if (temp != 0 && temp != 65535) // -55oC and undetermined
				{
					if (firstTime)
					{
						...
						unsigned char buffer[21];
#ifdef DEBUG_SatelIntegra
						Log(LOG_STATUS, "Satel Integra: Reading temperature zone %d name", index + 1);
#endif
						unsigned char cmd[3];
						cmd[0] = 0xEE;
						cmd[1] = 0x05;
						...
						{
							Log(LOG_ERROR, "Satel Integra: Receive info about zone %d failed", index + 1);
						}
					}
					else
					{
			...
			{
				Log(LOG_ERROR, "Satel Integra: Send 'Read Temperature' failed");
				return false;
			}
		}
...
#ifdef DEBUG_SatelIntegra
	Log(LOG_STATUS, "Satel Integra: Read outputs states");
#endif
	unsigned char buffer[33];
	...
	if (SendCommand(cmd, 1 + m_data32, buffer, 17 + m_data32 * 16) > 0)
	{
		...
		for (unsigned int index = 0; index < outputsCount; ++index)
		{
			if (!IsStopRequested(0))
			{
				...
				{
#ifdef DEBUG_SatelIntegra
					Log(LOG_STATUS, "Satel Integra: Reading output %d name", index + 1);
#endif
					unsigned char buffer[21];
					unsigned char cmd[3];
					cmd[0] = 0xEE;
						...
						if (buffer[3] != 0x00)
						{
							...
							if ((buffer[3] == 105) || (buffer[3] == 106)) // roller blind up and down
							{
								if (findBlindOutput == false)
								{
									...
									UpdateOutputName(index + 1 + 1024, &buffer[4], device::tswitch::type::Blinds);
								}
								findBlindOutput = !findBlindOutput;
							}

							ReportOutputState(index + 1, outputState);
							UpdateOutputName(index + 1, &buffer[4], m_isOutputSwitch[index] ? device::tswitch::type::OnOff : device::tswitch::type::Contact);
						}
						else
						{
#ifdef DEBUG_SatelIntegra
							Log(LOG_STATUS, "Satel Integra: output %d is not used", index + 1);
#endif
						}
					}
					else
					{
						Log(LOG_ERROR, "Satel Integra: Receive info about output %d failed", index);
					}
				}
...
{
#ifdef DEBUG_SatelIntegra
	Log(LOG_STATUS, "Satel Integra: Read arm state");
#endif
	...
	if (SendCommand(cmd, 1, buffer, 5) > 0)
	{
		for (unsigned int index = 0; index < 32; ++index)
		{
			...
			if ((firstTime || (m_armLastState[index] != armed)) && (m_isPartitions[index]))
			{
				if (armed)
				{
					Log(LOG_STATUS, "Satel Integra: partition %d arm", index + 1);
				}
				else
				{
					Log(LOG_STATUS, "Satel Integra: partition %d not arm", index + 1);
				}
	...
	{
		Log(LOG_ERROR, "Satel Integra: Send 'Get Armed partitions' failed");
		return false;
	}
...
{
#ifdef DEBUG_SatelIntegra
	Log(LOG_STATUS, "Satel Integra: Read partitions alarms");
#endif
	unsigned char buffer[5];
	...
	if (SendCommand(cmd, 1, buffer, 5) > 0)
	{
		...
		if (firstTime || (m_alarmLast != alarm))
		{
			if (alarm)
			{
				Log(LOG_STATUS, "Satel Integra: ALARM !!");
			}
			else
			{
				Log(LOG_STATUS, "Satel Integra: Alarm not set");
			}
	...
	{
		Log(LOG_ERROR, "Satel Integra: Send 'Get Alarm partitions' failed");
		return false;
	}
...
{
#ifdef DEBUG_SatelIntegra
	Log(LOG_STATUS, "Satel Integra: Read events");
#endif
	unsigned char buffer[15];
...
{
#ifdef DEBUG_SatelIntegra
	Log(LOG_STATUS, "Satel Integra: arming partition %d", partition);
#endif
	if (mode > 3)
	{
		Log(LOG_ERROR, "Satel Integra: incorrect arm mode %d", mode);
		return false;
	}
	...
	{
		Log(LOG_ERROR, "Satel Integra: Send 'Arm partition %d' failed", partition);
		return false;
	}

	m_armLastState[partition - 1] = true;
	Log(LOG_STATUS, "Satel Integra: Partition %d armed", partition);
	return true;
}
...
{
#ifdef DEBUG_SatelIntegra
	Log(LOG_STATUS, "Satel Integra: disarming partition %d", partition);
#endif
	...
	{
		Log(LOG_ERROR, "Satel Integra: Send 'Disarm partition %d' failed", partition);
		return false;
	}

	m_armLastState[partition - 1] = false;
	Log(LOG_STATUS, "Satel Integra: Partition %d disarmed", partition);
	return true;
}
	...
	if (output->ICMND.packettype == pTypeGeneralSwitch && output->ICMND.subtype == sSwitchTypeAC)
	{
		...
		if (general->unitcode == 1) // outputs
		{
			...
			if (SendCommand(cmd, 41, buffer, 1) != -1)
			{
				...
				Log(LOG_STATUS, "Satel Integra: switched output %d to %s", id, cmnd == gswitch_sOn ? "on" : "off");
				return true;
			}
			Log(LOG_ERROR, "Satel Integra: Switch output %d failed", id);
			return false;
		}
	}
...
void SatelIntegra::UpdateZoneName(const int Idx, const unsigned char* name, const int partition)
{
	...
	if (result.empty())
	{
...
#ifdef DEBUG_SatelIntegra
		Log(LOG_STATUS, "Satel Integra: update name for %d to '%s:%s'", Idx, namePrefix.c_str(), shortName.c_str());
#endif
		m_sql.safe_query("UPDATE DeviceStatus SET Name='%q:%q', SwitchType=%d, Unit=%d WHERE (HardwareID==%d) AND (DeviceID=='%q') AND (Unit=1)", namePrefix.c_str(), shortName.c_str(),
				 device::tswitch::type::Contact, partition, m_HwdID, sTmp.c_str());
	}
}
...
void SatelIntegra::UpdateTempName(const int Idx, const unsigned char* name, const int partition)
{
	...
	if (result.empty())
	{
		//Assign zone name from Integra
#ifdef DEBUG_SatelIntegra
		Log(LOG_STATUS, "Satel Integra: update name for %d to 'Temp:%s'", Idx, shortName.c_str());
#endif
		m_sql.safe_query("UPDATE DeviceStatus SET Name='Temp:%q', SwitchType=%d, Unit=%d WHERE (HardwareID==%d) AND (DeviceID=='%q') AND (Unit=0)", shortName.c_str(), device::tswitch::type::Contact, partition,
				 m_HwdID, sTmp.c_str());
	}
}

void SatelIntegra::UpdateOutputName(const int Idx, const unsigned char* name, const device::tswitch::type::value switchType)
{
	std::vector<std::vector<std::string> > result;
...
void SatelIntegra::UpdateAlarmAndArmName()
{
...
#ifdef DEBUG_SatelIntegra
		Log(LOG_STATUS, "Satel Integra: update Alarm name to 'Alarm'");
#endif
	...
	for (size_t i = 0; m_isPartitions.size(); ++i)
	{
		if (m_isPartitions[i])
		{
			std::string sTmp = std_format("%08X", i + 1);
			...
			if (result.empty())
			{
				//Assign name for Arm
#ifdef DEBUG_SatelIntegra
				Log(LOG_STATUS, "Satel Integra: update Arm name to 'Arm %d partition'", i + 1);
#endif
...
int SatelIntegra::SendCommand(const unsigned char* cmd, const unsigned int cmdLength, unsigned char *answer, const int expectedLength)
{
	...
	{
		Log(LOG_ERROR, "Satel Integra: Send command '%02X' failed", cmdPayload.first[2]);
		DestroySocket();
	...
	{
		Log(LOG_ERROR, "Satel Integra: connection lost.");
		DestroySocket();
		return -1;
	}
	...
	do
	{
		...
		{
			Log(LOG_ERROR, "Satel Integra: bad data length received (totalRet %d)", totalRet);
			DestroySocket();
			return -1;
		}
	...
	if (totalRet > 6)
	{
		if (buffer[0] == 0xFE && buffer[1] == 0xFE && buffer[totalRet - 1] == 0x0D && buffer[totalRet - 2] == 0xFE) // check prefix and sufix
		{
			...
			{
				Log(LOG_ERROR, "Satel Integra: bad data length received (expectedLength %d, totalRet %d, cmd %X, cmdLength %d)", expectedLength, totalRet - 6, cmd[0], cmdLength);
				DestroySocket();
				return -1;
			}
			...
			if ((crc & 0xFF) == buffer[totalRet - 3] && (crc >> 8) == buffer[totalRet - 4]) // check crc
			{
				if (buffer[2] == 0xEF)
				{
					...
					Log(LOG_ERROR, "Satel Integra: receive error: %s", error);
					return -1;
				}
				return answerLength;
			}
			Log(LOG_ERROR, "Satel Integra: receive bad CRC");
			return -1;
		}
		if (buffer[0] == 16)
		{
			Log(LOG_ERROR, "Satel Integra: busy");
			return -1;
		}
		Log(LOG_ERROR, "Satel Integra: received bad frame (prefix or sufix)");
		return -1;
	}
	Log(LOG_ERROR, "Satel Integra: received frame is too short.");
	DestroySocket();
	return -1;
}
