					...
					else if (name == NAME_GATEWAY)
					{
						...
						if (rgb != "") {
							...
							if (TrueGateway->GetGatewaySid() == sid)
							{
								...
								TrueGateway->InsertUpdateSwitch(sid.c_str(), NAME_GATEWAY_SOUND_ALARM_RINGTONE, false, device::tswitch::type::Selector, 3, 0, cmd, "", "", 255);
								TrueGateway->InsertUpdateSwitch(sid.c_str(), NAME_GATEWAY_SOUND_ALARM_CLOCK, false, device::tswitch::type::Selector, 4, 0, cmd, "", "", 255);
								TrueGateway->InsertUpdateSwitch(sid.c_str(), NAME_GATEWAY_SOUND_DOORBELL, false, device::tswitch::type::Selector, 5, 0, cmd, "", "", 255);
								TrueGateway->InsertUpdateSwitch(sid.c_str(), NAME_GATEWAY_SOUND_MP3, false, device::tswitch::type::OnOff, 6, 0, cmd, "", "", 255);
								TrueGateway->InsertUpdateSwitch(sid.c_str(), NAME_GATEWAY_SOUND_VOLUME_CONTROL, false, device::tswitch::type::Dimmer, 7, 0, cmd, "", "", 255);
							}
						}
						else {
							// Check for token
