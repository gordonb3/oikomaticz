...
typedef enum
{
	...
	ACT_RELAIS_CHANNEL_0,		    // 11, Xiaomi Relais Channel 0
	ACT_RELAIS_CHANNEL_1,		    // 12, Xiaomi Relais Channel 1
	ACT_LIGHT			    // 13, Xiaomi Light bulb
} XiaomiUnitCode;
...
#define MODEL_SELECTOR_WIRELESS_WALL_SINGLE_3 "remote.b186acn02"
...
#define NAME_ACT_RELAIS_CHANNEL_1 "Aqara Wireless Relay Controller - Channel 2"
...
int XiaomiGateway::get_local_ipaddr(std::vector<std::string> &ip_addrs)
{
	...
	{
		Log(LOG_ERROR, "getifaddrs failed! (when trying to determine local ip address)");
		perror("getifaddrs");
		return 0;
	}
	...
	for (ifa = myaddrs; ifa != nullptr; ifa = ifa->ifa_next)
	{
		...
		{
			Log(LOG_ERROR, "Could not convert to IP address, inet_ntop failed for interface %s", ifa->ifa_name);
		}
		else
		{
...
bool XiaomiGateway::WriteToHardware(const char *pdata, const unsigned char length)
{
	...
	if (packettype == pTypeGeneralSwitch)
	{
		...
		if (xcmd->unitcode == XiaomiUnitCode::SELECTOR_WIRED_WALL_SINGLE || xcmd->unitcode == XiaomiUnitCode::SELECTOR_WIRED_WALL_DUAL_CHANNEL_0 ||
		    ...
		    xcmd->unitcode == XiaomiUnitCode::ACT_RELAIS_CHANNEL_0 || xcmd->unitcode == XiaomiUnitCode::ACT_RELAIS_CHANNEL_1 || xcmd->unitcode == XiaomiUnitCode::ACT_LIGHT)
		{
...
void XiaomiGateway::InsertUpdateRGBGateway(const std::string &nodeid, const std::string &Name, const bool bIsOn, const int brightness, const int hue)
{
	...
	bool tIsOn = !(bIsOn);
	std::vector<std::vector<std::string>> result;
	result = m_sql.safe_query("SELECT nValue, LastLevel FROM DeviceStatus WHERE (HardwareID==%d) AND (DeviceID=='%q') AND (Type==%d) AND (SubType==%d)", m_HwdID, szDeviceID, pTypeColorSwitch,
				  sTypeColor_RGB_W);
	...
	if (result.empty())
	{
		...
		m_sql.safe_query("UPDATE DeviceStatus SET Name='%q', SwitchType=%d, LastLevel=%d WHERE (HardwareID == %d) AND (DeviceID == '%s') AND (Type == %d)", Name.c_str(), (device::tswitch::type::Dimmer),
...
void XiaomiGateway::InsertUpdateSwitch(const std::string &nodeid, const std::string &Name, const bool bIsOn, const device::tswitch::type::value switchtype, const int unitcode, const int level,
				       const std::string &messagetype, const std::string &load_power, const std::string &power_consumed, const int battery)
{
	unsigned int sID = GetShortID(nodeid);
	...
	result = m_sql.safe_query("SELECT nValue, BatteryLevel FROM DeviceStatus WHERE (HardwareID==%d) AND (DeviceID=='%q') AND (Type==%d) AND (Unit == '%d')", m_HwdID, ID.c_str(), xcmd.type,
				  xcmd.unitcode);
	if (result.empty())
	{
		...
		if (switchtype == device::tswitch::type::Selector)
		{
			result = m_sql.safe_query("SELECT ID FROM DeviceStatus WHERE (HardwareID==%d) AND (DeviceID=='%q') AND (Type==%d) AND (Unit == '%d')", m_HwdID, ID.c_str(), xcmd.type,
						  xcmd.unitcode);
			if (!result.empty())
			{
		...
		else if (switchtype == device::tswitch::type::OnOff && Name == NAME_GATEWAY_SOUND_MP3)
		{
	...
	else
	{
		...
		{
			if ((bIsOn == false && nvalue >= 1) || (bIsOn == true) || (Name == NAME_SELECTOR_WIRED_WALL_DUAL) || (Name == NAME_SELECTOR_WIRED_WALL_SINGLE) ||
...
bool XiaomiGateway::StartHardware()
{
	...
	std::vector<std::vector<std::string>> result;
	result = m_sql.safe_query("SELECT Password, Address FROM Hardware WHERE Type=%d AND ID=%d AND Enabled=1", hardware::type::XiaomiGateway, m_HwdID);
	...
	if (m_ListenPort9898)
	{
		Log(LOG_STATUS, "XiaomiGateway (ID=%d): Selected as main Gateway", m_HwdID);
	}
...
void XiaomiGateway::Do_Work()
{
	...
	try
	{
		...
		if (found != std::string::npos)
		{
			...
			Log(LOG_STATUS, "XiaomiGateway (ID=%d): Using %s for local IP address.", m_HwdID, m_LocalIp.c_str());
		}
	}
	catch (std::exception &e)
	{
		Log(LOG_STATUS, "XiaomiGateway (ID=%d): Could not detect local IP address using Boost.Asio: %s", m_HwdID, e.what());
	}
	...
	if (m_LocalIp.empty())
	{
		try
		{
			...
			std::string compareIp = m_GatewayIp.substr(0, (m_GatewayIp.length() - 3));
			Log(LOG_STATUS, "XiaomiGateway (ID=%d): XiaomiGateway IP address starts with: %s", m_HwdID, compareIp.c_str());
			...
			{
				Log(LOG_STATUS, "XiaomiGateway (ID=%d): Could not find local IP address with ifaddrs", m_HwdID);
			}
		}
		catch (std::exception &e)
		{
			Log(LOG_STATUS, "XiaomiGateway (ID=%d): Could not find local IP address with ifaddrs: %s", m_HwdID, e.what());
		}
	}
	...
	Log(LOG_STATUS, "XiaomiGateway (ID=%d): stopped", m_HwdID);
}
	...
	if (!error || error == boost::asio::error::message_size)
	{
		...
		if ((cmd == COMMAND_REPORT) || (cmd == COMMAND_READ_ACK) || (cmd == COMMAND_HEARTBEAT))
		{
			...
			if ((ret) || (!root2.isObject()))
			{
				...
				{
					type = device::tswitch::type::OnOff;
					name = NAME_ACT_RELAIS_CHANNEL_0;
				}
					...
					model == MODEL_SELECTOR_WIRED_WALL_SINGLE_4)
				{
					...
					name = NAME_SELECTOR_WIRED_WALL_SINGLE;
					 ...
					 model == MODEL_SELECTOR_WIRED_WALL_DUAL_4)
				{
					type = device::tswitch::type::END; // type = device::tswitch::type::OnOff; // TODO: fix this hack
					name = NAME_SELECTOR_WIRED_WALL_DUAL;
				}
				else if (model == MODEL_SELECTOR_WIRELESS_WALL_SINGLE_1 || model == MODEL_SELECTOR_WIRELESS_WALL_SINGLE_2 || model == MODEL_SELECTOR_WIRELESS_WALL_SINGLE_3)
				{
					type = device::tswitch::type::Selector;
				...
				{
					type = device::tswitch::type::Dimmer;
				}
				...
				if (type != device::tswitch::type::END)
				{
					...
					std::string aqara_wireless1 = root2[NAME_CHANNEL_0].asString(); // TODO: rename var as it's not always a wireless switch
					...
					else
					{
						...
						{
							// Need to sleep here as the gateway will send 2 update messages, and need time for the database to update the
							// state so that the event is not triggered twice
							sleep_milliseconds(100); // TODO: try to reduce or remove this sleep
							TrueGateway->InsertUpdateSwitch(sid, name, on, type, unitcode, level, cmd, load_power, power_consumed, battery);
						}
						else if (model == MODEL_ACT_RELAIS)
						{
							unitcode = XiaomiUnitCode::ACT_RELAIS_CHANNEL_0;
							name = NAME_ACT_RELAIS_CHANNEL_0;
							if (!aqara_wireless1.empty())
							{
								if (aqara_wireless1 == STATE_ON)
								{
									on = true;
								}
							}
							else if (!aqara_wireless2.empty())
							{
								unitcode = XiaomiUnitCode::ACT_RELAIS_CHANNEL_1;
								name = NAME_ACT_RELAIS_CHANNEL_1;
								if (aqara_wireless2 == STATE_ON)
								{
									on = true;
								}
							}
				...
				else if ((name == NAME_SELECTOR_WIRED_WALL_SINGLE) || (name == NAME_SELECTOR_WIRED_WALL_DUAL))
				{
					...
					type = device::tswitch::type::OnOff; // TODO: Needs to be set above but need different way of executing this code without hack
				...
				else if (name == NAME_GATEWAY)
				{
					...
					if (!rgb.empty())
					{
						...
						if (TrueGateway->GetGatewaySid() == sid)
						{
							...
							TrueGateway->InsertUpdateSwitch(sid, NAME_GATEWAY_SOUND_ALARM_RINGTONE, false, device::tswitch::type::Selector, 3, 0, cmd, "", "", 255);
							TrueGateway->InsertUpdateSwitch(sid, NAME_GATEWAY_SOUND_ALARM_CLOCK, false, device::tswitch::type::Selector, 4, 0, cmd, "", "", 255);
							TrueGateway->InsertUpdateSwitch(sid, NAME_GATEWAY_SOUND_DOORBELL, false, device::tswitch::type::Selector, 5, 0, cmd, "", "", 255);
							TrueGateway->InsertUpdateSwitch(sid, NAME_GATEWAY_SOUND_MP3, false, device::tswitch::type::OnOff, 6, 0, cmd, "", "", 255);
							TrueGateway->InsertUpdateSwitch(sid, NAME_GATEWAY_SOUND_VOLUME_CONTROL, false, device::tswitch::type::Dimmer, 7, 0, cmd, "", "", 255);
						}
					}
					else
					{
		...
		{
			_log.Log(LOG_STATUS, "XiaomiGateway: %s", data_);
		}
		start_receive();
	}
	else
...
{
	std::string cmdchannel;
	if (unitcode == XiaomiUnitCode::SELECTOR_WIRED_WALL_SINGLE || unitcode == XiaomiUnitCode::ACT_ONOFF_PLUG || unitcode == XiaomiUnitCode::SELECTOR_WIRED_WALL_DUAL_CHANNEL_0 ||
	    unitcode == XiaomiUnitCode::ACT_RELAIS_CHANNEL_0)
	{
		cmdchannel = NAME_CHANNEL_0;
	}
	else if (unitcode == XiaomiUnitCode::SELECTOR_WIRED_WALL_DUAL_CHANNEL_1 || unitcode == XiaomiUnitCode::ACT_RELAIS_CHANNEL_1)
	{
...
std::string XiaomiGateway::DetermineDevice(int32_t unitcode)
{
	...
	{
		cmddevice = MODEL_SELECTOR_WIRED_WALL_SINGLE_1;
	}
	else if (unitcode == XiaomiUnitCode::ACT_ONOFF_PLUG)
	{
		cmddevice = MODEL_ACT_ONOFF_PLUG;
	}
	...
	{
		cmddevice = MODEL_SELECTOR_WIRED_WALL_DUAL_1;
	}
	else if (unitcode == XiaomiUnitCode::ACT_RELAIS_CHANNEL_0 || unitcode == XiaomiUnitCode::ACT_RELAIS_CHANNEL_1)
	{
		cmddevice = MODEL_ACT_RELAIS;
	}
	else if (unitcode == XiaomiUnitCode::ACT_LIGHT)
	{
		cmddevice = MODEL_ACT_LIGHT;
	}
	return cmddevice;
}
