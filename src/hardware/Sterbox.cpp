...
bool CSterbox::StartHardware()
{
	...
	m_bIsStarted=true;
	sOnConnected(this);
	Log(LOG_STATUS, "Sterbox: Started");
...
void CSterbox::Do_Work()
{
	...
	Log(LOG_STATUS,"Sterbox: Worker stopped...");
}
...
bool CSterbox::WriteToHardware(const char *pdata, const unsigned char length)
{
	...
	if (packettype == pTypeLighting2)
	{
		...
		//Log(LOG_ERROR, "Sterbox: Size od szhex: %s", strhex.c_str());
		...
		{
			Log(LOG_ERROR, "Sterbox: Protocol Error sending relay command to: %s", m_szIPAddress.c_str());
			return false;
		}
		std::string strhex = szURL.str();
		Log(LOG_STATUS, "Sterbox: sending relay command to: %s", strhex.c_str());
...
void CSterbox::GetMeterDetails()
{
	...
	if (!HTTPClient::GET(szURL.str(),sResult))
	{
		Log(LOG_ERROR,"Sterbox: Error connecting to: %s", m_szIPAddress.c_str());
		return;
	}
	...
	if (results.size()<8)
	{
		Log(LOG_ERROR,"Sterbox: Result 8 Error connecting to: %s", m_szIPAddress.c_str());
		return;
	}
	//if (results[0] != "<body>")
	std::string tmpstr;
	...
	else
	{
		...
		//Log(LOG_ERROR,"Sterbox: Username : %s , IP: %s", m_Username.c_str(), m_szIPAddress.c_str());
	}
	...
	for (ii = 1; ii < results.size(); ii++)
	{
		...
		if (tmpstr.find("OU") != std::string::npos)
		{
			...
			if (pos1 != std::string::npos)
			{
				...
				//Log(LOG_ERROR,"Sterbox: OU Status: %s", tmpstr.c_str());
				...
				for (jj = 0; jj < inputs.size(); jj++)
				{
					...
					//tmpstr = tmpstr.substr(pos1+1);
					tmpstr2 = outputs[jj];
					//Log(LOG_ERROR,"Sterbox: OU Status: %s", tmpstr2.c_str());
					pos1 = tmpstr2.find('s');
					if (pos1 != std::string::npos)
					{
						lValue = 1;
		...
		else if (tmpstr.find("AN") != std::string::npos)
		{
			...
			if (pos1 != std::string::npos)
			{
				...
				//Log(LOG_ERROR,"Sterbox: Pass : %s", m_Password.c_str());
