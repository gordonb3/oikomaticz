...
bool CTellstick::WriteToHardware(const char *pdata, const unsigned char length)
{
    ...
    Log(LOG_NORM, "Tellstick: WriteToHardware %d id: %d cmd: %d", pSwitch->type, pSwitch->id, pSwitch->cmnd);
...
bool CTellstick::AddSwitchIfNotExits(const int id, const char* devname, bool isDimmer)
{
	...
	if (result.empty())
    {
        Log(LOG_NORM, "Tellstick: device %d %s: %s", id, sid ,devname);
...
{
    Log(LOG_NORM, "Tellstick: sensorEvent %d,%s,%s,%d,%s", deviceId, protocol, model, dataType, value);
    switch (dataType)
    {
...
{
    Log(LOG_NORM, "Tellstick: deviceEvent %d %d: %s", deviceId, method, data);
...
{
    Log(LOG_NORM, "Tellstick: rawDeviceEvent %d: %s", controllerId, data);
...
void CTellstick::Init()
{
    ...
    for (int i = 0; i < numDevices; i++)
    {
        ...
        Log(LOG_NORM, "Tellstick: %s method %d", name, m_td.Methods(id, TELLSTICK_TURNON | TELLSTICK_TURNOFF | TELLSTICK_DIM) & TELLSTICK_DIM);
...
bool CTellstick::StartHardware()
{
    ...
    m_bIsStarted=true;
    sOnConnected(this);
    Log(LOG_NORM, "Tellstick: StartHardware");
    //Start worker thread
...
{
    Log(LOG_NORM, "Tellstick: SendCommand %d id: %d cmd: %d",
             genSwitch.type, genSwitch.id, genSwitch.cmnd);
    switch (genSwitch.cmnd)
    {
    case gswitch_sOn:
        Log(LOG_NORM, "Tellstick: Switch ON");
        m_td.TurnOn(genSwitch.id);
        break;
    case gswitch_sOff:
        Log(LOG_NORM, "Tellstick: Switch OFF");
        m_td.TurnOff(genSwitch.id);
        break;
    case gswitch_sSetLevel:
        Log(LOG_NORM, "Tellstick: Dim level %d %d",
