...
{
	int LastMinute = -1;
	Log(LOG_STATUS, "PVOutput (Input): Worker started...");
	...
	Log(LOG_STATUS, "PVOutput (Input): Worker stopped...");
}
...
void CPVOutputInput::GetMeterDetails()
{
	...
	{
		Log(LOG_ERROR, "PVOutput (Input): Error login!");
		return;
	}
	...
	{
		Log(LOG_ERROR, "PVOutput (Input): Invalid Data received!");
		return;
	}
	/*
