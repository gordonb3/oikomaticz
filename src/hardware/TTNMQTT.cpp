...
bool CTTNMQTT::StartHardware()
{
	...
	// Let's get the 'home' Location of this Oikomaticz instance from the Preferences
...
void CTTNMQTT::on_message(const struct mosquitto_message *message)
{
...
#ifdef DEBUG_TTN_R
	Log(LOG_NORM, "TTN_MQTT: Topic: %s", topic.c_str());
	Log(LOG_NORM, "TTN_MQTT: qmsg: %s", qMessage.c_str());
#endif
	...
	try {
		...
		if(iGps == 1 || !(devlat == 0 || devlon == 0))
		{
			...
			Debug(DEBUG_RECEIVED, "Distance between Sensordevice and Oikomaticz Home is %f meters!", (double)nSsrDistance);
		}
