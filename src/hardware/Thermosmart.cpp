...
CThermosmart::CThermosmart(const int ID, const std::string &Username, const std::string &Password, const int Mode1, const int Mode2, const int Mode3, const int Mode4, const int Mode5, const int Mode6)
{
	...
	{
		Log(LOG_ERROR, "Thermosmart: Please update your username/password!...");
	}
	else
	{
		m_UserName = Username;
...
{
	Log(LOG_STATUS,"Thermosmart: Worker started...");
	...
	Log(LOG_STATUS,"Thermosmart: Worker stopped...");
}
...
bool CThermosmart::Login()
{
	...
	if (!HTTPClient::POST(sURL, szPostdata, ExtraHeaders, sResult))
		{
			Log(LOG_ERROR,"Thermosmart: Error login!");
			return false;
		}
	...
	{
		Log(LOG_ERROR, "Thermosmart: Error login!, check username/password");
		return false;
	}
	...
	{
		Log(LOG_ERROR, "Thermosmart: Invalid/no data received...");
		return false;
	}

	if (root["access_token"].empty()||root["thermostat"].empty())
	{
		Log(LOG_ERROR, "Thermosmart: No access granted, check username/password...");
		return false;
	}
	...
	Log(LOG_STATUS, "Thermosmart: Login successfull!...");
...
void CThermosmart::GetMeterDetails()
{
	...
	if (!HTTPClient::GET(sURL, sResult))
	{
		Log(LOG_ERROR, "Thermosmart: Error getting thermostat data!");
		m_bDoLogin = true;
		return;
	}
...
void CThermosmart::SetSetpoint(const int idx, const float temp)
{
	...
	{
		Log(LOG_ERROR, "Thermosmart: Error setting thermostat data!");
		m_bDoLogin = true;
		return;
	}
...
void CThermosmart::SetPauseStatus(const bool bIsPause)
{
	...
	if (!HTTPClient::POST(sURL, szPostdata, ExtraHeaders, sResult))
	{
		Log(LOG_ERROR, "Thermosmart: Error setting Pause status!");
		m_bDoLogin = true;
		return;
	}
}
