...
void S0MeterBase::SendMeter(unsigned char ID, double musage, double mtotal)
{
	...
	if (meterype == device::tmeter::type::ENERGY)
	{
...
void S0MeterBase::ParseLine()
{
	...
	if (results[0][0] == '/')
	{
		...
		Log(LOG_STATUS, "S0 Meter: ID: %s, Version: %s", MeterID.c_str(), SoftwareVersion.c_str());
		return;
	}
	if (results.size() < 4)
	{
		Log(LOG_ERROR, "S0 Meter: Invalid Data received! %s", sLine.c_str());
		return;
	}
		...
		for (int ii = 0; ii < totmeters; ii++)
		{
...
#ifdef _DEBUG
				Log(LOG_STATUS, "S0 Meter: M%d, Watt: %.3f", ii + 1, m_meters[ii].m_CurrentUsage);
#endif
			}
			else
			{
			...
			if ((m_meters[ii].total_pulses != 0) || (m_meters[ii].m_firstTime))
			{
				...
				double counter_value =
					m_meters[ii].m_counter_start + (((double)(m_meters[ii].total_pulses - m_meters[ii].first_total_pulses_received) / ((double)m_meters[ii].m_pulse_per_unit)));
...
namespace http
{
	namespace server
	{
		void CWebServer::SetS0MeterType(WebEmSession &session, const request &req, std::string &redirect_uri)
		{
			...
			szExtra << S0M1Type << ";" << M1PulsesPerHour << ";" << S0M2Type << ";" << M2PulsesPerHour << ";" << S0M3Type << ";" << M3PulsesPerHour << ";" << S0M4Type << ";"
				<< M4PulsesPerHour << ";" << S0M5Type << ";" << M5PulsesPerHour;
