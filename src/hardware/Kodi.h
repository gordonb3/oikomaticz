...
class CKodiNode : public std::enable_shared_from_this<CKodiNode>, StoppableTask
{
	class CKodiStatus
	{
		...
		device::tmedia::status::value Status()
		{
			return m_nStatus;
		};
		...
		{
			return device::tmedia::status::Description(m_nStatus);
		};
		void Status(device::tmedia::status::value pStatus)
		{
			m_nStatus = pStatus;
		};
		...
		bool IsOn()
		{
			return (m_nStatus != device::tmedia::status::OFF);
		};
		bool IsStreaming()
		{
			return ((m_nStatus == device::tmedia::status::VIDEO) || (m_nStatus == device::tmedia::status::AUDIO) || (m_nStatus == device::tmedia::status::PAUSED) || (m_nStatus == device::tmedia::status::PHOTO));
		};
	...
	bool IsOn()
	{
		return (m_CurrentStatus.Status() != device::tmedia::status::OFF);
	};
