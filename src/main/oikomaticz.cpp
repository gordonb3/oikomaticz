/*
 ...
 along with Oikomaticz. If not, see <http://www.gnu.org/licenses/>.
*/
...
namespace
{
	constexpr const char *szHelp
	{
		...
		"\t-sslcert file_path (for example /opt/oikomaticz/server_cert.pem)\n"
		...
		"\t-pidfile pid file location (for example /var/run/oikomaticz.pid)\n"
		"\t-syslog [user|daemon|local0 .. local7] (use syslog as log output, defaults to facility 'user')\n"
		"\t-f config_file (for example /etc/oikomaticz.conf)\n"
#endif
		""
	};
...
#endif
{
	...
	if (szStartupFolder.empty())
	{
		...
		if(start_pos != std::string::npos) {
			szStartupFolder.replace(start_pos, 9, "\\oikomaticz\\");
	...
	if (dbasefile.empty()) {
		dbasefile = szUserDataFolder + "oikomaticz.db";
#ifdef WIN32
#ifndef _DEBUG
		if (!IsUserAnAdmin())
		{
	...
	if (!bUseConfigFile) {
		if (cmdLine.HasSwitch("-dbase"))
		{
			...
			dbasefile = cmdLine.GetSafeArgument("-dbase", 0, "oikomaticz.db");
		}
	}
	...
	if ((g_bRunAsDaemon)||(g_bUseSyslog))
	{
		...
		syslog(LOG_INFO, "Oikomaticz is starting up....");
	}
	...
	if ((g_bRunAsDaemon) && (g_bUseSyslog))
	{
		syslog(LOG_INFO, "Oikomaticz running...");
	}
#endif
	...
	if (g_bRunAsDaemon)
	{
		daemonShutdown();
		if (g_bUseSyslog)
			syslog(LOG_INFO, "Oikomaticz stopped...");
