...
namespace http {
	namespace server {
		...
		void CWebServerHelper::RestartProxy() {
			...
			// restart
#ifdef WWW_ENABLE_SSL
			cWebem *my_pWebEm = (plainServer_ != nullptr ? plainServer_->m_pWebEm : (secureServer_ != nullptr ? secureServer_->m_pWebEm : NULL));
#else
			cWebem* my_pWebEm = plainServer_ != nullptr ? plainServer_->m_pWebEm : NULL;
#endif
