...
#define DOMOTICZ_DB_VERSION 176
...
bool CSQLHelper::OpenDatabase()
{
	...
	if ((!bNewInstall) && (dbversion < DOMOTICZ_DB_VERSION))
	{
		...
		if (dbversion < 83)
		{
			...
			if (result.empty())
			{
				safe_query("INSERT INTO Hardware (Name, Enabled, Type, Address, Port, Username, Password, Mode1, Mode2, Mode3, Mode4, Mode5, Mode6) VALUES ('Motherboard',1, %d,'127.0.0.1',8085,'','',0,0,0,0,0,0)", hardware::type::System);
			}
		}
		if (dbversion < 85)
		{
		...
		if (dbversion < 100)
		{
			...
			szQuery << "SELECT ID FROM Hardware WHERE ( "
				...
				<< "([Type] = " << hardware::type::VisualCrossing << ") OR "
				<< "([Type] = " << hardware::type::AccuWeather << ") "
				<< ")";
			result = query(szQuery.str());
			if (!result.empty())
			{
...
// BETA 4.10268 - project separation
		...
		if (dbversion < 151)
		{
			...
			std::vector<std::vector<std::string>> result;
			result = safe_query("SELECT ID, Extra FROM Hardware WHERE Type=%d", hardware::type::MQTT);
			if (!result.empty())
			{
				for (const auto& sd : result)
				{
					...
					if (strarray.size() > 3)
					{
						safe_query("UPDATE Hardware SET Type=%d WHERE (ID=%q)", hardware::type::MQTTAutoDiscovery, ID.c_str());
					}
				}
			}
		}
			...
			if (!result.empty())
			{
				for (const auto& sd : result)
				{
					...
					for (const auto& sd2 : result2)
					{
						...
						if (szParams[0] == notification::type::Description(notification::type::TEMPERATURE, 1)[0])
						{
							szParams[0] = notification::type::Description(notification::type::SETPOINT, 1)[0];
		...
		{ // new version of BleBox module
			result = m_sql.safe_query("SELECT ID FROM HARDWARE WHERE ([Type]==%d)", hardware::type::BleBox);
			if (!result.empty())
			{
				int hwID = atoi(result[0][0].c_str());
					...
					for (const auto& sd : result)
					{
						...
						else if (deviceId.size() == 7)
						{
							...
							if (value1 == 168)
							{
								...
								int type = 0;
								if (sd[2] == std::to_string(device::tswitch::type::BlindsPercentage)) type = 1;
								if (sd[2] == std::to_string(device::tswitch::type::Dimmer)) type = 2;
		...
		if (dbversion < 170)
		{
			...
			m_sql.safe_query("UPDATE HARDWARE SET Port=443, SerialPort=433 WHERE ([Type]==%d) AND Port=80", hardware::type::Philips_Hue);
		}
		if (dbversion < 171)
		{
	...
	else
	{
		...
		else
		{
			if (
				...
				|| (stype == device::tswitch::type::SMOKEDETECTOR)
				)
			{
...
void CSQLHelper::UpdateMeter()
{
	...
	result = safe_query(
		...
		"(Type=%d AND SubType=%d) OR " //pTypeP1BusDevice, sTypeP1Gas
	...
	if (!result.empty())
	{
		for (const auto &sd : result)
		{
			...
			device::tmeter::type::value metertype = (device::tmeter::type::value)atoi(sd[11].c_str());
...
void CSQLHelper::AddCalendarUpdateMeter()
{
	...
	for (const auto &sddev : resultdevices)
	{
		...
		device::tmeter::type::value metertype = (device::tmeter::type::value)atoi(sd[6].c_str());
...
bool CSQLHelper::RestoreDatabaseFromFile(const std::string& sourceFilePath)
{
	...
	// 1. Validate that the source file is a valid Oikomaticz SQLite database
	sqlite3* dbase_restore = nullptr;
	...
	if (rc != SQLITE_OK)
	{
		_log.Log(LOG_ERROR, "Restore Database: Not a valid Oikomaticz database!");
		return false;
	}
...
void CSQLHelper::RefreshActualPrices()
{
	...
	if (!result.empty())
	{
		for (const auto& sd : result)
		{
			...
			unsigned char dType = atoi(sd[1].c_str());
			unsigned char dSubType = atoi(sd[2].c_str());
			device::tmeter::type::value metertype = (device::tmeter::type::value)atoi(sd[3].c_str());
