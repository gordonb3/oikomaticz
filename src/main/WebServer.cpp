...
namespace http
{
	namespace server
	{
		...
		void CWebServer::Cmd_GetHardwareTypes(WebEmSession &session, const request &req, Json::Value &root)
		{
			...
			for (int ii = 0; ii < hardware::type::END; ii++)
			{
				bool bDoAdd = true;
#ifndef _DEBUG
#ifdef WIN32
				if ((ii == hardware::type::RaspberryBMP085) || (ii == hardware::type::RaspberryHTU21D) || (ii == hardware::type::RaspberryTSL2561) || (ii == hardware::type::RaspberryPCF8574) ||
				    (ii == hardware::type::RaspberryBME280) || (ii == hardware::type::RaspberryMCP23017))
				{
					bDoAdd = false;
				}
				else
				{
#ifndef WITH_LIBUSB
					if ((ii == hardware::type::VoltcraftCO20) || (ii == hardware::type::TE923))
					{
						bDoAdd = false;
					}
#endif
		...
		void CWebServer::Cmd_AddHardware(WebEmSession &session, const request &req, Json::Value &root)
		{
			...
			if ((name.empty()) || (senabled.empty()) || (shtype.empty()))
				return;
			hardware::type::value htype = (hardware::type::value)atoi(shtype.c_str());
			...
			else if (IsNetworkDevice(htype))
			{
				...
				if (htype == hardware::type::MySensorsMQTT || htype == hardware::type::MQTT)
				{
			...
			else if (htype == hardware::type::DomoticzInternal)
			{
				// DomoticzInternal cannot be added manually
				return;
			}
			else if (htype == hardware::type::Domoticz)
			{
				// Remote Domoticz
				if (address.empty() || port == 0)
					return;
			}
			else if (htype == hardware::type::TE923)
			{
				// all fine here!
			}
			else if (htype == hardware::type::VoltcraftCO20)
			{
				// all fine here!
			}
			else if (htype == hardware::type::System)
			{
				// There should be only one
				std::vector<std::vector<std::string>> result;
				result = m_sql.safe_query("SELECT ID FROM Hardware WHERE (Type==%d)", hardware::type::System);
				if (!result.empty())
					return;
			}
			else if (htype == hardware::type::OneWire)
			{
				// all fine here!
			}
			else if (htype == hardware::type::Rtl433)
			{
				// all fine here!
			}
			else if (htype == hardware::type::Pinger)
			{
				// all fine here!
			}
			else if (htype == hardware::type::Kodi)
			{
				// all fine here!
			}
			else if (htype == hardware::type::PanasonicTV)
			{
				// all fine here!
			}
			else if (htype == hardware::type::LogitechMediaServer)
			{
				// all fine here!
			}
			else if (htype == hardware::type::RaspberryBMP085)
			{
				// all fine here!
			}
			else if (htype == hardware::type::RaspberryHTU21D)
			{
				// all fine here!
			}
			else if (htype == hardware::type::RaspberryTSL2561)
			{
				// all fine here!
			}
			else if (htype == hardware::type::RaspberryBME280)
			{
				// all fine here!
			}
			else if (htype == hardware::type::RaspberryMCP23017)
			{
				// all fine here!
			}
			else if (htype == hardware::type::Dummy)
			{
				// all fine here!
			}
			else if (htype == hardware::type::Tellstick)
			{
				// all fine here!
			}
			else if (htype == hardware::type::EVOHOME_SCRIPT || htype == hardware::type::EVOHOME_SERIAL || htype == hardware::type::EVOHOME_WEB || htype == hardware::type::EVOHOME_TCP)
			{
				// all fine here!
			}
			else if (htype == hardware::type::PiFace)
			{
				// all fine here!
			}
			else if (htype == hardware::type::HTTPPOLLER)
			{
				// all fine here!
			}
			...
			else if (htype == hardware::type::HEOS)
			{
				// all fine here!
			}
			else if (htype == hardware::type::Yeelight)
			{
				// all fine here!
			}
			else if (htype == hardware::type::XiaomiGateway)
			{
				// all fine here!
			}
			else if (htype == hardware::type::Arilux)
			{
				// all fine here!
			}
			...
			else if ((htype == hardware::type::Wunderground) || (htype == hardware::type::DarkSky) || (htype == hardware::type::AccuWeather) || (htype == hardware::type::OpenWeatherMap) || (htype == hardware::type::ICYTHERMOSTAT) ||
			...
			else if (htype == hardware::type::SolarEdgeAPI)
			{
				if ((username.empty()))
					return;
			}
			else if (htype == hardware::type::Nest_OAuthAPI)
			{
				if ((username.empty()) && (extra == "||"))
					return;
			}
			else if (htype == hardware::type::SBFSpot)
			{
				if (username.empty())
					return;
			}
			else if (htype == hardware::type::HarmonyHub)
			{
				if ((address.empty() || port == 0))
					return;
			}
			else if (htype == hardware::type::Philips_Hue)
			{
			...
			else if (htype == hardware::type::WINDDELEN)
			{
			...
			else if (htype == hardware::type::HoneywellLyric)
			{
				// all fine here!
			}
			else if (htype == hardware::type::RaspberryGPIO)
			{
				// all fine here!
			}
			else if (htype == hardware::type::SysfsGpio)
			{
				// all fine here!
			}
			else if (htype == hardware::type::OpenWebNetTCP)
			{
				// All fine here
			}
			else if (htype == hardware::type::Daikin)
			{
				// All fine here
			}
			else if (htype == hardware::type::GoodweAPI)
			{
				if (username.empty())
					return;
			}
			else if (htype == hardware::type::PythonPlugin)
			{
				// All fine here
			}
			else if (htype == hardware::type::RaspberryPCF8574)
			{
				// All fine here
			}
			else if (htype == hardware::type::OpenWebNetUSB)
			{
				// All fine here
			}
			else if (htype == hardware::type::IntergasInComfortLAN2RF)
			{
				// All fine here
			}
			...
			else if ((htype == hardware::type::S0SmartMeterUSB) || (htype == hardware::type::S0SmartMeterTCP))
			{
		...
		void CWebServer::Cmd_UpdateHardware(WebEmSession &session, const request &req, Json::Value &root)
		{
			...
			if (IsSerialDevice(htype))
			{
				...
				if (bEnabled)
				{
			...
			else if ((htype == hardware::type::RFXLAN) || (htype == hardware::type::P1SmartMeterLAN) || (htype == hardware::type::YouLess) || (htype == hardware::type::OpenThermGatewayTCP) || (htype == hardware::type::LimitlessLights) ||
				 ...
				 (htype == hardware::type::MySensorsTCP) || (htype == hardware::type::MySensorsMQTT) || (htype == hardware::type::MQTT) || (htype == hardware::type::TTN_MQTT) || (htype == hardware::type::FRITZBOX) ||
				 (htype == hardware::type::ETH8020) || (htype == hardware::type::Sterbox) || (htype == hardware::type::KMTronicTCP) || (htype == hardware::type::KMTronicUDP) || (htype == hardware::type::SOLARMAXTCP) ||
				 (htype == hardware::type::RelayNet) || (htype == hardware::type::SatelIntegra) || (htype == hardware::type::eHouseTCP) || (htype == hardware::type::RFLINKTCP) ||
				 (htype == hardware::type::Comm5TCP || (htype == hardware::type::Comm5SMTCP) || (htype == hardware::type::CurrentCostMeterLAN)) || (htype == hardware::type::NefitEastLAN) ||
				 (htype == hardware::type::DenkoviHTTPDevices) || (htype == hardware::type::DenkoviTCPDevices) || (htype == hardware::type::Ec3kMeterTCP) || (htype == hardware::type::MultiFun) ||
				 (htype == hardware::type::ZIBLUETCP) || (htype == hardware::type::OnkyoAVTCP) || (htype == hardware::type::OctoPrint) || (htype == hardware::type::TeleinfoMeterTCP))
			{
				// Lan
				if (address.empty())
					return;
		...
		void CWebServer::Cmd_DeleteHardware(WebEmSession &session, const request &req, Json::Value &root)
		{
			...
			if ((pBaseHardware != nullptr) && (pBaseHardware->HwdType == hardware::type::DomoticzInternal))
			{
		...
		void CWebServer::Cmd_GetForecastConfig(WebEmSession &session, const request &req, Json::Value &root)
		{
			...
			if (root["Forecasthardware"] > 0)
			{
				...
				{
					if (pHardware->HwdType == hardware::type::OpenWeatherMap)
					{
						root["Forecasthardwaretype"] = hardware::type::OpenWeatherMap;
					...
					{
						root["Forecasthardwaretype"] = hardware::type::BuienRadar;
			...
			*/
			...
			else if (cparam == "getmanualhardware")
			{
				...
				if (!result.empty())
				{
					...
					for (const auto &sd : result)
					{
						...
						hardware::type::value Type = (hardware::type::value)atoi(sd[2].c_str());

						if ((Type == hardware::type::RFXLAN) || (Type == hardware::type::RFXtrx315) || (Type == hardware::type::RFXtrx433) || (Type == hardware::type::RFXtrx868) || (Type == hardware::type::EnOceanESP2) ||
						    (Type == hardware::type::EnOceanESP3) || (Type == hardware::type::Dummy) || (Type == hardware::type::Tellstick) || (Type == hardware::type::EVOHOME_SCRIPT) ||
						    (Type == hardware::type::EVOHOME_SERIAL) || (Type == hardware::type::EVOHOME_WEB) || (Type == hardware::type::EVOHOME_TCP) || (Type == hardware::type::RaspberryGPIO) ||
						    (Type == hardware::type::RFLINKUSB) || (Type == hardware::type::RFLINKTCP) || (Type == hardware::type::ZIBLUEUSB) || (Type == hardware::type::ZIBLUETCP) || (Type == hardware::type::OpenWebNetTCP) ||
						    (Type == hardware::type::OpenWebNetUSB) || (Type == hardware::type::SysfsGpio) || (Type == hardware::type::USBtinGateway))
						{
							root["result"][ii]["idx"] = ID;
							root["result"][ii]["Name"] = Name;
							ii++;
						...
						switch (Type)
						{
							...
							case pTypeHunter:
								...
								if (bdoAdd)
								{
									...
									bool bIsDimmer = ((switchtype == device::tswitch::type::Dimmer) || (switchtype == device::tswitch::type::BlindsPercentage) ||
											  (switchtype == device::tswitch::type::BlindsPercentageInverted) || (switchtype == device::tswitch::type::Selector));
			...
			else if (cparam == "testswitch")
			{
				...
				if (lighttype == 70)
				{
					...
					if (pBaseHardware == nullptr)
						return;
					if ((pBaseHardware->HwdType != hardware::type::EnOceanESP2) && (pBaseHardware->HwdType != hardware::type::EnOceanESP3) && (pBaseHardware->HwdType != hardware::type::USBtinGateway))
						return;
					unsigned long rID = 0;
					if (pBaseHardware->HwdType == hardware::type::EnOceanESP2)
					{
						CEnOceanESP2 *pEnoceanHardware = reinterpret_cast<CEnOceanESP2 *>(pBaseHardware);
						rID = pEnoceanHardware->m_id_base + iUnitTest;
					}
					else if (pBaseHardware->HwdType == hardware::type::EnOceanESP3)
					{
						CEnOceanESP3 *pEnoceanHardware = reinterpret_cast<CEnOceanESP3 *>(pBaseHardware);
						rID = pEnoceanHardware->m_id_base + iUnitTest;
					}
					else if (pBaseHardware->HwdType == hardware::type::USBtinGateway) // Like EnOcean (Lighting2 with Base_ID offset)
					{
				...
				else if (lighttype == 68)
				{
					...
					if (pGpio->HwdType != hardware::type::RaspberryGPIO)
					{
						root["status"] = "ERROR";
				...
				else if (lighttype == 69)
				{
					...
					if (pSysfsGpio->HwdType != hardware::type::SysfsGpio)
					{
						root["status"] = "ERROR";
				...
				if (pBaseHardware != nullptr)
				{
					if ((pBaseHardware->HwdType == hardware::type::RFLINKUSB) || (pBaseHardware->HwdType == hardware::type::RFLINKTCP))
					{
			...
			else if (cparam == "addswitch")
			{
				...
				if (lighttype == 70)
				{
					...
					else if (pBaseHardware->HwdType == hardware::type::USBtinGateway)
					{
						USBtin *pUSBtinHardware = reinterpret_cast<USBtin *>(pBaseHardware);
						rID = pUSBtinHardware->switch_id_base;
						std::stringstream ssunitcode;
				...
				else if (lighttype == 69)
				{
					...
					if ((pSysfsGpio == nullptr) || (pSysfsGpio->HwdType != hardware::type::SysfsGpio))
					{
						return;
					}
#else
		...
		void CWebServer::PostSettings(WebEmSession &session, const request &req, reply &rep)
		{
			...
			m_sql.UpdatePreferencesVar("Title", (Title.empty()) ? "Oikomaticz" : Title);
						...
						const std::string &username, const std::string &hardwareid)
		{
			...
			if (!result.empty())
			{
				for (const auto &sd : result)
				{
					...
					tlist.HardwareType = hardware::type::Long_Desc(tlist.HardwareTypeVal);
#else
					if (tlist.HardwareTypeVal != hardware::type::PythonPlugin)
					{
						tlist.HardwareType = hardware::type::Long_Desc(tlist.HardwareTypeVal);
					}
					else
			...
			if (!result.empty())
			{
				for (const auto &sd : result)
				{
					...
					if (pHardware != nullptr)
					{
						if (pHardware->HwdType == hardware::type::SolarEdgeAPI)
						{
							int seSensorTimeOut = 60 * 24 * 60;
							bHaveTimeout = (now - checktime >= seSensorTimeOut * 60);
						}
						else if (pHardware->HwdType == hardware::type::Wunderground)
						{
						...
						else if (pHardware->HwdType == hardware::type::DarkSky)
						{
						...
						else if (pHardware->HwdType == hardware::type::AccuWeather)
						{
						...
						else if (pHardware->HwdType == hardware::type::OpenWeatherMap)
						{
							COpenWeatherMap *pWHardware = reinterpret_cast<COpenWeatherMap *>(pHardware);
							std::string forecast_url = pWHardware->GetForecastURL();
							if (!forecast_url.empty())
						...
						else if (pHardware->HwdType == hardware::type::Meteorologisk)
						{
					    ...
					    (dType == pTypeHunter))
					{
						...
						{
							if ((pHardware != nullptr) && (pHardware->HwdType == hardware::type::LogitechMediaServer))
					...
					else if (dType == pTypeSecurity1)
					{
						...
						if ((dSubType == sTypeKD101) || (dSubType == sTypeSA30) || (dSubType == sTypeRM174RF) || (switchtype == device::tswitch::type::SMOKEDETECTOR))
						{
							root["result"][ii]["SwitchTypeVal"] = device::tswitch::type::SMOKEDETECTOR;
							root["result"][ii]["TypeImg"] = "smoke";
							root["result"][ii]["SwitchType"] = device::tswitch::type::Description(device::tswitch::type::SMOKEDETECTOR);
		...
		void CWebServer::RType_Hardware(WebEmSession &session, const request &req, Json::Value &root)
		{
			...
			if (!result.empty())
			{
				...
				for (const auto &sd : result)
				{
					...
					if (pHardware != nullptr)
					{
						if ((pHardware->HwdType == hardware::type::RFXtrx315) || (pHardware->HwdType == hardware::type::RFXtrx433) || (pHardware->HwdType == hardware::type::RFXtrx868) ||
						...
						else if ((pHardware->HwdType == hardware::type::MySensorsUSB) || (pHardware->HwdType == hardware::type::MySensorsTCP) || (pHardware->HwdType == hardware::type::MySensorsMQTT))
						{
							MySensorsBase *pMyHardware = reinterpret_cast<MySensorsBase *>(pHardware);
							root["result"][ii]["version"] = pMyHardware->GetGatewayVersion();
						}
						else if ((pHardware->HwdType == hardware::type::OpenThermGateway) || (pHardware->HwdType == hardware::type::OpenThermGatewayTCP))
						{
							OTGWBase *pMyHardware = reinterpret_cast<OTGWBase *>(pHardware);
							root["result"][ii]["version"] = pMyHardware->m_Version;
						}
						else if ((pHardware->HwdType == hardware::type::RFLINKUSB) || (pHardware->HwdType == hardware::type::RFLINKTCP))
						{
		...
		void CWebServer::RType_SetUsed(WebEmSession &session, const request &req, Json::Value &root)
		{
			...
			if (!strunit.empty())
			{
				...
				{
					std::vector<std::string> sd = result[0];
					hardware::type::value Type = (hardware::type::value)dType;
					if (Type == hardware::type::PythonPlugin)
					{
						bUpdateUnit = false;
