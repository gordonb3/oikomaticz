...
)
{
	...
	case hardware::type::P1SmartMeter:
		pHardware = new P1MeterSerial(ID, SerialPort, (Mode1 == 1) ? 115200 : 9600, (Mode2 != 0), Mode3, Mode4, Password);
		break;
...
// Evohome
	...
	case hardware::type::P1SmartMeterLAN:
		//LAN
		pHardware = new P1MeterTCP(ID, Address, Port, (Mode2 != 0), Mode3, Mode4, Password);
		break;
	...
	case hardware::type::DarkSky:
		...
		break;
	case hardware::type::VisualCrossing:
...
// MyHome
	...
	case hardware::type::AlfenEveCharger:
		...
		break;
	case hardware::type::EneverPriceFeeds:
...
void MainWorker::decode_Temp(const CDomoticzHardwareBase* pHardware, const tRBUF* pResponse, _tRxMessageProcessingResult& procResult)
{
	...
	else if ((pHardware->HwdType == hardware::type::EnOceanESP2) || (pHardware->HwdType == hardware::type::EnOceanESP3))
	{
		// WARNING
...
void MainWorker::decode_Fan(const CDomoticzHardwareBase* pHardware, const tRBUF* pResponse, _tRxMessageProcessingResult& procResult)
{
	...
	if (pResponse->ICMND.subtype == sTypeOrcon)
	{
		...
		if (!result.empty())
		{
			...
			if (switchType == device::tswitch::type::Selector && !options.empty())
			{
				...
				GetLightStatus(devType, subType, (const device::tswitch::type::value)switchType, cmnd, sValue, lstatus, llevel, bHaveDimmer, maxDimLevel, bHaveGroupCmd);
		...
		if (strarray.size() >= 3)
		{
			if (pEvo->updatetype == pEvoHW->updSetPoint)//SetPoint
			{
				...
				if (pEvo->mode <= pEvoHW->zmTmp)//for the moment only update this if we get a valid setpoint mode as we can now send setpoint on its own
				{
					...
					else if ((pEvo->mode == pEvoHW->zmAuto) && (pHardware->HwdType == hardware::type::EVOHOME_WEB))
					{
...
void MainWorker::decode_RFXSensor(const CDomoticzHardwareBase* pHardware, const tRBUF* pResponse, _tRxMessageProcessingResult& procResult)
{
	...
	if ((pHardware->HwdType == hardware::type::EnOceanESP2) || (pHardware->HwdType == hardware::type::EnOceanESP3))
	{
		// WARNING
...
bool MainWorker::GetSensorData(const uint64_t idx, int& nValue, std::string& sValue)
{
	...
	else if (devType == pTypeP1BusDevice)
	{
		...
		if (!result2.empty())
		{
			...
			uint64_t total_min_usage = std::stoull(sd2[0].c_str());
			uint64_t actual_usage = std::stoull(sValue);
			uint64_t total_today_usage = actual_usage - total_min_usage;
...
MainWorker::eSwitchLightReturnCode MainWorker::SwitchLightInt(const std::vector<std::string>& sd, std::string switchcmd, int level, const _tColor color, const bool IsTesting, const std::string& User)
{
	...
	case pTypeFan:
	{
		...
		std::string slevel;
		if ((switchtype == device::tswitch::type::Selector) && ((switchcmd == "Set Level") || (switchcmd == "Set Group Level")))
		{
		...
		if (dSubType == sTypeOrcon)
		{
			...
			// Destination ID = Fan ID Oikomaticz - we're sending TO the fan
			lcmd.FAN2.did1 = ID2;
			lcmd.FAN2.did2 = ID3;
			lcmd.FAN2.did3 = ID4;
			lcmd.FAN2.filler = 0;
...
bool MainWorker::SetSetPointInt(const std::vector<std::string>& sd, const float TempValue, const std::string& User)
{
	...
	if (pHardware->HwdType == hardware::type::PythonPlugin)
	{
		...
		|| (pHardware->HwdType == hardware::type::MQTTAutoDiscovery)
		|| (pHardware->HwdType == hardware::type::AlfenEveCharger)
		)
	{
		...
		else if (pHardware->HwdType == hardware::type::AlfenEveCharger)
		{
...
bool MainWorker::SetThermostatState(const std::string& idx, const int newState)
{
	...
	if (pHardware->HwdType == hardware::type::NestThermostat)
	{
		CNest* pGateway = dynamic_cast<CNest*>(pHardware);
		pGateway->SetProgramState(newState);
		return true;
	}
	if (pHardware->HwdType == hardware::type::Nest_OAuthAPI)
	{
		CNestOAuthAPI* pGateway = dynamic_cast<CNestOAuthAPI*>(pHardware);
		pGateway->SetProgramState(newState);
		return true;
	}
	if (pHardware->HwdType == hardware::type::ANNATHERMOSTAT)
	{
		CAnnaThermostat* pGateway = dynamic_cast<CAnnaThermostat*>(pHardware);
		pGateway->SetProgramState(newState);
		return true;
	}
	if (pHardware->HwdType == hardware::type::Netatmo)
	{
		CNetatmo* pGateway = dynamic_cast<CNetatmo*>(pHardware);
	...
	if (pHardware->HwdType == hardware::type::IntergasInComfortLAN2RF)
	{
		CInComfort* pGateway = dynamic_cast<CInComfort*>(pHardware);
		pGateway->SetProgramState(newState);
		return true;
	...
	const std::string& userName, const int signallevel, const int batterylevel, const bool parseTrigger)
{
	...
	try
	{
		...
		if (pHardware)
		{
			if ((pHardware->HwdType == hardware::type::MySensorsUSB) || (pHardware->HwdType == hardware::type::MySensorsTCP) || (pHardware->HwdType == hardware::type::MySensorsMQTT))
			{
				unsigned long ID;
				std::stringstream s_strid;
				s_strid << std::hex << DeviceID;
	...
	if (iHP_E_Idx != 0)
	{
		...
		else
		{
			...
			if (!result.empty())
			{
				...
				if (pHardware != nullptr)
				{
					if (pHardware->HwdType == hardware::type::EneverPriceFeeds)
					{
