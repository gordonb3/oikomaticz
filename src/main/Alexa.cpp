...
void CWebServer::Alexa_HandleDiscovery(WebEmSession& session, const request& req, Json::Value& root)
{
			...
			for (const auto& temp_row : devices_result)
			{
				    ...
				    temp_name == device_name)
				{
					...
					for (const auto& mode_row : devices_result)
					{
						...
						if (mode_device_name == mode_name && mode_switch_type == device::tswitch::type::Selector)
						{
	...
	for (const auto& device_row : devices_result)
	{
		    ...
		    || ((device_type == pTypeRego6XXValue) && (device_subtype == sTypeRego6XXStatus)))
		{
			...
			if (switch_type == device::tswitch::type::Selector)
			{
			...
			// Handle blinds
			else if (switch_type == device::tswitch::type::Blinds || switch_type == device::tswitch::type::BlindsPercentage ||
			         switch_type == device::tswitch::type::BlindsPercentageWithStop || switch_type == device::tswitch::type::BlindsWithStop)
			{
				Json::Value endpoint = CreateEndpoint("blind_" + device_idx, device_name, "Blind", "INTERIOR_BLIND");

				bool has_percentage = (switch_type == device::tswitch::type::BlindsPercentage || switch_type == device::tswitch::type::BlindsPercentageWithStop);
				bool has_stop = (switch_type == device::tswitch::type::BlindsPercentageWithStop || switch_type == device::tswitch::type::BlindsWithStop);
			...
			// Handle door locks
			else if (switch_type == device::tswitch::type::DoorLock || switch_type == device::tswitch::type::DoorLockInverted)
			{
			...
			else if (switch_type == device::tswitch::type::OnOff || switch_type == device::tswitch::type::Dimmer)
			{
				...
				Json::Value endpoint = CreateEndpoint("switch_" + device_idx, device_name,
					has_color ? "RGB Light" : ((switch_type == device::tswitch::type::Dimmer) ? "Dimmable Light" : "Switch"),
					"LIGHT");
				...
				if (switch_type == device::tswitch::type::Dimmer)
				{
...
static void Alexa_HandleControl_PowerController(WebEmSession& session, const Json::Value& request_json, Json::Value& root, uint64_t device_idx, const std::string& directive_name)
{
	if (directive_name == "TurnOn" || directive_name == "TurnOff")
	{
		...
		else if (whatAmI == "switch")
		{
			...
			if (switch_type == device::tswitch::type::Dimmer && directive_name == "TurnOn")
			{
...
static void Alexa_HandleControl_ColorTemperatureController(WebEmSession& session, const Json::Value& request_json, Json::Value& root, uint64_t device_idx, const std::string& directive_name)
{
	if (directive_name == "SetColorTemperature")
	{
		...
		// Oikomaticz uses 0-255 for cool white and warm white levels
...
static void Alexa_HandleControl_LockController(WebEmSession& session, const Json::Value& request_json, Json::Value& root, uint64_t device_idx, const std::string& directive_name)
{
	if (directive_name == "Lock" || directive_name == "Unlock")
	{
		...
		bool is_inverted = (switch_type == device::tswitch::type::DoorLockInverted);
...
static void Alexa_HandleControl_ReportState(WebEmSession& session, const Json::Value& request_json, Json::Value& root, uint64_t device_idx)
{
	    ...
	    || ((dType == pTypeRego6XXValue) && (dSubType == sTypeRego6XXStatus)))
	{
		...
		else if (switchtype == device::tswitch::type::Blinds || switchtype == device::tswitch::type::BlindsPercentage ||
			 switchtype == device::tswitch::type::BlindsPercentageWithStop || switchtype == device::tswitch::type::BlindsWithStop)
		{
			root["context"]["properties"].append(CreateProperty("Alexa.RangeController", "rangeValue", level, "Blind.Lift"));
			root["context"]["properties"].append(CreateEndpointHealthProperty());
			return;
		}
		else if (switchtype == device::tswitch::type::DoorLock || switchtype == device::tswitch::type::DoorLockInverted)
		{
			// Door locks
			bool is_inverted = (switchtype == device::tswitch::type::DoorLockInverted);
		...
		else if (switchtype == device::tswitch::type::OnOff || switchtype == device::tswitch::type::Dimmer)
		{
